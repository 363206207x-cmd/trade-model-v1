...
public interface AiCallLogMapper {
    ...
    @Select("<script>"
            ...
            + "  SELECT ar.analysis_id FROM tm_analysis_run ar WHERE "
            + "    ((ar.owner_type = 'USER' AND ar.owner_id = #{userId}) OR "
            + "     (ar.owner_type = 'SYSTEM' AND EXISTS ("
            + "       SELECT 1 FROM tm_asset_pool_item ap WHERE UPPER(ap.symbol) = UPPER(ar.symbol) "
            + "       AND ap.active = TRUE AND ((ap.owner_type = 'USER' AND ap.owner_id = #{userId}) OR "
