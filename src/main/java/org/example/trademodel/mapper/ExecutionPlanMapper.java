...
public interface ExecutionPlanMapper {
    ...
    @Select("SELECT * FROM tm_execution_plan WHERE analysis_id = #{analysisId} "
            + "AND decision_id = #{decisionId} ORDER BY create_time DESC, plan_id DESC LIMIT 1")
    ExecutionPlanDO selectLatestByDecisionIdentity(@Param("analysisId") String analysisId,
