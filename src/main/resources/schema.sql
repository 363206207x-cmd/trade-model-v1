...
ALTER TABLE tm_analysis_run ALTER COLUMN analysis_mode SET DEFAULT 'OPPORTUNITY_DECISION';
ALTER TABLE tm_analysis_run ALTER COLUMN analysis_mode SET NOT NULL;
ALTER TABLE tm_analysis_run ADD CONSTRAINT IF NOT EXISTS ck_tm_analysis_run_mode
    CHECK (analysis_mode IN ('ANALYSIS_PREVIEW', 'OPPORTUNITY_DECISION'));
ALTER TABLE tm_analysis_run ADD CONSTRAINT IF NOT EXISTS ck_tm_analysis_run_preview_mode
    CHECK ((preview = TRUE AND analysis_mode = 'ANALYSIS_PREVIEW')
        OR (preview = FALSE AND analysis_mode = 'OPPORTUNITY_DECISION'));

ALTER TABLE tm_execution_plan ADD COLUMN IF NOT EXISTS plan_lifecycle_state VARCHAR(32);
ALTER TABLE tm_execution_plan ADD COLUMN IF NOT EXISTS plan_version INT NOT NULL DEFAULT 1;
ALTER TABLE tm_execution_plan ADD COLUMN IF NOT EXISTS supersedes_plan_id VARCHAR(64);
ALTER TABLE tm_execution_plan ADD COLUMN IF NOT EXISTS superseded_by_plan_id VARCHAR(64);
UPDATE tm_execution_plan
SET plan_lifecycle_state = CASE WHEN final_plan = TRUE THEN 'NEEDS_REVALIDATION' ELSE 'INVALIDATED' END
WHERE plan_lifecycle_state IS NULL;
ALTER TABLE tm_execution_plan ALTER COLUMN plan_lifecycle_state SET DEFAULT 'INVALIDATED';
ALTER TABLE tm_execution_plan ALTER COLUMN plan_lifecycle_state SET NOT NULL;
ALTER TABLE tm_execution_plan ADD CONSTRAINT IF NOT EXISTS ck_tm_execution_plan_lifecycle
    CHECK (plan_lifecycle_state IN (
        'CURRENT', 'NEEDS_REVALIDATION', 'SUPERSEDED',
        'TRACKING_STOPPED', 'INVALIDATED', 'EXPIRED'));
ALTER TABLE tm_execution_plan ADD CONSTRAINT IF NOT EXISTS ck_tm_execution_plan_version
    CHECK (plan_version > 0);

CREATE TABLE IF NOT EXISTS tm_plan_revalidation_record (
    ...
    not_trade_instruction BOOLEAN NOT NULL DEFAULT TRUE,
    not_order_execution BOOLEAN NOT NULL DEFAULT TRUE,
    CONSTRAINT ck_tm_plan_revalidation_trigger CHECK (trigger_type IN (
        'HOT_RESET', 'EVENT_WINDOW', 'DATA_REFRESH', 'EVIDENCE_CHANGED', 'MANUAL_REVALIDATION')),
    CONSTRAINT ck_tm_plan_revalidation_state CHECK (state IN (
    ...
    CONSTRAINT fk_tm_plan_revalidation_plan FOREIGN KEY (plan_id) REFERENCES tm_execution_plan(plan_id),
    CONSTRAINT fk_tm_plan_revalidation_result_plan FOREIGN KEY (result_plan_id) REFERENCES tm_execution_plan(plan_id)
);
CREATE INDEX IF NOT EXISTS idx_tm_plan_revalidation_plan
    ON tm_plan_revalidation_record(plan_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_tm_plan_revalidation_trace
...
CREATE TABLE IF NOT EXISTS tm_message (
    ...
    CONSTRAINT ck_tm_message_category CHECK (category IN (
        'HIGH_PERMISSION_OPPORTUNITY', 'OPPORTUNITY_PLAN_SAFETY_CHANGE', 'POSITION_LOGIC_RISK_CHANGE')),
    CONSTRAINT ck_tm_message_business_state CHECK (business_state IN ('ACTIVE', 'EXPIRED', 'RESOLVED')),
    CONSTRAINT ck_tm_message_read_state CHECK (read_state IN ('UNREAD', 'READ')),
    CONSTRAINT ck_tm_message_safety CHECK (not_trade_instruction = TRUE AND not_order_execution = TRUE)
);
CREATE INDEX IF NOT EXISTS idx_tm_message_user_time ON tm_message(user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_tm_message_source ON tm_message(source_type, source_id);

CREATE TABLE IF NOT EXISTS tm_channel_delivery (
...
CREATE TABLE IF NOT EXISTS tm_async_task (
    ...
    CONSTRAINT ck_tm_async_task_owner CHECK (
        (owner_type = 'SYSTEM' AND owner_id = 0) OR (owner_type = 'USER' AND owner_id > 0)),
    CONSTRAINT ck_tm_async_task_type CHECK (task_type IN (
        'POOL_SCAN', 'ANALYSIS_PREVIEW', 'REANALYSIS', 'THREE_AI', 'PLAN_REVALIDATION', 'HOT_RESET')),
    CONSTRAINT ck_tm_async_task_state CHECK (state IN (
        'QUEUED', 'RUNNING', 'PARTIAL', 'SUCCEEDED', 'FAILED', 'CANCELLED')),
    CONSTRAINT ck_tm_async_task_retry CHECK (
        retry_count >= 0 AND max_retries >= 0 AND retry_count <= max_retries)
);
CREATE INDEX IF NOT EXISTS idx_tm_async_task_owner_time
...
CREATE TABLE IF NOT EXISTS tm_event_asset_relation (
    ...
    trace_id VARCHAR(128),
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT ck_tm_event_asset_event_type CHECK (event_type IN ('MACRO', 'INDUSTRY', 'PROJECT', 'HOT_RESET')),
    CONSTRAINT ck_tm_event_asset_relation_type CHECK (relation_type IN (
...
CREATE INDEX IF NOT EXISTS idx_tm_event_asset_symbol_time
    ON tm_event_asset_relation(symbol, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_tm_event_asset_event
    ON tm_event_asset_relation(event_type, event_id);

ALTER TABLE tm_account_risk_snapshot
    ADD COLUMN IF NOT EXISTS account_risk_coverage_state VARCHAR(16) NOT NULL DEFAULT 'UNKNOWN';
ALTER TABLE tm_account_risk_snapshot ADD CONSTRAINT IF NOT EXISTS ck_tm_account_risk_coverage
    CHECK (account_risk_coverage_state IN ('COMPLETE', 'PARTIAL', 'UNKNOWN'));

ALTER TABLE tm_review_result ADD COLUMN IF NOT EXISTS missed_reason VARCHAR(32);
ALTER TABLE tm_review_result ADD COLUMN IF NOT EXISTS later_outcome VARCHAR(16);
ALTER TABLE tm_review_result ADD CONSTRAINT IF NOT EXISTS ck_tm_review_missed_reason
    CHECK (missed_reason IS NULL OR missed_reason IN (
        'NOT_TRIGGERED', 'BLOCKED_BY_SYSTEM', 'PUSHED_NOT_FILLED', 'USER_SKIPPED'));
ALTER TABLE tm_review_result ADD CONSTRAINT IF NOT EXISTS ck_tm_review_later_outcome
    CHECK (later_outcome IS NULL OR later_outcome IN ('VALID', 'INVALID', 'INCONCLUSIVE'));

ALTER TABLE tm_user_config ADD COLUMN IF NOT EXISTS telegram_chat_id VARCHAR(128);
ALTER TABLE tm_user_config ADD COLUMN IF NOT EXISTS telegram_binding_status VARCHAR(24) NOT NULL DEFAULT 'UNBOUND';
ALTER TABLE tm_user_config ADD COLUMN IF NOT EXISTS notification_filters_json CLOB;
ALTER TABLE tm_user_config ADD COLUMN IF NOT EXISTS default_pool_mode VARCHAR(24) NOT NULL DEFAULT 'SYSTEM_DEFAULT';
ALTER TABLE tm_user_config ADD CONSTRAINT IF NOT EXISTS ck_tm_user_config_telegram_binding
    CHECK (telegram_binding_status IN ('UNBOUND', 'PENDING', 'BOUND', 'FAILED'));
ALTER TABLE tm_user_config ADD CONSTRAINT IF NOT EXISTS ck_tm_user_config_default_pool
    CHECK (default_pool_mode IN ('SYSTEM_DEFAULT', 'USER_CUSTOM'));
