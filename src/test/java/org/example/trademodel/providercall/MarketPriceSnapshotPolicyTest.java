...
class MarketPriceSnapshotPolicyTest {
    ...
    @Test void stalePositiveSnapshotIsExplicitlyStale() { assertThat(MarketPriceSnapshotPolicy.isStale(result("1", SnapshotFreshnessStatus.STALE, null))).isTrue(); }
    @Test void staleSnapshotReturnsQuoteStaleCode() { assertThat(MarketPriceSnapshotPolicy.failureCode(result("1", SnapshotFreshnessStatus.STALE, null))).isEqualTo("QUOTE_STALE"); }
    @Test void unavailableSnapshotPreservesProviderCode() { assertThat(MarketPriceSnapshotPolicy.failureCode(result(null, SnapshotFreshnessStatus.ERROR, "QUOTE_UNAVAILABLE"))).isEqualTo("QUOTE_UNAVAILABLE"); }
    ...
    private ProviderCallResult<MarketPriceSnapshot> result(String price, SnapshotFreshnessStatus freshness, String error) {
        ...
        ProviderSnapshotMetadata metadata = new ProviderSnapshotMetadata("TEST", ProviderDatasetType.PRICE,
                ...
                freshness, "trace", "key", false, freshness == SnapshotFreshnessStatus.STALE, error,
