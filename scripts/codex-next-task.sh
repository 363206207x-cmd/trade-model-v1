...
validate_contract_task() {
  ...
  local current_package_phase current_package_mode current_package_status current_package_branch current_package_starting_full_sha
  ...
  local official_domain_home_analysis_telegram_closure_authorization_status official_domain_home_analysis_telegram_closure_implementation_status
  ...
  official_domain_home_analysis_telegram_closure_authorization_status="$(yaml_value "$TASK_FILE" v4_1_official_domain_home_analysis_telegram_closure_authorization_status)"
  official_domain_home_analysis_telegram_closure_implementation_status="$(yaml_value "$TASK_FILE" v4_1_official_domain_home_analysis_telegram_closure_implementation_status)"
  ...
  [[ "$current_package_phase" == "TRINE_LOGIC_V4_1_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_AUTHORIZATION" && "$current_package_status" == "COMPLETED" ]] || { echo "TASK_VALIDATION_FAILED official-domain Home-analysis and Telegram authorization declaration mismatch" >&2; failed=1; }
  ...
  [[ "$current_package_branch" == "codex/v4-1-official-domain-home-analysis-telegram-authorization" ]] || { echo "TASK_VALIDATION_FAILED official-domain Home-analysis and Telegram authorization branch mismatch" >&2; failed=1; }
  [[ "$current_package_starting_full_sha" == "aa683013f401cceb289b7bbc245a8ea75e00f3c1" ]] || { echo "TASK_VALIDATION_FAILED official-domain Home-analysis and Telegram authorization starting SHA mismatch" >&2; failed=1; }
  [[ "$current_package_edits" == "true" && "$current_package_implementation" == "false" && "$current_package_pr" == "true" && "$current_package_push" == "true" && "$current_package_merge" == "true" && "$current_package_deployment" == "false" ]] || { echo "TASK_VALIDATION_FAILED official-domain Home-analysis and Telegram authorization permissions mismatch" >&2; failed=1; }
  ...
  [[ "$authorized_next_phase" == "V41_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE" && "$authorized_next_mode" == "IMPLEMENTATION" ]] || { echo "TASK_VALIDATION_FAILED authorized official-domain Home-analysis and Telegram package mismatch" >&2; failed=1; }
  ...
  [[ "$authorized_next_edits" == "true" && "$authorized_next_implementation" == "true" && "$authorized_next_pr" == "true" && "$authorized_next_push" == "true" && "$authorized_next_merge" == "true" && "$authorized_next_deployment" == "false" && "$authorized_next_staging_deployment" == "true" && "$authorized_next_production_deployment" == "false" && "$authorized_next_canonical_figma" == "false" && "$authorized_next_mobile" == "false" && "$authorized_next_canonical_figma_key" == "NONE" ]] || { echo "TASK_VALIDATION_FAILED bounded official-domain Home-analysis and Telegram permissions are incomplete" >&2; failed=1; }
  [[ "$(printf '%s\n' "$authorized_next_allowed_paths" | sed '/^$/d' | wc -l | tr -d ' ')" == "20" ]] || { echo "TASK_VALIDATION_FAILED official-domain Home-analysis and Telegram closure must contain exactly twenty implementation paths" >&2; failed=1; }
  if printf '%s\n' "$authorized_next_allowed_paths" | grep -Eq '[*?]|(^|/)(src|docs|scripts)/?$'; then
    echo "TASK_VALIDATION_FAILED official-domain Home-analysis and Telegram allowlist must not contain wildcards or directory grants" >&2
    failed=1
  fi
...
official_domain_home_analysis_telegram_closure_authorization_runtime_status="$(state_value "$state_text" V4_1_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_AUTHORIZATION_STATUS)"
official_domain_home_analysis_telegram_closure_implementation_runtime_status="$(state_value "$state_text" V4_1_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_IMPLEMENTATION_STATUS)"
  ...
  AUTHORIZED_NEXT_PACKAGE)
    ...
    case "$resolved_mode" in
      ...
      IMPLEMENTATION)
        ...
        elif [[ "$resolved_package" == "ANALYSIS_RUN_IDEMPOTENCY_TRANSACTION_BOUNDARY_FIX" || "$resolved_package" == "V41_REAL_PROVIDER_AND_THREE_AI_RUNTIME_CLOSURE" || "$resolved_package" == "V41_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE" || "$resolved_package" == "V41_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE" ]]; then
...
V4_1_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_AUTHORIZATION_STATUS: $official_domain_home_analysis_telegram_closure_authorization_runtime_status
V4_1_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_IMPLEMENTATION_STATUS: $official_domain_home_analysis_telegram_closure_implementation_runtime_status
