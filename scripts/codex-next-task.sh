...
validate_contract_task() {
  ...
  if [[ "$current_package_phase" == "TRINE_LOGIC_V4_1_WEB_RUNTIME_TRUTH_PLAN_CLOSURE_AUTHORIZATION" ]]; then
    ...
    [[ "$(path_list_fingerprint "$authorized_next_allowed_paths")" == "ed5ef17092582f1d8672660c81ae1e647956ad40" ]] || { echo "TASK_VALIDATION_FAILED Web implementation allowlist fingerprint mismatch" >&2; failed=1; }
    ...
    [[ "$(yaml_value "$TASK_FILE" web_runtime_truth_plan_implementation_allowlist_count)" == "46" && "$(yaml_value "$TASK_FILE" web_runtime_truth_plan_gate_allowlist_count)" == "10" ]] || { echo "TASK_VALIDATION_FAILED runtime truth path counts mismatch" >&2; failed=1; }
    [[ "$(yaml_value "$TASK_FILE" web_runtime_truth_plan_implementation_allowlist_fingerprint)" == "ed5ef17092582f1d8672660c81ae1e647956ad40" && "$(yaml_value "$TASK_FILE" web_runtime_truth_plan_gate_allowlist_fingerprint)" == "935d97dd7525424f366e3c86610e867b72081208" ]] || { echo "TASK_VALIDATION_FAILED runtime truth registered fingerprints mismatch" >&2; failed=1; }
