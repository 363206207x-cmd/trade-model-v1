...
validate_contract_task() {
  ...
  if [[ "$current_package_phase" == "TRINE_LOGIC_V4_1_WEB_RUNTIME_TRUTH_PLAN_CLOSURE_AUTHORIZATION" ]]; then
    ...
    [[ "$current_package_starting_full_sha" == "0d6a1e36518c4bf935bba7e7074ec87edbd6947f" ]] || { echo "TASK_VALIDATION_FAILED Web live direction and risk closure authorization starting SHA mismatch" >&2; failed=1; }
    ...
    [[ "$(path_list_fingerprint "$current_package_allowed_paths")" == "935d97dd7525424f366e3c86610e867b72081208" ]] || { echo "TASK_VALIDATION_FAILED Web gate allowlist fingerprint mismatch" >&2; failed=1; }
    [[ "$(path_list_fingerprint "$authorized_next_allowed_paths")" == "968c25bf01511882be7eed34fed39e05c0e130f9" ]] || { echo "TASK_VALIDATION_FAILED Web implementation allowlist fingerprint mismatch" >&2; failed=1; }
    ...
    [[ "$(yaml_value "$TASK_FILE" authorized_next_package_branch)" == "codex/v4-1-web-home-runtime-priority-closure" ]] || { echo "TASK_VALIDATION_FAILED runtime truth branch mismatch" >&2; failed=1; }
    [[ "$(yaml_value "$TASK_FILE" web_runtime_truth_plan_implementation_allowlist_count)" == "58" && "$(yaml_value "$TASK_FILE" web_runtime_truth_plan_gate_allowlist_count)" == "10" ]] || { echo "TASK_VALIDATION_FAILED runtime truth path counts mismatch" >&2; failed=1; }
    [[ "$(yaml_value "$TASK_FILE" web_runtime_truth_plan_implementation_allowlist_fingerprint)" == "968c25bf01511882be7eed34fed39e05c0e130f9" && "$(yaml_value "$TASK_FILE" web_runtime_truth_plan_gate_allowlist_fingerprint)" == "935d97dd7525424f366e3c86610e867b72081208" ]] || { echo "TASK_VALIDATION_FAILED runtime truth registered fingerprints mismatch" >&2; failed=1; }
    ...
    [[ "$(yaml_value "$TASK_FILE" web_runtime_truth_plan_database_permission_window)" == "NONE_EXISTING_ASSET_POOL_STORAGE_NO_MIGRATION" && "$(yaml_value "$TASK_FILE" web_runtime_truth_plan_database_permission_recovery_required)" == "true" ]] || { echo "TASK_VALIDATION_FAILED runtime truth temporary migration boundary mismatch" >&2; failed=1; }
