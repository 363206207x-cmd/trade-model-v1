...
validate_contract_task() {
  ...
  local current_package_phase current_package_mode current_package_status current_package_branch current_package_starting_full_sha current_package_allowed_paths
  ...
  local v4_1_telegram_remediation_authorization_status v4_1_telegram_remediation_implementation_status
  ...
  local real_provider_three_ai_runtime_closure_authorization_status real_provider_three_ai_runtime_closure_implementation_status
  local official_domain_session_runtime_visibility_closure_authorization_status official_domain_session_runtime_visibility_closure_implementation_status
  ...
  current_package_edits="$(yaml_value "$TASK_FILE" current_package_repository_edits_allowed)"
  current_package_implementation="$(yaml_value "$TASK_FILE" current_package_implementation_allowed)"
  current_package_pr="$(yaml_value "$TASK_FILE" current_package_implementation_pr_allowed)"
  current_package_push="$(yaml_value "$TASK_FILE" current_package_push_allowed)"
  current_package_merge="$(yaml_value "$TASK_FILE" current_package_merge_allowed)"
  current_package_deployment="$(yaml_value "$TASK_FILE" current_package_deployment_allowed)"
  ...
  v4_1_telegram_remediation_authorization_status="$(yaml_value "$TASK_FILE" v4_1_telegram_remediation_authorization_status)"
  v4_1_telegram_remediation_implementation_status="$(yaml_value "$TASK_FILE" v4_1_telegram_remediation_implementation_status)"
  v4_1_core_production_loop_authorization_status="$(yaml_value "$TASK_FILE" v4_1_core_production_loop_authorization_status)"
  v4_1_core_production_loop_implementation_status="$(yaml_value "$TASK_FILE" v4_1_core_production_loop_implementation_status)"
  real_data_home_blocker_closure_authorization_status="$(yaml_value "$TASK_FILE" real_data_home_blocker_closure_authorization_status)"
  ...
  real_provider_three_ai_runtime_closure_authorization_status="$(yaml_value "$TASK_FILE" v4_1_real_provider_three_ai_runtime_closure_authorization_status)"
  ...
  official_domain_session_runtime_visibility_closure_authorization_status="$(yaml_value "$TASK_FILE" v4_1_official_domain_session_runtime_visibility_closure_authorization_status)"
  ...
  real_logic_chain_closure_authorization_status="$(yaml_value "$TASK_FILE" v4_1_real_logic_chain_closure_authorization_status)"
  real_logic_chain_closure_implementation_status="$(yaml_value "$TASK_FILE" v4_1_real_logic_chain_closure_implementation_status)"
  web_live_direction_risk_closure_authorization_status="$(yaml_value "$TASK_FILE" v4_1_web_live_direction_risk_closure_authorization_status)"
  ...
  if [[ "$current_package_phase" == "TRINE_LOGIC_V4_1_WEB_RUNTIME_TRUTH_PLAN_CLOSURE_AUTHORIZATION" ]]; then
    ...
    [[ "$current_package_branch" == "codex/v4-1-web-runtime-truth-plan-closure-authorization" ]] || { echo "TASK_VALIDATION_FAILED Web live direction and risk closure authorization branch mismatch" >&2; failed=1; }
    [[ "$current_package_starting_full_sha" == "f4dd92dae4623a42a2bff626bc653c4db5bde3b0" ]] || { echo "TASK_VALIDATION_FAILED Web live direction and risk closure authorization starting SHA mismatch" >&2; failed=1; }
    [[ "$authorized_next_phase" == "V41_WEB_RUNTIME_TRUTH_PLAN_CLOSURE" && "$authorized_next_mode" == "IMPLEMENTATION" ]] || { echo "TASK_VALIDATION_FAILED authorized Web live direction and risk closure package mismatch" >&2; failed=1; }
    [[ "$authorized_next_starting_full_sha" == "$current_package_starting_full_sha" && "$authorized_next_starting_full_sha" =~ ^[0-9a-fA-F]{40}$ ]] || { echo "TASK_VALIDATION_FAILED Web live direction and risk closure starting SHA identity mismatch" >&2; failed=1; }
    ...
    [[ "$(path_list_fingerprint "$current_package_allowed_paths")" == "91bc1358febd5a1b1303db117bc8ac0152308c1e" ]] || { echo "TASK_VALIDATION_FAILED Web gate allowlist fingerprint mismatch" >&2; failed=1; }
    [[ "$(path_list_fingerprint "$authorized_next_allowed_paths")" == "8d01408be2820eccbf9e457ab3851391f9d401ea" ]] || { echo "TASK_VALIDATION_FAILED Web implementation allowlist fingerprint mismatch" >&2; failed=1; }
    [[ "$current_package_edits" == "true" && "$current_package_implementation" == "false" && "$current_package_pr" == "true" && "$current_package_push" == "true" && "$current_package_merge" == "true" && "$current_package_deployment" == "false" ]] || { echo "TASK_VALIDATION_FAILED Web live direction and risk closure authorization permissions mismatch" >&2; failed=1; }
    [[ "$authorized_next_edits" == "true" && "$authorized_next_implementation" == "true" && "$authorized_next_pr" == "true" && "$authorized_next_push" == "true" && "$authorized_next_merge" == "true" && "$authorized_next_deployment" == "false" && "$authorized_next_staging_deployment" == "true" && "$authorized_next_production_deployment" == "false" && "$authorized_next_canonical_figma" == "false" && "$authorized_next_mobile" == "false" && "$authorized_next_canonical_figma_key" == "NONE" ]] || { echo "TASK_VALIDATION_FAILED bounded Web live direction and risk closure permissions are incomplete" >&2; failed=1; }
    if printf '%s\n%s\n' "$current_package_allowed_paths" "$authorized_next_allowed_paths" | grep -Eq '[*?]|(^|/)(src|docs|scripts)/?$'; then
      echo "TASK_VALIDATION_FAILED Web live direction and risk closure allowlists must not contain wildcards or directory grants" >&2
      failed=1
    fi
    [[ "$(yaml_value "$TASK_FILE" authorized_next_package_branch)" == "codex/v4-1-web-runtime-truth-plan-closure" ]] || { echo "TASK_VALIDATION_FAILED runtime truth branch mismatch" >&2; failed=1; }
    [[ "$(yaml_value "$TASK_FILE" web_runtime_truth_plan_implementation_allowlist_count)" == "35" && "$(yaml_value "$TASK_FILE" web_runtime_truth_plan_gate_allowlist_count)" == "8" ]] || { echo "TASK_VALIDATION_FAILED runtime truth path counts mismatch" >&2; failed=1; }
    [[ "$(yaml_value "$TASK_FILE" web_runtime_truth_plan_implementation_allowlist_fingerprint)" == "8d01408be2820eccbf9e457ab3851391f9d401ea" && "$(yaml_value "$TASK_FILE" web_runtime_truth_plan_gate_allowlist_fingerprint)" == "91bc1358febd5a1b1303db117bc8ac0152308c1e" ]] || { echo "TASK_VALIDATION_FAILED runtime truth registered fingerprints mismatch" >&2; failed=1; }
    local runtime_truth_flag
    for runtime_truth_flag in algorithm_changes_allowed ai_calls_allowed telegram_sends_allowed trading_allowed owner_position_mutation_allowed database_permission_now_allowed database_permanent_privilege_expansion_allowed; do
      [[ "$(yaml_value "$TASK_FILE" "web_runtime_truth_plan_$runtime_truth_flag")" == "false" ]] || { echo "TASK_VALIDATION_FAILED runtime truth forbidden permission: $runtime_truth_flag" >&2; failed=1; }
    done
    [[ "$(yaml_value "$TASK_FILE" web_runtime_truth_plan_database_permission_window)" == "TEMPORARY_STAGING_V23_MIGRATION_WINDOW_ONLY" && "$(yaml_value "$TASK_FILE" web_runtime_truth_plan_database_permission_recovery_required)" == "true" ]] || { echo "TASK_VALIDATION_FAILED runtime truth temporary migration boundary mismatch" >&2; failed=1; }
  elif [[ "$current_package_phase" == "TRINE_LOGIC_V4_1_WEB_LIVE_DIRECTION_RISK_CLOSURE_AUTHORIZATION" ]]; then
    [[ "$current_package_status" == "COMPLETED" ]] || { echo "TASK_VALIDATION_FAILED Web live direction and risk closure authorization declaration mismatch" >&2; failed=1; }
    [[ "$current_package_mode" == "DOCS_GATE_BASELINE_RECONCILIATION" ]] || { echo "TASK_VALIDATION_FAILED current authorization mode mismatch" >&2; failed=1; }
    [[ "$current_package_branch" == "codex/v4-1-web-live-direction-risk-closure-authorization" ]] || { echo "TASK_VALIDATION_FAILED Web live direction and risk closure authorization branch mismatch" >&2; failed=1; }
    [[ "$current_package_starting_full_sha" == "b87cb1878405a6fe8693add1036251d6631e2520" ]] || { echo "TASK_VALIDATION_FAILED Web live direction and risk closure authorization starting SHA mismatch" >&2; failed=1; }
    [[ "$authorized_next_phase" == "V41_WEB_LIVE_DIRECTION_RISK_CLOSURE" && "$authorized_next_mode" == "IMPLEMENTATION" ]] || { echo "TASK_VALIDATION_FAILED authorized Web live direction and risk closure package mismatch" >&2; failed=1; }
    [[ "$authorized_next_starting_full_sha" == "$current_package_starting_full_sha" && "$authorized_next_starting_full_sha" =~ ^[0-9a-fA-F]{40}$ ]] || { echo "TASK_VALIDATION_FAILED Web live direction and risk closure starting SHA identity mismatch" >&2; failed=1; }
    [[ "$authorized_next_mode" != "$current_package_mode" ]] || { echo "TASK_VALIDATION_FAILED current and authorized next modes must be distinct" >&2; failed=1; }
    [[ "$(path_list_fingerprint "$current_package_allowed_paths")" == "7b5cf25f5dc0632643a09b5086322bf860c1fea3" ]] || { echo "TASK_VALIDATION_FAILED Web gate allowlist fingerprint mismatch" >&2; failed=1; }
    [[ "$(path_list_fingerprint "$authorized_next_allowed_paths")" == "762545081db4e62a92e37af104345b21ec9cd935" ]] || { echo "TASK_VALIDATION_FAILED Web implementation allowlist fingerprint mismatch" >&2; failed=1; }
    [[ "$current_package_edits" == "true" && "$current_package_implementation" == "false" && "$current_package_pr" == "true" && "$current_package_push" == "true" && "$current_package_merge" == "true" && "$current_package_deployment" == "false" ]] || { echo "TASK_VALIDATION_FAILED Web live direction and risk closure authorization permissions mismatch" >&2; failed=1; }
    [[ "$authorized_next_edits" == "true" && "$authorized_next_implementation" == "true" && "$authorized_next_pr" == "true" && "$authorized_next_push" == "true" && "$authorized_next_merge" == "true" && "$authorized_next_deployment" == "false" && "$authorized_next_staging_deployment" == "true" && "$authorized_next_production_deployment" == "false" && "$authorized_next_canonical_figma" == "false" && "$authorized_next_mobile" == "false" && "$authorized_next_canonical_figma_key" == "NONE" ]] || { echo "TASK_VALIDATION_FAILED bounded Web live direction and risk closure permissions are incomplete" >&2; failed=1; }
    if printf '%s\n%s\n' "$current_package_allowed_paths" "$authorized_next_allowed_paths" | grep -Eq '[*?]|(^|/)(src|docs|scripts)/?$'; then
      echo "TASK_VALIDATION_FAILED Web live direction and risk closure allowlists must not contain wildcards or directory grants" >&2
  ...
  [[ "$current_package_phase" == "TRINE_LOGIC_V4_1_REAL_LOGIC_CHAIN_CLOSURE_AUTHORIZATION" && "$current_package_status" == "COMPLETED" ]] || { echo "TASK_VALIDATION_FAILED Real logic chain closure authorization declaration mismatch" >&2; failed=1; }
  [[ "$current_package_mode" == "DOCS_GATE_BASELINE_RECONCILIATION" ]] || { echo "TASK_VALIDATION_FAILED current authorization mode mismatch" >&2; failed=1; }
  [[ "$current_package_branch" == "codex/v4-1-real-logic-chain-closure-authorization" ]] || { echo "TASK_VALIDATION_FAILED Real logic chain closure authorization branch mismatch" >&2; failed=1; }
  [[ "$current_package_starting_full_sha" == "b6904ba7c12759ab8a89da40991f9809cdc73e24" ]] || { echo "TASK_VALIDATION_FAILED Real logic chain closure authorization starting SHA mismatch" >&2; failed=1; }
  [[ "$current_package_edits" == "true" && "$current_package_implementation" == "false" && "$current_package_pr" == "true" && "$current_package_push" == "true" && "$current_package_merge" == "true" && "$current_package_deployment" == "false" ]] || { echo "TASK_VALIDATION_FAILED Real logic chain closure authorization permissions mismatch" >&2; failed=1; }
  ...
  [[ "$authorized_next_phase" == "V41_REAL_LOGIC_CHAIN_CLOSURE" && "$authorized_next_mode" == "IMPLEMENTATION" ]] || { echo "TASK_VALIDATION_FAILED authorized Real logic chain closure package mismatch" >&2; failed=1; }
  ...
  [[ "$(printf '%s\n' "$authorized_next_allowed_paths" | sed '/^$/d' | wc -l | tr -d ' ')" == "98" ]] || { echo "TASK_VALIDATION_FAILED Real logic chain closure closure must contain exactly ninety-eight implementation paths" >&2; failed=1; }
...
normalization_source_parent_sha="$(state_value "$state_text" NORMALIZATION_SOURCE_PARENT_SHA)"
normalized_base_full_sha="$(state_value "$state_text" NORMALIZED_BASE_FULL_SHA)"
source_starting_sha_match="$(state_value "$state_text" SOURCE_STARTING_SHA_MATCH)"
normalization_match="$(state_value "$state_text" NORMALIZATION_MATCH)"
...
v4_1_telegram_remediation_authorization_runtime_status="$(state_value "$state_text" V4_1_TELEGRAM_REMEDIATION_AUTHORIZATION_STATUS)"
...
v4_1_baseline_reconciliation_gate_runtime_status="$(state_value "$state_text" V4_1_BASELINE_RECONCILIATION_GATE_STATUS)"
...
real_provider_three_ai_runtime_closure_authorization_runtime_status="$(state_value "$state_text" V4_1_REAL_PROVIDER_THREE_AI_RUNTIME_CLOSURE_AUTHORIZATION_STATUS)"
...
official_domain_session_runtime_visibility_closure_authorization_runtime_status="$(state_value "$state_text" V4_1_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_AUTHORIZATION_STATUS)"
official_domain_session_runtime_visibility_closure_implementation_runtime_status="$(state_value "$state_text" V4_1_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_IMPLEMENTATION_STATUS)"
real_logic_chain_closure_authorization_runtime_status="$(state_value "$state_text" V4_1_REAL_LOGIC_CHAIN_CLOSURE_AUTHORIZATION_STATUS)"
real_logic_chain_closure_implementation_runtime_status="$(state_value "$state_text" V4_1_REAL_LOGIC_CHAIN_CLOSURE_IMPLEMENTATION_STATUS)"
web_live_direction_risk_closure_authorization_runtime_status="$(state_value "$state_text" V4_1_WEB_LIVE_DIRECTION_RISK_CLOSURE_AUTHORIZATION_STATUS)"
...
frontend_interaction_implementation_runtime_status="$(state_value "$state_text" FRONTEND_INTERACTION_IMPLEMENTATION_STATUS)"
    ...
    case "$resolved_mode" in
      ...
      IMPLEMENTATION)
        ...
        if [[ "$resolved_package" == "REAL_DATA_HOME_BLOCKER_CLOSURE" ]]; then
          [[ "$resolved_pr_creation_permission" == "true" ]] || { echo "STOP: one-pass B01-B04 package resolved without PR creation permission." >&2; exit 1; }
        else
        ...
        if [[ "$resolved_package" == "FUNDAMENTAL_AI_V4_1_FINAL_INTERACTION_PAGE_AND_RUNTIME_IMPLEMENTATION" ]]; then
        ...
        elif [[ "$resolved_package" == "FUNDAMENTAL_AI_V4_1_TARGET_RUNTIME_BLOCKER_REMEDIATION" ]]; then
        ...
        elif [[ "$resolved_package" == "FUNDAMENTAL_AI_V4_1_CORE_PRODUCTION_LOOP_AUTOMATION" ]]; then
        ...
        elif [[ "$resolved_package" == "ANALYSIS_RUN_IDEMPOTENCY_TRANSACTION_BOUNDARY_FIX" || "$resolved_package" == "V41_REAL_PROVIDER_AND_THREE_AI_RUNTIME_CLOSURE" || "$resolved_package" == "V41_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE" || "$resolved_package" == "V41_REAL_LOGIC_CHAIN_CLOSURE" || "$resolved_package" == "V41_WEB_RUNTIME_TRUTH_PLAN_CLOSURE" || "$resolved_package" == "V41_WEB_LIVE_DIRECTION_RISK_CLOSURE" ]]; then
          ...
          [[ "$canonical_figma_file_key" == "NONE" ]] || { echo "STOP: local-real package resolved a forbidden Figma file." >&2; exit 1; }
        elif [[ "$resolved_package" == "MULTI_USER_ACCOUNT_REGISTRATION_CLOSURE" ]]; then
          [[ "$canonical_figma_desktop_implementation_allowed" == "false" ]] || { echo "STOP: multi-user package resolved with forbidden Figma permission." >&2; exit 1; }
          [[ "$mobile_implementation_allowed" == "false" ]] || { echo "STOP: multi-user package resolved with forbidden Mobile permission." >&2; exit 1; }
          [[ "$canonical_figma_file_key" == "NONE" ]] || { echo "STOP: multi-user package resolved a forbidden Figma file." >&2; exit 1; }
        fi
        ;;
      *)
...
NORMALIZATION_SOURCE_PARENT_SHA: $normalization_source_parent_sha
NORMALIZED_BASE_FULL_SHA: $normalized_base_full_sha
SOURCE_STARTING_SHA_MATCH: $source_starting_sha_match
...
V4_1_TELEGRAM_REMEDIATION_AUTHORIZATION_STATUS: $v4_1_telegram_remediation_authorization_runtime_status
...
V4_1_BASELINE_RECONCILIATION_GATE_STATUS: $v4_1_baseline_reconciliation_gate_runtime_status
...
V4_1_REAL_PROVIDER_THREE_AI_RUNTIME_CLOSURE_AUTHORIZATION_STATUS: $real_provider_three_ai_runtime_closure_authorization_runtime_status
...
V4_1_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_AUTHORIZATION_STATUS: $official_domain_session_runtime_visibility_closure_authorization_runtime_status
V4_1_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_IMPLEMENTATION_STATUS: $official_domain_session_runtime_visibility_closure_implementation_runtime_status
V4_1_REAL_LOGIC_CHAIN_CLOSURE_AUTHORIZATION_STATUS: $real_logic_chain_closure_authorization_runtime_status
V4_1_REAL_LOGIC_CHAIN_CLOSURE_IMPLEMENTATION_STATUS: $real_logic_chain_closure_implementation_runtime_status
V4_1_WEB_LIVE_DIRECTION_RISK_CLOSURE_AUTHORIZATION_STATUS: $web_live_direction_risk_closure_authorization_runtime_status
...
FRONTEND_INTERACTION_IMPLEMENTATION_STATUS: $frontend_interaction_implementation_runtime_status
