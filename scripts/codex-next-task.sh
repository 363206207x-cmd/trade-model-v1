...
validate_contract_task() {
  ...
  [[ "$current_package_phase" == "TRINE_LOGIC_V4_1_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_AUTHORIZATION" && "$current_package_status" == "COMPLETED" ]] || { echo "TASK_VALIDATION_FAILED official-domain session and runtime-visibility authorization declaration mismatch" >&2; failed=1; }
  ...
  [[ "$current_package_branch" == "codex/v4-1-official-domain-session-runtime-visibility-authorization" ]] || { echo "TASK_VALIDATION_FAILED official-domain session and runtime-visibility authorization branch mismatch" >&2; failed=1; }
  [[ "$current_package_starting_full_sha" == "ddd29f1b08512bf2d782a2db5c610b72932ec95e" ]] || { echo "TASK_VALIDATION_FAILED official-domain session and runtime-visibility authorization starting SHA mismatch" >&2; failed=1; }
  [[ "$current_package_edits" == "true" && "$current_package_implementation" == "false" && "$current_package_pr" == "true" && "$current_package_push" == "true" && "$current_package_merge" == "true" && "$current_package_deployment" == "false" ]] || { echo "TASK_VALIDATION_FAILED official-domain session and runtime-visibility authorization permissions mismatch" >&2; failed=1; }
  ...
  [[ "$authorized_next_phase" == "V41_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE" && "$authorized_next_mode" == "IMPLEMENTATION" ]] || { echo "TASK_VALIDATION_FAILED authorized official-domain session and runtime-visibility package mismatch" >&2; failed=1; }
  ...
  [[ "$authorized_next_edits" == "true" && "$authorized_next_implementation" == "true" && "$authorized_next_pr" == "true" && "$authorized_next_push" == "true" && "$authorized_next_merge" == "true" && "$authorized_next_deployment" == "false" && "$authorized_next_staging_deployment" == "true" && "$authorized_next_production_deployment" == "false" && "$authorized_next_canonical_figma" == "false" && "$authorized_next_mobile" == "false" && "$authorized_next_canonical_figma_key" == "NONE" ]] || { echo "TASK_VALIDATION_FAILED bounded official-domain session and runtime visibility permissions are incomplete" >&2; failed=1; }
  [[ "$(printf '%s\n' "$authorized_next_allowed_paths" | sed '/^$/d' | wc -l | tr -d ' ')" == "24" ]] || { echo "TASK_VALIDATION_FAILED official-domain session and runtime visibility closure must contain exactly twenty-four implementation paths" >&2; failed=1; }
  if printf '%s\n' "$authorized_next_allowed_paths" | grep -Eq '[*?]|(^|/)(src|docs|scripts)/?$'; then
    echo "TASK_VALIDATION_FAILED official-domain session and runtime visibility allowlist must not contain wildcards or directory grants" >&2
    failed=1
  fi
  ...
  AUTHORIZED_NEXT_PACKAGE)
    ...
    case "$resolved_mode" in
      ...
      IMPLEMENTATION)
        ...
        elif [[ "$resolved_package" == "ANALYSIS_RUN_IDEMPOTENCY_TRANSACTION_BOUNDARY_FIX" || "$resolved_package" == "V41_REAL_PROVIDER_AND_THREE_AI_RUNTIME_CLOSURE" || "$resolved_package" == "V41_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE" || "$resolved_package" == "V41_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE" ]]; then
