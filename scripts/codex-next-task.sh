...
validate_contract_task() {
  ...
  local telegram_full_interaction_usability_closure_authorization_status telegram_full_interaction_usability_closure_implementation_status
  ...
  telegram_full_interaction_usability_closure_authorization_status="$(yaml_value "$TASK_FILE" v4_1_telegram_full_interaction_usability_closure_authorization_status)"
  ...
  [[ "$current_package_phase" == "TRINE_LOGIC_V4_1_TELEGRAM_FULL_INTERACTION_USABILITY_CLOSURE_AUTHORIZATION" && "$current_package_status" == "COMPLETED" ]] || { echo "TASK_VALIDATION_FAILED Telegram full-interaction usability authorization declaration mismatch" >&2; failed=1; }
  ...
  [[ "$current_package_branch" == "codex/v4-1-telegram-interaction-usability-authorization" ]] || { echo "TASK_VALIDATION_FAILED Telegram full-interaction usability authorization branch mismatch" >&2; failed=1; }
  [[ "$current_package_starting_full_sha" == "b5e52d501becf675f4299fc7c5d389707de81af0" ]] || { echo "TASK_VALIDATION_FAILED Telegram full-interaction usability authorization starting SHA mismatch" >&2; failed=1; }
  [[ "$current_package_edits" == "true" && "$current_package_implementation" == "false" && "$current_package_pr" == "true" && "$current_package_push" == "true" && "$current_package_merge" == "true" && "$current_package_deployment" == "false" ]] || { echo "TASK_VALIDATION_FAILED Telegram full-interaction usability authorization permissions mismatch" >&2; failed=1; }
  ...
  [[ "$authorized_next_phase" == "V41_TELEGRAM_FULL_INTERACTION_USABILITY_CLOSURE" && "$authorized_next_mode" == "IMPLEMENTATION" ]] || { echo "TASK_VALIDATION_FAILED authorized Telegram full-interaction usability package mismatch" >&2; failed=1; }
  ...
  [[ "$authorized_next_edits" == "true" && "$authorized_next_implementation" == "true" && "$authorized_next_pr" == "true" && "$authorized_next_push" == "true" && "$authorized_next_merge" == "true" && "$authorized_next_deployment" == "false" && "$authorized_next_staging_deployment" == "true" && "$authorized_next_production_deployment" == "false" && "$authorized_next_canonical_figma" == "false" && "$authorized_next_mobile" == "false" && "$authorized_next_canonical_figma_key" == "NONE" ]] || { echo "TASK_VALIDATION_FAILED bounded Telegram full-interaction usability permissions are incomplete" >&2; failed=1; }
  [[ "$(printf '%s\n' "$authorized_next_allowed_paths" | sed '/^$/d' | wc -l | tr -d ' ')" == "60" ]] || { echo "TASK_VALIDATION_FAILED Telegram full-interaction usability closure must contain exactly sixty implementation paths" >&2; failed=1; }
  if printf '%s\n' "$authorized_next_allowed_paths" | grep -Eq '[*?]|(^|/)(src|docs|scripts)/?$'; then
    echo "TASK_VALIDATION_FAILED Telegram full-interaction usability allowlist must not contain wildcards or directory grants" >&2
    failed=1
  fi
...
telegram_full_interaction_usability_closure_authorization_runtime_status="$(state_value "$state_text" V4_1_TELEGRAM_FULL_INTERACTION_USABILITY_CLOSURE_AUTHORIZATION_STATUS)"
telegram_full_interaction_usability_closure_implementation_runtime_status="$(state_value "$state_text" V4_1_TELEGRAM_FULL_INTERACTION_USABILITY_CLOSURE_IMPLEMENTATION_STATUS)"
  ...
  AUTHORIZED_NEXT_PACKAGE)
    ...
    case "$resolved_mode" in
      ...
      IMPLEMENTATION)
        ...
        elif [[ "$resolved_package" == "ANALYSIS_RUN_IDEMPOTENCY_TRANSACTION_BOUNDARY_FIX" || "$resolved_package" == "V41_REAL_PROVIDER_AND_THREE_AI_RUNTIME_CLOSURE" || "$resolved_package" == "V41_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE" || "$resolved_package" == "V41_TELEGRAM_FULL_INTERACTION_USABILITY_CLOSURE" ]]; then
...
V4_1_TELEGRAM_FULL_INTERACTION_USABILITY_CLOSURE_AUTHORIZATION_STATUS: $telegram_full_interaction_usability_closure_authorization_runtime_status
V4_1_TELEGRAM_FULL_INTERACTION_USABILITY_CLOSURE_IMPLEMENTATION_STATUS: $telegram_full_interaction_usability_closure_implementation_runtime_status
