...
validate_contract_task() {
  ...
  [[ "$current_package_phase" == "TRINE_LOGIC_V4_1_REAL_PROVIDER_AND_THREE_AI_RUNTIME_CLOSURE_AUTHORIZATION" && "$current_package_status" == "COMPLETED" ]] || { echo "TASK_VALIDATION_FAILED real Provider/Three-AI runtime closure authorization declaration mismatch" >&2; failed=1; }
  ...
  [[ "$current_package_branch" == "codex/v4-1-real-provider-three-ai-runtime-closure-authorization" ]] || { echo "TASK_VALIDATION_FAILED real Provider/Three-AI runtime closure authorization branch mismatch" >&2; failed=1; }
  [[ "$current_package_starting_full_sha" == "52201ba2d3d39d03aee8a005064e1ccf628f2491" ]] || { echo "TASK_VALIDATION_FAILED real Provider/Three-AI runtime closure authorization starting SHA mismatch" >&2; failed=1; }
  [[ "$current_package_edits" == "true" && "$current_package_implementation" == "false" && "$current_package_pr" == "true" && "$current_package_push" == "true" && "$current_package_merge" == "true" && "$current_package_deployment" == "false" ]] || { echo "TASK_VALIDATION_FAILED real Provider/Three-AI runtime closure authorization permissions mismatch" >&2; failed=1; }
  ...
  [[ "$authorized_next_phase" == "V41_REAL_PROVIDER_AND_THREE_AI_RUNTIME_CLOSURE" && "$authorized_next_mode" == "IMPLEMENTATION" ]] || { echo "TASK_VALIDATION_FAILED authorized real Provider/Three-AI runtime closure package mismatch" >&2; failed=1; }
  ...
  AUTHORIZED_NEXT_PACKAGE)
    ...
    case "$resolved_mode" in
      ...
      IMPLEMENTATION)
        ...
        elif [[ "$resolved_package" == "ANALYSIS_RUN_IDEMPOTENCY_TRANSACTION_BOUNDARY_FIX" || "$resolved_package" == "V41_REAL_PROVIDER_AND_THREE_AI_RUNTIME_CLOSURE" ]]; then
