...
validate_contract_task() {
  ...
  [[ "$current_package_phase" == "LOCAL_REAL_READINESS_SYNC_AND_REAL_ANALYSIS_ENABLEMENT_AUTHORIZATION" && "$current_package_status" == "COMPLETED" ]] || { echo "TASK_VALIDATION_FAILED local-real authorization declaration mismatch" >&2; failed=1; }
  ...
  [[ "$authorized_next_phase" == "LOCAL_REAL_READINESS_SYNC_AND_REAL_ANALYSIS_ENABLEMENT" && "$authorized_next_mode" == "IMPLEMENTATION" ]] || { echo "TASK_VALIDATION_FAILED authorized local-real package mismatch" >&2; failed=1; }
  ...
  [[ "$authorized_next_edits" == "true" && "$authorized_next_implementation" == "true" && "$authorized_next_pr" == "true" && "$authorized_next_canonical_figma" == "false" && "$authorized_next_mobile" == "false" && "$authorized_next_canonical_figma_key" == "NONE" ]] || { echo "TASK_VALIDATION_FAILED bounded local-real permissions are incomplete" >&2; failed=1; }
  ...
  [[ "$v4_1_telegram_authorization_status" == "EFFECTIVE_MERGED_MAIN" && "$v4_1_telegram_implementation_status" == "COMPLETE" && "$local_real_authorization_status" == "AUTHORIZED_PENDING_MERGED_MAIN" && "$local_real_implementation_status" == "NOT_STARTED" && "$p1b_scope" == "LOCAL_REAL_READINESS_AND_CURRENT_HOME_BINDING_ONLY" ]] || { echo "TASK_VALIDATION_FAILED local-real authorization boundary mismatch" >&2; failed=1; }
  [[ -f docs/FUNDAMENTAL_AI_LOCAL_REAL_SOURCE_MAPPING.md && -f docs/FUNDAMENTAL_AI_LOCAL_REAL_OWNERSHIP_MAP.md && -f docs/FUNDAMENTAL_AI_LOCAL_REAL_READINESS_AUTHORIZATION.md && -f docs/FUNDAMENTAL_AI_LOCAL_REAL_AUTHORIZATION_VALIDATION.md && -f docs/product-sources/FUNDAMENTAL_AI_V4_1_DECISION_CHAIN.md ]] || { echo "TASK_VALIDATION_FAILED local-real authorization artifacts are missing" >&2; failed=1; }
