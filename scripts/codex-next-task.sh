...
validate_contract_task() {
  ...
  [[ "$(printf '%s\n' "$authorized_next_allowed_paths" | sed '/^$/d' | wc -l | tr -d ' ')" == "37" ]] || { echo "TASK_VALIDATION_FAILED GPT background/Three-AI closure must contain exactly thirty-seven implementation paths" >&2; failed=1; }
