...
validate_contract_task() {
  ...
  [[ "$current_package_phase" == "FUNDAMENTAL_AI_V4_1_TELEGRAM_HIGH_VALUE_ALERT_CHANNEL_REMEDIATION_AUTHORIZATION" && "$current_package_status" == "COMPLETED" ]] || { echo "TASK_VALIDATION_FAILED Telegram remediation authorization declaration mismatch" >&2; failed=1; }
  ...
  [[ "$authorized_next_phase" == "FUNDAMENTAL_AI_V4_1_TELEGRAM_HIGH_VALUE_ALERT_CHANNEL_INTEGRATION" && "$authorized_next_mode" == "IMPLEMENTATION" ]] || { echo "TASK_VALIDATION_FAILED authorized Telegram remediation package mismatch" >&2; failed=1; }
  ...
  [[ "$authorized_next_edits" == "true" && "$authorized_next_implementation" == "true" && "$authorized_next_pr" == "true" && "$authorized_next_canonical_figma" == "false" && "$authorized_next_mobile" == "false" && "$authorized_next_canonical_figma_key" == "NONE" ]] || { echo "TASK_VALIDATION_FAILED bounded Telegram remediation permissions are incomplete" >&2; failed=1; }
  ...
  [[ "$v4_1_telegram_authorization_status" == "EFFECTIVE_MERGED_MAIN" && "$v4_1_telegram_implementation_status" == "COMPLETE" && "$v4_1_telegram_remediation_authorization_status" == "AUTHORIZED_PENDING_MERGED_MAIN" && "$v4_1_telegram_remediation_implementation_status" == "NOT_STARTED" && "$local_real_authorization_status" == "EFFECTIVE_MERGED_MAIN" && "$local_real_implementation_status" == "COMPLETE" && "$frontend_interaction_authorization_status" == "EFFECTIVE_MERGED_MAIN" && "$frontend_interaction_implementation_status" == "COMPLETE" && "$multi_user_authorization_status" == "EFFECTIVE_MERGED_MAIN" && "$multi_user_implementation_status" == "NOT_STARTED" && "$p1b_scope" == "V4_1_TELEGRAM_TWO_CATEGORY_REMEDIATION_ONLY" ]] || { echo "TASK_VALIDATION_FAILED Telegram remediation authorization boundary mismatch" >&2; failed=1; }
  [[ -f docs/TRINE_LOGIC_TELEGRAM_TWO_CATEGORY_REMEDIATION_SOURCE_MAPPING.md && -f docs/TRINE_LOGIC_TELEGRAM_TWO_CATEGORY_REMEDIATION_OWNERSHIP_MAP.md && -f docs/TRINE_LOGIC_TELEGRAM_TWO_CATEGORY_REMEDIATION_AUTHORIZATION.md && -f docs/TRINE_LOGIC_TELEGRAM_TWO_CATEGORY_REMEDIATION_AUTHORIZATION_VALIDATION.md && -f docs/product-sources/FUNDAMENTAL_AI_V4_1_DECISION_CHAIN.md ]] || { echo "TASK_VALIDATION_FAILED Telegram remediation authorization artifacts are missing" >&2; failed=1; }
