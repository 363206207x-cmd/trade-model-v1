...
web_live_direction_risk_contract_matches() {
  [[ "$current_package_phase" == "TRINE_LOGIC_V4_1_WEB_LIVE_DIRECTION_RISK_CLOSURE_AUTHORIZATION" ]] \
    ...
    && [[ "$current_package_branch" == "codex/v4-1-web-live-direction-risk-closure-authorization" ]] \
    && [[ "$current_package_starting_full_sha" == "b87cb1878405a6fe8693add1036251d6631e2520" ]] \
    && [[ "$authorized_next_package_phase" == "V41_WEB_LIVE_DIRECTION_RISK_CLOSURE" ]] \
    && [[ "$authorized_next_package_mode" == "IMPLEMENTATION" ]] \
    && [[ "$authorized_next_package_branch" == "codex/v4-1-web-live-direction-risk-closure" ]] \
...
web_runtime_truth_plan_contract_matches() {
  [[ "$current_package_phase" == "TRINE_LOGIC_V4_1_WEB_RUNTIME_TRUTH_PLAN_CLOSURE_AUTHORIZATION" ]] \
    ...
    && [[ "$(path_list_fingerprint "$authorized_next_package_allowed_paths")" == "e029dfa6ed68f7c68df95f99fcf211d6e210c55c" ]] \
    && ! printf '%s\n%s\n' "$current_package_allowed_paths" "$authorized_next_package_allowed_paths" | grep -Eq '[*?]|(^|/)(src|docs|scripts)/?$' \
    ...
    && [[ "$web_runtime_truth_plan_implementation_allowlist_count" == "61" ]] \
    && [[ "$web_runtime_truth_plan_gate_allowlist_count" == "10" ]] \
    && [[ "$web_runtime_truth_plan_implementation_allowlist_fingerprint" == "e029dfa6ed68f7c68df95f99fcf211d6e210c55c" ]] \
...
emit_resolved_task_state() {
  ...
  printf 'V4_1_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_AUTHORIZATION_STATUS: %s\n' "${gpt_background_three_ai_timeout_closure_authorization_runtime_status:-BLOCKED}"
  ...
  printf 'V4_1_REAL_LOGIC_CHAIN_CLOSURE_AUTHORIZATION_STATUS: %s\n' "${real_logic_chain_closure_authorization_runtime_status:-BLOCKED}"
...
evaluate_real_logic_chain_closure_transition() {
  effective_task_mode="$current_package_mode"
  p1a_transition_allowed="YES"
  p1a_completion_status="PASS"
  next_transition_allowed="NO"
  ...
  [[ "$current_package_phase" == "TRINE_LOGIC_V4_1_REAL_LOGIC_CHAIN_CLOSURE_AUTHORIZATION" ]] || return 0
  [[ "$current_package_mode" == "DOCS_GATE_BASELINE_RECONCILIATION" ]] || return 0
  [[ "$authorized_next_package_phase" == "V41_REAL_LOGIC_CHAIN_CLOSURE" ]] || return 0
  [[ "$authorized_next_package_mode" == "IMPLEMENTATION" ]] || return 0

  if [[ "$current_package_status" != "COMPLETED" ]]; then
    next_task_authorization_status="BLOCKED_REAL_LOGIC_CHAIN_CLOSURE_AUTHORIZATION_INCOMPLETE"
    return 0
  fi
  if [[ "$real_logic_chain_closure_authorization_declared_status" != "AUTHORIZED_PENDING_MERGED_MAIN" ]]; then
    next_task_authorization_status="BLOCKED_REAL_LOGIC_CHAIN_CLOSURE_SCOPE_NOT_AUTHORIZED"
    return 0
  fi
  if ! is_full_git_sha "$current_package_starting_full_sha" \
    || ! is_full_git_sha "$authorized_next_package_starting_full_sha" \
    ...
    || [[ "$authorized_next_package_starting_full_sha" != "$current_package_starting_full_sha" ]] \
    || [[ "$(printf '%s\n' "$authorized_next_package_allowed_paths" | awk 'NF {count++} END {print count+0}')" != "98" ]]; then
    next_task_authorization_status="BLOCKED_INVALID_OR_MISSING_STARTING_FULL_SHA"
    return 0
  fi
  if ! is_true_flag "$current_package_repository_edits_allowed" \
    ...
    || ! is_true_flag "$current_package_merge_allowed" \
    || ! is_false_flag "$current_package_deployment_allowed"; then
    next_task_authorization_status="BLOCKED_REAL_LOGIC_CHAIN_CLOSURE_GATE_PERMISSIONS_INVALID"
    return 0
  fi
  if ! is_true_flag "$authorized_next_repository_edits_allowed" \
    || ! is_true_flag "$authorized_next_implementation_allowed" \
    ...
    || ! is_false_flag "$authorized_next_mobile_implementation_allowed" \
    || [[ "$authorized_next_canonical_figma_file_key" != "NONE" ]]; then
    next_task_authorization_status="BLOCKED_REAL_LOGIC_CHAIN_CLOSURE_PERMISSIONS_INCOMPLETE"
    return 0
  fi
  if [[ "${machine_gate_effective_on_origin_main:-NO}" != "YES" ]]; then
    next_task_authorization_status="BLOCKED_PENDING_V41_REAL_LOGIC_CHAIN_CLOSURE_AUTHORIZATION_MERGED_MAIN"
    real_logic_chain_closure_authorization_runtime_status="PENDING_MERGED_MAIN"
    return 0
  fi
  if [[ "${machine_identity_allowed:-NO}" != "YES" ]]; then
    next_task_authorization_status="${machine_identity_block_reason:-BLOCKED_EXACT_MACHINE_IDENTITY}"
  ...
  authorization_status="AUTHORIZED"
  next_task_authorization_status="ALLOWED"
  real_logic_chain_closure_authorization_runtime_status="EFFECTIVE_MERGED_MAIN"
}
...
evaluate_web_live_direction_risk_closure_transition() {
  ...
  next_transition_allowed="NO"
  authorization_status="BLOCKED"
  next_task_authorization_status="BLOCKED_WEB_LIVE_DIRECTION_RISK_CLOSURE_CONTRACT"

  web_live_direction_risk_contract_matches || return 0
  if [[ "$product_source_gate_status" != "PASS" ]]; then
    next_task_authorization_status="BLOCKED_PRODUCT_SOURCE_GATE"
    return 0
  fi
  if [[ "${machine_gate_effective_on_origin_main:-NO}" != "YES" ]]; then
    web_live_direction_risk_closure_authorization_runtime_status="PENDING_MERGED_MAIN"
    next_task_authorization_status="BLOCKED_PENDING_V41_WEB_LIVE_DIRECTION_RISK_CLOSURE_AUTHORIZATION_MERGED_MAIN"
    return 0
  fi
  if [[ "${machine_identity_allowed:-NO}" != "YES" ]]; then
    next_task_authorization_status="${machine_identity_block_reason:-BLOCKED_EXACT_MACHINE_IDENTITY}"
  ...
  web_live_direction_risk_closure_authorization_runtime_status="AUTHORIZED"
  next_transition_allowed="YES"
  authorization_status="AUTHORIZED"
  next_task_authorization_status="ALLOWED"
}
...
evaluate_runtime_transition() {
  ...
  if [[ "$current_package_phase" == "TRINE_LOGIC_V4_1_REAL_LOGIC_CHAIN_CLOSURE_AUTHORIZATION" \
    && "$authorized_next_package_phase" == "V41_REAL_LOGIC_CHAIN_CLOSURE" ]]; then
    evaluate_real_logic_chain_closure_transition
    return 0
  fi
...
# Owner-authorized continuation only: #1295 remains the audited source, while
# #1300 must preserve both its merged-main commit and the reviewed repair head.
# Query the exact repository/PRs; branch spelling alone is not authorization.
v42_pr1300_continuation_matches() {
  local number="$1" head_branch="$2" head_sha="$3" source_row continuation_row
  local repository="363206207x-cmd/trade-model-v1"
  local merged_source="8d77902da7a66f26559a92a9646d99952f89e9b8"
  local continuation_base="f2a3903668d8e8129e18bace3d533b6273ebf8a6"
  [[ "$requested_package" == V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE \
    && "$number" == 1300 && "$head_branch" == "$v42_implementation_branch" ]] || return 1
  ...
  continuation_row="$(gh api "repos/$repository/pulls/1300" --method GET \
    --jq '[.number,.state,.base.ref,.base.repo.full_name,.head.repo.full_name,.head.ref,.head.sha]|@tsv' 2>/dev/null)" || return 1
  [[ "$continuation_row" == "$(printf '1300\topen\tmain\t%s\t%s\t%s\t%s' \
    "$repository" "$repository" "$v42_implementation_branch" "$head_sha")" ]] || return 1
  git cat-file -e "$merged_source^{commit}" >/dev/null 2>&1 \
    && git cat-file -e "$continuation_base^{commit}" >/dev/null 2>&1 \
    && git cat-file -e "$head_sha^{commit}" >/dev/null 2>&1 \
    && git merge-base --is-ancestor "$merged_source" origin/main >/dev/null 2>&1 \
    && git merge-base --is-ancestor "$merged_source" "$head_sha" >/dev/null 2>&1 \
    && git merge-base --is-ancestor "$v42_source_head_full_sha" "$head_sha" >/dev/null 2>&1 \
    && git merge-base --is-ancestor "$continuation_base" "$head_sha" >/dev/null 2>&1 \
...
evaluate_machine_runtime_identity() {
  ...
  if [[ "$expected_package" == "$current_package_phase" ]]; then
    if [[ "$expected_package" == "TRINE_LOGIC_V4_1_REAL_LOGIC_CHAIN_CLOSURE_AUTHORIZATION" ]] \
      && is_full_git_sha "$current_package_starting_full_sha" \
...
run_exact_machine_gate_self_test() {
  local failed=0
  local owner_package="TRINE_LOGIC_V4_1_REAL_LOGIC_CHAIN_CLOSURE_AUTHORIZATION"
  local owner_branch="codex/v4-1-real-logic-chain-closure-authorization"
  local owner_sha="b6904ba7c12759ab8a89da40991f9809cdc73e24"
  local target_package="V41_REAL_LOGIC_CHAIN_CLOSURE"
  local target_branch="codex/v4-1-real-logic-chain-closure"
  local target_sha="b6904ba7c12759ab8a89da40991f9809cdc73e24"
  local web_owner_package="TRINE_LOGIC_V4_1_WEB_LIVE_DIRECTION_RISK_CLOSURE_AUTHORIZATION"
  local web_owner_branch="codex/v4-1-web-live-direction-risk-closure-authorization"
  local web_target_package="V41_WEB_LIVE_DIRECTION_RISK_CLOSURE"
  local web_target_branch="codex/v4-1-web-live-direction-risk-closure"
  local web_starting_sha="b87cb1878405a6fe8693add1036251d6631e2520"
  ...
  assert_machine_pass WEB_GATE_01_EXACT_IDENTITY \
    "$web_target_package" "$web_target_package" "$web_target_branch" "$web_target_branch" "$web_starting_sha" "$web_starting_sha" \
    IMPLEMENTATION NO YES true true true true true false src/main/java/org/example/trademodel/v41/V41StructuralDirectionEngine.java "$target_paths" YES YES
  assert_machine_blocked WEB_GATE_02_WRONG_PACKAGE \
    "$web_target_package" WRONG_PACKAGE "$web_target_branch" "$web_target_branch" "$web_starting_sha" "$web_starting_sha" \
    IMPLEMENTATION NO YES true true true true true false "" "$target_paths" YES YES
  assert_machine_blocked WEB_GATE_03_WRONG_BRANCH \
    "$web_target_package" "$web_target_package" "$web_target_branch" codex/wrong-branch "$web_starting_sha" "$web_starting_sha" \
    IMPLEMENTATION NO YES true true true true true false "" "$target_paths" YES YES
  assert_machine_blocked WEB_GATE_04_WRONG_SHA \
    "$web_target_package" "$web_target_package" "$web_target_branch" "$web_target_branch" "$web_starting_sha" b60eff8d83c0e1d04371bd425267f1e8d0e4f95c \
    IMPLEMENTATION NO YES true true true true true false "" "$target_paths" YES YES
  assert_machine_blocked WEB_GATE_05_SHORT_SHA \
    "$web_target_package" "$web_target_package" "$web_target_branch" "$web_target_branch" b87cb187 b87cb187 \
    IMPLEMENTATION NO YES true true true true true false "" "$target_paths" YES YES
  assert_machine_blocked WEB_GATE_06_NOT_MERGED_MAIN \
    "$web_target_package" "$web_target_package" "$web_target_branch" "$web_target_branch" "$web_starting_sha" "$web_starting_sha" \
    IMPLEMENTATION NO NO true true true true true false "" "$target_paths" YES YES
  assert_machine_blocked WEB_GATE_07_OUT_OF_ALLOWLIST \
    "$web_target_package" "$web_target_package" "$web_target_branch" "$web_target_branch" "$web_starting_sha" "$web_starting_sha" \
    IMPLEMENTATION NO YES true true true true true false src/main/java/org/example/trademodel/service/AutomaticTradeService.java "$target_paths" YES YES
  assert_machine_blocked WEB_GATE_08_PRODUCTION_DEPLOYMENT \
    "$web_target_package" "$web_target_package" "$web_target_branch" "$web_target_branch" "$web_starting_sha" "$web_starting_sha" \
    IMPLEMENTATION NO YES true true true true true true "" "$target_paths" YES YES
  assert_machine_blocked WEB_GATE_09_OWNER_DATA_MUTATION_PATH \
    "$web_target_package" "$web_target_package" "$web_target_branch" "$web_target_branch" "$web_starting_sha" "$web_starting_sha" \
    IMPLEMENTATION NO YES true true true true true false src/main/resources/db/migration/V99__mutate_owner_positions.sql "$target_paths" YES YES

  prepare_web_transition_fixture() {
    current_package_phase="$web_owner_package"
    current_package_mode="DOCS_GATE_BASELINE_RECONCILIATION"
    current_package_status="COMPLETED"
    current_package_branch="$web_owner_branch"
    current_package_starting_full_sha="$web_starting_sha"
    current_package_allowed_paths="$web_gate_paths"
    current_package_repository_edits_allowed="true"
    current_package_implementation_allowed="false"
    current_package_implementation_pr_allowed="true"
    current_package_push_allowed="true"
    current_package_merge_allowed="true"
    current_package_deployment_allowed="false"
    authorized_next_package_phase="$web_target_package"
    authorized_next_package_mode="IMPLEMENTATION"
    authorized_next_package_branch="$web_target_branch"
    authorized_next_package_starting_full_sha="$web_starting_sha"
    authorized_next_package_allowed_paths="$target_paths"
    authorized_next_repository_edits_allowed="true"
    authorized_next_implementation_allowed="true"
    authorized_next_implementation_pr_allowed="true"
    authorized_next_push_allowed="true"
    authorized_next_merge_allowed="true"
    authorized_next_deployment_allowed="false"
    authorized_next_staging_deployment_allowed="true"
    authorized_next_production_deployment_allowed="false"
    authorized_next_mobile_implementation_allowed="false"
    authorized_next_canonical_figma_desktop_implementation_allowed="false"
    authorized_next_canonical_figma_file_key="NONE"
    web_live_direction_risk_closure_authorization_declared_status="AUTHORIZED_PENDING_MERGED_MAIN"
    web_live_direction_risk_closure_implementation_status="NOT_STARTED"
  ...
  assert_web_transition() {
    local name="$1" expected_runtime="$2" expected_transition="$3" expected_authorization="$4" expected_reason="$5"
    evaluate_web_live_direction_risk_closure_transition
    if [[ "$web_live_direction_risk_closure_authorization_runtime_status" == "$expected_runtime" \
      && "$next_transition_allowed" == "$expected_transition" \
      && "$authorization_status" == "$expected_authorization" \
      && "$next_task_authorization_status" == "$expected_reason" \
      && "$real_logic_chain_closure_authorization_runtime_status" == "LEGACY_RUNTIME_SENTINEL" \
      && "$real_logic_chain_closure_authorization_declared_status" == "LEGACY_DECLARED_SENTINEL" \
      && "$real_logic_chain_closure_implementation_status" == "LEGACY_IMPLEMENTATION_SENTINEL" ]]; then
      printf '%s: PASS\n' "$name"
    else
      printf '%s: FAIL\n' "$name"
      failed=1
  ...
  prepare_web_transition_fixture
  assert_web_transition WEB_TRANSITION_01_PENDING_MERGED_MAIN PENDING_MERGED_MAIN NO BLOCKED BLOCKED_PENDING_V41_WEB_LIVE_DIRECTION_RISK_CLOSURE_AUTHORIZATION_MERGED_MAIN
  prepare_web_transition_fixture
  machine_gate_effective_on_origin_main="YES"
  assert_web_transition WEB_TRANSITION_02_AUTHORIZED AUTHORIZED YES AUTHORIZED ALLOWED
  prepare_web_transition_fixture
  machine_gate_effective_on_origin_main="YES"
  authorized_next_package_phase="WRONG_PACKAGE"
  assert_web_transition WEB_TRANSITION_03_WRONG_PACKAGE BLOCKED NO BLOCKED BLOCKED_WEB_LIVE_DIRECTION_RISK_CLOSURE_CONTRACT
  prepare_web_transition_fixture
  machine_gate_effective_on_origin_main="YES"
  authorized_next_package_branch="codex/wrong-branch"
  assert_web_transition WEB_TRANSITION_04_WRONG_BRANCH BLOCKED NO BLOCKED BLOCKED_WEB_LIVE_DIRECTION_RISK_CLOSURE_CONTRACT
  prepare_web_transition_fixture
  machine_gate_effective_on_origin_main="YES"
  authorized_next_package_starting_full_sha="b60eff8d83c0e1d04371bd425267f1e8d0e4f95c"
  assert_web_transition WEB_TRANSITION_05_WRONG_SHA BLOCKED NO BLOCKED BLOCKED_WEB_LIVE_DIRECTION_RISK_CLOSURE_CONTRACT
  prepare_web_transition_fixture
  machine_gate_effective_on_origin_main="YES"
  authorized_next_package_starting_full_sha="b87cb187"
  assert_web_transition WEB_TRANSITION_06_SHORT_SHA BLOCKED NO BLOCKED BLOCKED_WEB_LIVE_DIRECTION_RISK_CLOSURE_CONTRACT
  prepare_web_transition_fixture
  machine_gate_effective_on_origin_main="YES"
  authorized_next_package_allowed_paths="$(printf '%s\n%s\n' "$target_paths" src/main/java/org/example/trademodel/service/Unsafe.java)"
  assert_web_transition WEB_TRANSITION_07_ALLOWLIST_ADDED BLOCKED NO BLOCKED BLOCKED_WEB_LIVE_DIRECTION_RISK_CLOSURE_CONTRACT
  prepare_web_transition_fixture
  machine_gate_effective_on_origin_main="YES"
  authorized_next_package_allowed_paths="$(printf '%s\n' "$target_paths" | sed '1d')"
  assert_web_transition WEB_TRANSITION_08_ALLOWLIST_MISSING BLOCKED NO BLOCKED BLOCKED_WEB_LIVE_DIRECTION_RISK_CLOSURE_CONTRACT
  prepare_web_transition_fixture
  machine_gate_effective_on_origin_main="YES"
  authorized_next_production_deployment_allowed="true"
  assert_web_transition WEB_TRANSITION_09_PRODUCTION_FORBIDDEN BLOCKED NO BLOCKED BLOCKED_WEB_LIVE_DIRECTION_RISK_CLOSURE_CONTRACT
  prepare_web_transition_fixture
  machine_gate_effective_on_origin_main="YES"
    ...
    git() {
      ...
      if [[ "$V42_OUTER_CASE" == CONT_* ]]; then
        case "$*" in
          ...
          "cat-file -e $continuation_sha^{commit}"|"cat-file -e $baseline_sha^{commit}"|"cat-file -e $merged_sha^{commit}")
            [[ "$V42_OUTER_CASE" != CONT_MISSING_COMMIT ]]; return $? ;;
          "merge-base --is-ancestor $merged_sha origin/main")
            [[ "$V42_OUTER_CASE" != CONT_SOURCE_NOT_MAIN ]]; return $? ;;
          "merge-base --is-ancestor $merged_sha $continuation_sha")
            [[ "$V42_OUTER_CASE" != CONT_MERGE_NOT_ANCESTOR ]]; return $? ;;
          "merge-base --is-ancestor $source_sha $continuation_sha")
            [[ "$V42_OUTER_CASE" != CONT_AUDIT_NOT_ANCESTOR ]]; return $? ;;
          "merge-base --is-ancestor $baseline_sha $continuation_sha")
            [[ "$V42_OUTER_CASE" != CONT_BASE_NOT_ANCESTOR ]]; return $? ;;
          "merge-base --is-ancestor $continuation_sha HEAD")
            [[ "$V42_OUTER_CASE" != CONT_LOCAL_DIVERGED ]]; return $? ;;
          "diff --name-only --no-renames origin/main...$continuation_sha")
            case "$V42_OUTER_CASE" in
              CONT_PATH_OUTSIDE) path='src/main/java/NotAuthorized.java' ;;
              CONT_GATE_OWNER) path='scripts/v1-state.sh' ;;
              CONT_PATH_WHITESPACE) path='  ' ;;
              CONT_PATH_WILDCARD) path='src/main/java/*' ;;
              CONT_PATH_BLANK) printf '%s\n\n%s\n' "$path" "$path"; return 0 ;;
              CONT_PATH_ERROR) return 128 ;;
            esac
            printf '%s\n' "$path"; return 0 ;;
        esac
      fi
    ...
    gh() {
      ...
      case "$1 $2" in
        ...
        'pr list')
          ...
          if [[ "$V42_OUTER_CASE" == CONT_* ]]; then
            [[ "$V42_OUTER_CASE" != CONT_WRONG_BRANCH ]] || head_branch='codex/not-authorized'
            printf '1300\t%s\t%s\tIsolated authorized continuation fixture\ttrue\n' "$head_branch" "$head_sha"
          elif [[ "$V42_OUTER_CASE" == SAME_BRANCH_OTHER_PR ]]; then
        ...
        'api repos/363206207x-cmd/trade-model-v1/pulls/1295')
          [[ "$V42_OUTER_CASE" == CONT_* && "$V42_OUTER_CASE" != CONT_SOURCE_UNKNOWN ]] || return 1
          [[ "$V42_OUTER_CASE" != CONT_SOURCE_UNMERGED ]] || merged=false
          [[ "$V42_OUTER_CASE" != CONT_WRONG_MERGE ]] || merged_sha="$head_sha"
          [[ "$V42_OUTER_CASE" != CONT_SOURCE_WRONG_REPO ]] || repository='other/repository'
        ...
        'api repos/363206207x-cmd/trade-model-v1/pulls/1300')
          [[ "$V42_OUTER_CASE" == CONT_* && "$V42_OUTER_CASE" != CONT_PR_UNKNOWN ]] || return 1
          [[ "$V42_OUTER_CASE" != CONT_WRONG_REPO ]] || repository='other/repository'
          [[ "$V42_OUTER_CASE" != CONT_FORK ]] || head_repository='other/repository'
          [[ "$V42_OUTER_CASE" != CONT_PR_CLOSED ]] || state=closed
          [[ "$V42_OUTER_CASE" != CONT_WRONG_BASE ]] || base=not-main
          [[ "$V42_OUTER_CASE" != CONT_HEAD_RACE ]] || head_sha='4444444444444444444444444444444444444444'
          printf '1300\t%s\t%s\t%s\t%s\t%s\t%s\n' "$state" "$base" "$repository" "$head_repository" "$head_branch" "$head_sha"
          return 0 ;;
      esac
      return 127
    }
...
CONT_WRONG_REPO|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_FORK|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_HEAD_RACE|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_PR_CLOSED|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_PR_UNKNOWN|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_SOURCE_UNKNOWN|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_SOURCE_UNMERGED|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_SOURCE_WRONG_REPO|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_WRONG_MERGE|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_MISSING_COMMIT|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_SOURCE_NOT_MAIN|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_MERGE_NOT_ANCESTOR|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_AUDIT_NOT_ANCESTOR|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_BASE_NOT_ANCESTOR|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_LOCAL_DIVERGED|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_PATH_OUTSIDE|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_GATE_OWNER|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_PATH_WHITESPACE|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_PATH_WILDCARD|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_PATH_BLANK|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
CONT_PATH_ERROR|V42_ASSET_CARD_DIRECTIONAL_RISK_AND_RUNTIME_CLOSURE|BLOCKED
...
load_task_package_contract() {
  ...
  authorized_next_staging_deployment_allowed="$(yaml_value "$TASK_FILE" authorized_next_package_staging_deployment_allowed)"
  authorized_next_production_deployment_allowed="$(yaml_value "$TASK_FILE" authorized_next_package_production_deployment_allowed)"
  ...
  gpt_background_three_ai_timeout_closure_authorization_declared_status="$(yaml_value "$TASK_FILE" v4_1_gpt_background_three_ai_timeout_closure_authorization_status)"
  ...
  real_logic_chain_closure_authorization_declared_status="$(yaml_value "$TASK_FILE" v4_1_real_logic_chain_closure_authorization_status)"
  ...
  case "$scenario" in
    ...
    predecessor_incomplete)
      if [[ "$current_package_phase" == "TRINE_LOGIC_V4_1_BASELINE_RECONCILIATION_GATE" ]]; then
        ...
        real_logic_chain_closure_authorization_declared_status="BLOCKED_PENDING_REVIEW"
        blockers_text="REAL_LOGIC_CHAIN_CLOSURE_NOT_AUTHORIZED"
...
if [[ "$current_package_phase" == "TRINE_LOGIC_V4_1_REAL_LOGIC_CHAIN_CLOSURE_AUTHORIZATION" ]]; then
  if [[ "$current_package_status" != "COMPLETED" \
    || "$current_package_mode" != "DOCS_GATE_BASELINE_RECONCILIATION" \
    || "$current_package_branch" != "codex/v4-1-real-logic-chain-closure-authorization" \
    || "$current_package_starting_full_sha" != "b6904ba7c12759ab8a89da40991f9809cdc73e24" \
    || "$authorized_next_package_phase" != "V41_REAL_LOGIC_CHAIN_CLOSURE" \
    || "$authorized_next_package_mode" != "IMPLEMENTATION" \
    || "$authorized_next_package_branch" != "codex/v4-1-real-logic-chain-closure" \
    || "$authorized_next_package_starting_full_sha" != "b6904ba7c12759ab8a89da40991f9809cdc73e24" \
    ...
    || "$real_provider_three_ai_runtime_closure_authorization_declared_status" != "EFFECTIVE_MERGED_MAIN" \
    || "$real_provider_three_ai_runtime_closure_implementation_status" != "COMPLETE" \
    || "$gpt_background_three_ai_timeout_closure_authorization_declared_status" != "EFFECTIVE_MERGED_MAIN" \
    ...
    || "$real_logic_chain_closure_authorization_declared_status" != "AUTHORIZED_PENDING_MERGED_MAIN" \
    || "$real_logic_chain_closure_implementation_status" != "NOT_STARTED" \
    ...
    || "$p1b_scope" != "V41_REAL_LOGIC_CHAIN_CLOSURE_ONLY" ]]; then
    blockers+=("TASK_PACKAGE_DECLARATION_CONFLICT")
  elif ! is_true_flag "$current_package_repository_edits_allowed" \
    || ! is_false_flag "$current_package_implementation_allowed" \
    || ! is_true_flag "$current_package_implementation_pr_allowed" \
