...
if [[ "${diagnostic_enabled}" == "true" ]]; then
  ...
  allowed_output="$(awk '/^(AI_PROVIDER|AI_MODEL|AI_AUTH_STATUS|AI_HTTP_STATUS_CLASS|AI_ERROR_CATEGORY|AI_PROVIDER_ERROR_REASON|AI_RESPONSE_PARSE_STATUS|AI_TOKEN_USAGE_PRESENT|AI_REQUEST_ID_PRESENT|AI_TIMEOUT_LIMIT_MS|AI_LATENCY_MS|AI_PROVIDER_LIVE_SMOKE|LIVE_PROVIDER_CALLS|REAL_KEYS_READ|PRODUCTION_READINESS|GEMINI_RESPONSE_SHAPE_DIAGNOSTIC|HTTP_STATUS|TOP_LEVEL_FIELDS|NESTED_OBJECT_PATHS|FIELD_TYPES|EXPECTED_FIELDS|MISSING_FIELDS|UNEXPECTED_FIELDS|TYPE_MISMATCH|GEMINI_SCHEMA_DIAGNOSTIC|ACTUAL_FIELDS|TYPE_MISMATCH_FIELDS): / { print }' "${output_file}")"
fi
