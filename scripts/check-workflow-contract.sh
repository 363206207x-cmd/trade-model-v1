...
[[ "$official_domain_session_runtime_visibility_closure_authorization_status" == "AUTHORIZED_PENDING_MERGED_MAIN" ]] || fail "official-domain session and runtime visibility authorization must remain pending merged-main effectivity"
[[ "$official_domain_session_runtime_visibility_closure_implementation_status" == "NOT_STARTED" ]] || fail "official-domain session and runtime visibility implementation must remain not started in the gate package"
...
[[ "$current_package_phase" == "TRINE_LOGIC_V4_1_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_AUTHORIZATION" && "$current_package_status" == "COMPLETED" ]] || fail "official-domain session and runtime visibility authorization declaration mismatch"
[[ "$current_package_mode" == "DOCS_GATE_BASELINE_RECONCILIATION" ]] || fail "official-domain session and runtime visibility authorization mode mismatch"
[[ "$current_package_branch" == "codex/v4-1-official-domain-session-runtime-visibility-authorization" ]] || fail "official-domain session and runtime visibility authorization branch mismatch"
[[ "$current_package_starting_full_sha" == "ddd29f1b08512bf2d782a2db5c610b72932ec95e" ]] || fail "official-domain session and runtime visibility authorization starting SHA mismatch"
[[ "$current_package_starting_full_sha" =~ ^[0-9a-fA-F]{40}$ ]] || fail "official-domain session and runtime visibility authorization SHA must be full length"
[[ "$current_package_edits" == "true" && "$current_package_implementation" == "false" && "$current_package_pr" == "true" && "$current_package_push" == "true" && "$current_package_merge" == "true" && "$current_package_deployment" == "false" ]] || fail "official-domain session and runtime visibility authorization permissions mismatch"
...
[[ "$authorized_next_package_phase" == "V41_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE" ]] || fail "authorized next package phase mismatch"
[[ "$authorized_next_package_branch" == "codex/v4-1-official-domain-session-runtime-visibility" ]] || fail "authorized next package branch mismatch"
[[ "$authorized_next_package_starting_full_sha" == "ddd29f1b08512bf2d782a2db5c610b72932ec95e" ]] || fail "authorized next package starting SHA mismatch"
...
[[ "$authorized_next_package_pr" == "true" ]] || fail "authorized official-domain session and runtime visibility PR creation must be true for the one-pass task"
[[ "$authorized_next_package_push" == "true" && "$authorized_next_package_merge" == "true" && "$authorized_next_package_deployment" == "false" && "$authorized_next_package_staging_deployment" == "true" && "$authorized_next_package_production_deployment" == "false" ]] || fail "authorized official-domain session and runtime visibility push/merge/staging-only deployment permissions mismatch"
...
[[ "$p1b_scope" == "V41_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_ONLY" ]] || fail "scope must remain V41_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_ONLY"
...
[[ "$(printf '%s\n' "$current_package_allowed_paths" | sort)" == "$(printf '%s\n' "$expected_owner_paths" | sort)" ]] \
  || fail "official-domain session and runtime visibility authorization allowlist must contain exactly the eight gate-owner paths"
if printf '%s\n' "$current_package_allowed_paths" | grep -Eq '[*?]|(^|/)(src|docs|scripts)/?$'; then
  fail "official-domain session and runtime visibility authorization allowlist must not contain wildcards or directory-level grants"
fi
expected_implementation_paths="$(printf '%s\n' \
  ...
  src/test/java/org/example/trademodel/security/PersistentJdbcSessionContractTest.java \
...
printf '%s\n' "$authorization_handoff" | grep -Fq "RESOLVED_HANDOFF_STAGE: V41_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_AUTHORIZATION_REVIEW" \
  || fail "official-domain session and runtime visibility authorization review stage mismatch"
...
printf '%s\n' "$authorization_ready_handoff" | grep -Fq "RESOLVED_HANDOFF_STAGE: V41_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_AUTHORIZATION_FINAL_MERGE_PATH" \
  || fail "ready official-domain session and runtime visibility authorization did not resolve final merge path"

assert_handoff_blocked authorization_pending_request_v4_1 BLOCKED_PENDING_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_AUTHORIZATION_MERGED_MAIN
assert_handoff_blocked authorization_merged_unsynced BLOCKED_PENDING_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_AUTHORIZATION_MERGED_MAIN
...
for v4_1_expected in \
  ...
  "V4_1_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_IMPLEMENTATION_STATUS: NOT_STARTED" \
...
assert_handoff_blocked v4_1_unauthorized BLOCKED_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_SCOPE_NOT_AUTHORIZED
assert_handoff_blocked v4_1_permission_missing BLOCKED_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_PERMISSIONS_INCOMPLETE
...
printf '%s\n' "$final_gate_handoff" | grep -Fq "RESOLVED_HANDOFF_STAGE: V41_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_AUTHORIZATION_FINAL_MERGE_PATH" \
  || fail "official-domain session and runtime visibility authorization final gate stage mismatch"
...
assert_chain_allowed SEPARATE_CONFLICT_CURRENT_AUTHORIZATION separate_conflicting_pr_current \
  CURRENT_PACKAGE_CONTINUATION CURRENT_PACKAGE_CONTINUATION GH_QUERY
assert_chain_blocked V4_1_PERMISSION_MISSING v4_1_permission_missing BLOCKED_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_PERMISSIONS_INCOMPLETE \
  --request-package "$authorized_next_package_phase"
assert_chain_blocked V4_1_BEFORE_AUTHORIZATION authorization_pending_request_v4_1 BLOCKED_PENDING_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_AUTHORIZATION_MERGED_MAIN \
  --request-package "$authorized_next_package_phase"
assert_chain_blocked V4_1_UNAUTHORIZED v4_1_unauthorized BLOCKED_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_SCOPE_NOT_AUTHORIZED \
  --request-package "$authorized_next_package_phase"
...
assert_outer_blocked V4_1_PERMISSION_MISSING v4_1_permission_missing BLOCKED_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_PERMISSIONS_INCOMPLETE \
  --request-package "$authorized_next_package_phase"
assert_outer_blocked V4_1_PENDING_MERGE authorization_pending_request_v4_1 BLOCKED_PENDING_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_AUTHORIZATION_MERGED_MAIN \
  --request-package "$authorized_next_package_phase"
assert_outer_blocked V4_1_UNAUTHORIZED v4_1_unauthorized BLOCKED_OFFICIAL_DOMAIN_SESSION_RUNTIME_VISIBILITY_CLOSURE_SCOPE_NOT_AUTHORIZED \
  --request-package "$authorized_next_package_phase"
