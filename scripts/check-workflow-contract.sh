...
[[ "$local_real_authorization_status" == "AUTHORIZED_PENDING_MERGED_MAIN" ]] || fail "local-real authorization must remain pending merged-main effectivity"
[[ "$local_real_implementation_status" == "NOT_STARTED" ]] || fail "local-real implementation must remain not started"
...
[[ "$current_package_phase" == "LOCAL_REAL_READINESS_SYNC_AND_REAL_ANALYSIS_ENABLEMENT_AUTHORIZATION" && "$current_package_status" == "COMPLETED" ]] || fail "local-real authorization declaration mismatch"
...
[[ "$authorized_next_package_phase" == "LOCAL_REAL_READINESS_SYNC_AND_REAL_ANALYSIS_ENABLEMENT" ]] || fail "authorized next package phase mismatch"
...
[[ "$authorized_next_package_canonical_figma" == "false" ]] || fail "local-real Canonical Figma Desktop permission must remain false"
[[ "$authorized_next_package_mobile" == "false" ]] || fail "authorized v4.1 Mobile permission must remain false"
[[ "$authorized_next_package_canonical_figma_key" == "NONE" ]] || fail "local-real package must not resolve a Figma key"
...
[[ "$p1b_scope" == "LOCAL_REAL_READINESS_AND_CURRENT_HOME_BINDING_ONLY" ]] || fail "scope must remain LOCAL_REAL_READINESS_AND_CURRENT_HOME_BINDING_ONLY"
...
for transition_condition in \
  ...
  V4_1_TELEGRAM_IMPLEMENTATION_EFFECTIVE_MERGED_MAIN \
  LOCAL_REAL_SOURCE_AND_OWNERSHIP_MAPPED \
  LOCAL_REAL_AUTHORIZATION_EFFECTIVE_MERGED_MAIN \
...
printf '%s\n' "$authorization_handoff" | grep -Fq "RESOLVED_HANDOFF_STAGE: LOCAL_REAL_AUTHORIZATION_REVIEW" \
  || fail "local-real authorization review stage mismatch"
...
printf '%s\n' "$authorization_ready_handoff" | grep -Fq "RESOLVED_HANDOFF_STAGE: LOCAL_REAL_AUTHORIZATION_FINAL_MERGE_PATH" \
  || fail "ready authorization did not resolve final merge path"

assert_handoff_blocked predecessor_incomplete BLOCKED_V4_1_TELEGRAM_IMPLEMENTATION_NOT_COMPLETE
assert_handoff_blocked authorization_pending_request_v4_1 BLOCKED_PENDING_LOCAL_REAL_AUTHORIZATION_MERGED_MAIN
...
assert_handoff_blocked v4_1_unauthorized BLOCKED_LOCAL_REAL_SCOPE_NOT_AUTHORIZED
assert_handoff_blocked v4_1_permission_missing BLOCKED_LOCAL_REAL_PERMISSIONS_INCOMPLETE
assert_handoff_blocked authorization_merged_validated BLOCKED_UNKNOWN_RESOLVED_STATE \
  --request-package LOCAL_REAL_READINESS_SYNC_AND_REAL_ANALYSIS_ENABLEMEN
assert_handoff_blocked authorization_merged_validated BLOCKED_UNKNOWN_RESOLVED_STATE \
...
printf '%s\n' "$final_gate_handoff" | grep -Fq "RESOLVED_HANDOFF_STAGE: LOCAL_REAL_AUTHORIZATION_FINAL_MERGE_PATH" \
  || fail "local-real authorization final gate stage mismatch"
assert_chain_blocked V4_1_PREDECESSOR_NOT_COMPLETE predecessor_incomplete BLOCKED_V4_1_TELEGRAM_IMPLEMENTATION_NOT_COMPLETE \
...
assert_chain_allowed SEPARATE_CONFLICT_CURRENT_AUTHORIZATION separate_conflicting_pr_current \
  CURRENT_PACKAGE_CONTINUATION CURRENT_PACKAGE_CONTINUATION GH_QUERY
assert_chain_blocked V4_1_PERMISSION_MISSING v4_1_permission_missing BLOCKED_LOCAL_REAL_PERMISSIONS_INCOMPLETE \
  --request-package "$authorized_next_package_phase"
assert_chain_blocked V4_1_BEFORE_AUTHORIZATION authorization_pending_request_v4_1 BLOCKED_PENDING_LOCAL_REAL_AUTHORIZATION_MERGED_MAIN \
  --request-package "$authorized_next_package_phase"
assert_chain_blocked V4_1_UNAUTHORIZED v4_1_unauthorized BLOCKED_LOCAL_REAL_SCOPE_NOT_AUTHORIZED \
  --request-package "$authorized_next_package_phase"
...
assert_outer_blocked V4_1_PREDECESSOR_NOT_COMPLETE predecessor_incomplete BLOCKED_V4_1_TELEGRAM_IMPLEMENTATION_NOT_COMPLETE \
  --request-package "$authorized_next_package_phase"
assert_outer_allowed V4_1_AUTHORIZATION_EFFECTIVE authorization_merged_validated IMPLEMENTATION \
  --request-package "$authorized_next_package_phase"
assert_outer_blocked V4_1_PERMISSION_MISSING v4_1_permission_missing BLOCKED_LOCAL_REAL_PERMISSIONS_INCOMPLETE \
  --request-package "$authorized_next_package_phase"
assert_outer_blocked V4_1_PENDING_MERGE authorization_pending_request_v4_1 BLOCKED_PENDING_LOCAL_REAL_AUTHORIZATION_MERGED_MAIN \
  --request-package "$authorized_next_package_phase"
assert_outer_blocked V4_1_UNAUTHORIZED v4_1_unauthorized BLOCKED_LOCAL_REAL_SCOPE_NOT_AUTHORIZED \
  --request-package "$authorized_next_package_phase"
...
bash scripts/validate-local-real-authorization.sh >/dev/null \
  || fail "local-real authorization validation failed"
