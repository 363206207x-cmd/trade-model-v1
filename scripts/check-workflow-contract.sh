...
[[ "$analysis_run_idempotency_tx_fix_authorization_status" == "AUTHORIZED_PENDING_MERGED_MAIN" ]] || fail "analysis-run idempotency fix authorization must remain pending merged-main effectivity"
[[ "$analysis_run_idempotency_tx_fix_implementation_status" == "NOT_STARTED" ]] || fail "analysis-run idempotency fix must remain not started in the gate package"
...
[[ "$current_package_phase" == "TRINE_LOGIC_V4_1_ANALYSIS_RUN_IDEMPOTENCY_TX_FIX_AUTHORIZATION" && "$current_package_status" == "COMPLETED" ]] || fail "analysis-run idempotency authorization declaration mismatch"
[[ "$current_package_mode" == "DOCS_GATE_BASELINE_RECONCILIATION" ]] || fail "analysis-run idempotency authorization mode mismatch"
[[ "$current_package_branch" == "codex/v4-1-analysis-run-idempotency-tx-fix-authorization" ]] || fail "analysis-run idempotency authorization branch mismatch"
[[ "$current_package_starting_full_sha" == "0e9bd779b10e9d3140b8ceaea0a5193a28d6264f" ]] || fail "analysis-run idempotency authorization starting SHA mismatch"
[[ "$current_package_starting_full_sha" =~ ^[0-9a-fA-F]{40}$ ]] || fail "analysis-run idempotency authorization SHA must be full length"
[[ "$current_package_edits" == "true" && "$current_package_implementation" == "false" && "$current_package_pr" == "true" && "$current_package_push" == "true" && "$current_package_merge" == "true" && "$current_package_deployment" == "false" ]] || fail "analysis-run idempotency authorization permissions mismatch"
...
[[ "$authorized_next_package_phase" == "ANALYSIS_RUN_IDEMPOTENCY_TRANSACTION_BOUNDARY_FIX" ]] || fail "authorized next package phase mismatch"
[[ "$authorized_next_package_branch" == "codex/v4-1-analysis-run-idempotency-tx-fix" ]] || fail "authorized next package branch mismatch"
...
[[ "$authorized_next_package_pr" == "true" ]] || fail "authorized idempotency fix PR creation must be true for the one-pass task"
[[ "$authorized_next_package_push" == "true" && "$authorized_next_package_merge" == "true" && "$authorized_next_package_deployment" == "false" ]] || fail "authorized idempotency fix push/merge/production-deployment permissions mismatch"
...
[[ "$p1b_scope" == "ANALYSIS_RUN_IDEMPOTENCY_TRANSACTION_BOUNDARY_FIX_ONLY" ]] || fail "scope must remain ANALYSIS_RUN_IDEMPOTENCY_TRANSACTION_BOUNDARY_FIX_ONLY"
...
[[ "$(printf '%s\n' "$current_package_allowed_paths" | sort)" == "$(printf '%s\n' "$expected_owner_paths" | sort)" ]] \
  || fail "analysis-run idempotency authorization allowlist must contain exactly the eight gate-owner paths"
if printf '%s\n' "$current_package_allowed_paths" | grep -Eq '[*?]|(^|/)(src|docs|scripts)/?$'; then
  fail "analysis-run idempotency authorization allowlist must not contain wildcards or directory-level grants"
fi
expected_implementation_paths="$(printf '%s\n' \
  src/main/java/org/example/trademodel/analysisrun/AnalysisIdempotencyGuardImpl.java \
  src/main/java/org/example/trademodel/mapper/AnalysisRunMapper.java \
  src/test/java/org/example/trademodel/analysisrun/AnalysisIdempotencyGuardImplTest.java \
  src/test/java/org/example/trademodel/analysisrun/AnalysisIdempotencyGuardIntegrationTest.java \
  src/test/java/org/example/trademodel/analysisrun/AnalysisIdempotencyGuardPostgreSqlIntegrationTest.java \
...
[[ "$(printf '%s\n' "$authorized_next_package_allowed_paths" | sort)" == "$(printf '%s\n' "$expected_implementation_paths" | sort)" ]] \
  || fail "analysis-run idempotency implementation allowlist must contain exactly the six authorized paths"
if printf '%s\n' "$authorized_next_package_allowed_paths" | grep -Eq '[*?]|(^|/)(src|docs|scripts)/?$'; then
  fail "analysis-run idempotency implementation allowlist must not contain wildcards or directory-level grants"
fi
...
for transition_condition in \
  OWNER_EXPLICIT_SINGLE_DEFECT_END_TO_END_AUTHORIZATION \
  AUTHORIZATION_STARTED_FROM_CLEAN_EXACT_ORIGIN_MAIN \
  EXACT_PACKAGE_MATCH \
  EXACT_BRANCH_MATCH \
  EXACT_40_CHARACTER_STARTING_SHA_MATCH \
  EXACT_SIX_PATH_IMPLEMENTATION_ALLOWLIST \
...
printf '%s\n' "$authorization_handoff" | grep -Fq "RESOLVED_HANDOFF_STAGE: ANALYSIS_RUN_IDEMPOTENCY_TX_FIX_AUTHORIZATION_REVIEW" \
  || fail "analysis-run idempotency authorization review stage mismatch"
...
printf '%s\n' "$authorization_ready_handoff" | grep -Fq "RESOLVED_HANDOFF_STAGE: ANALYSIS_RUN_IDEMPOTENCY_TX_FIX_AUTHORIZATION_FINAL_MERGE_PATH" \
  || fail "ready analysis-run idempotency authorization did not resolve final merge path"

assert_handoff_blocked authorization_pending_request_v4_1 BLOCKED_PENDING_IDEMPOTENCY_FIX_AUTHORIZATION_MERGED_MAIN
assert_handoff_blocked authorization_merged_unsynced BLOCKED_PENDING_IDEMPOTENCY_FIX_AUTHORIZATION_MERGED_MAIN
...
for v4_1_expected in \
  ...
  "ANALYSIS_RUN_IDEMPOTENCY_TX_FIX_IMPLEMENTATION_STATUS: NOT_STARTED" \
...
assert_handoff_blocked v4_1_unauthorized BLOCKED_IDEMPOTENCY_FIX_SCOPE_NOT_AUTHORIZED
assert_handoff_blocked v4_1_permission_missing BLOCKED_IDEMPOTENCY_FIX_PERMISSIONS_INCOMPLETE
assert_handoff_blocked authorization_merged_validated BLOCKED_UNKNOWN_RESOLVED_STATE \
  --request-package ANALYSIS_RUN_IDEMPOTENCY_TRANSACTION_BOUNDARY_FI
assert_handoff_blocked authorization_merged_validated BLOCKED_UNKNOWN_RESOLVED_STATE \
...
printf '%s\n' "$final_gate_handoff" | grep -Fq "RESOLVED_HANDOFF_STAGE: ANALYSIS_RUN_IDEMPOTENCY_TX_FIX_AUTHORIZATION_FINAL_MERGE_PATH" \
  || fail "analysis-run idempotency authorization final gate stage mismatch"
...
assert_chain_allowed SEPARATE_CONFLICT_CURRENT_AUTHORIZATION separate_conflicting_pr_current \
  CURRENT_PACKAGE_CONTINUATION CURRENT_PACKAGE_CONTINUATION GH_QUERY
assert_chain_blocked V4_1_PERMISSION_MISSING v4_1_permission_missing BLOCKED_IDEMPOTENCY_FIX_PERMISSIONS_INCOMPLETE \
  --request-package "$authorized_next_package_phase"
assert_chain_blocked V4_1_BEFORE_AUTHORIZATION authorization_pending_request_v4_1 BLOCKED_PENDING_IDEMPOTENCY_FIX_AUTHORIZATION_MERGED_MAIN \
  --request-package "$authorized_next_package_phase"
assert_chain_blocked V4_1_UNAUTHORIZED v4_1_unauthorized BLOCKED_IDEMPOTENCY_FIX_SCOPE_NOT_AUTHORIZED \
  --request-package "$authorized_next_package_phase"
...
assert_outer_blocked V4_1_PERMISSION_MISSING v4_1_permission_missing BLOCKED_IDEMPOTENCY_FIX_PERMISSIONS_INCOMPLETE \
  --request-package "$authorized_next_package_phase"
assert_outer_blocked V4_1_PENDING_MERGE authorization_pending_request_v4_1 BLOCKED_PENDING_IDEMPOTENCY_FIX_AUTHORIZATION_MERGED_MAIN \
  --request-package "$authorized_next_package_phase"
assert_outer_blocked V4_1_UNAUTHORIZED v4_1_unauthorized BLOCKED_IDEMPOTENCY_FIX_SCOPE_NOT_AUTHORIZED \
  --request-package "$authorized_next_package_phase"
