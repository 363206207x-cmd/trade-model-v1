...
[[ "$gpt_background_three_ai_timeout_closure_authorization_status" == "AUTHORIZED_PENDING_MERGED_MAIN" ]] || fail "GPT background/Three-AI timeout closure authorization must remain pending merged-main effectivity"
[[ "$gpt_background_three_ai_timeout_closure_implementation_status" == "NOT_STARTED" ]] || fail "GPT background/Three-AI timeout closure must remain not started in the gate package"
...
[[ "$current_package_phase" == "TRINE_LOGIC_V4_1_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_AUTHORIZATION" && "$current_package_status" == "COMPLETED" ]] || fail "GPT background/Three-AI timeout closure authorization declaration mismatch"
[[ "$current_package_mode" == "DOCS_GATE_BASELINE_RECONCILIATION" ]] || fail "GPT background/Three-AI timeout closure authorization mode mismatch"
[[ "$current_package_branch" == "codex/v4-1-gpt-background-three-ai-timeout-closure-authorization" ]] || fail "GPT background/Three-AI timeout closure authorization branch mismatch"
[[ "$current_package_starting_full_sha" == "1c13286eb64bb5b074e960352f8e290a317eb704" ]] || fail "GPT background/Three-AI timeout closure authorization starting SHA mismatch"
[[ "$current_package_starting_full_sha" =~ ^[0-9a-fA-F]{40}$ ]] || fail "GPT background/Three-AI timeout closure authorization SHA must be full length"
[[ "$current_package_edits" == "true" && "$current_package_implementation" == "false" && "$current_package_pr" == "true" && "$current_package_push" == "true" && "$current_package_merge" == "true" && "$current_package_deployment" == "false" ]] || fail "GPT background/Three-AI timeout closure authorization permissions mismatch"
...
[[ "$authorized_next_package_phase" == "V41_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE" ]] || fail "authorized next package phase mismatch"
[[ "$authorized_next_package_branch" == "codex/v4-1-gpt-background-three-ai-timeout-closure" ]] || fail "authorized next package branch mismatch"
[[ "$authorized_next_package_starting_full_sha" == "1c13286eb64bb5b074e960352f8e290a317eb704" ]] || fail "authorized next package starting SHA mismatch"
...
[[ "$authorized_next_package_pr" == "true" ]] || fail "authorized GPT background/Three-AI timeout closure PR creation must be true for the one-pass task"
[[ "$authorized_next_package_push" == "true" && "$authorized_next_package_merge" == "true" && "$authorized_next_package_deployment" == "false" ]] || fail "authorized GPT background/Three-AI timeout closure push/merge/production-deployment permissions mismatch"
...
[[ "$p1b_scope" == "V41_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_ONLY" ]] || fail "scope must remain V41_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_ONLY"
...
[[ "$(printf '%s\n' "$current_package_allowed_paths" | sort)" == "$(printf '%s\n' "$expected_owner_paths" | sort)" ]] \
  || fail "GPT background/Three-AI timeout closure authorization allowlist must contain exactly the eight gate-owner paths"
...
expected_implementation_paths="$(printf '%s\n' \
  ...
  src/main/java/org/example/trademodel/analysisrun/AnalysisRunResult.java \
  src/main/java/org/example/trademodel/ai/AiBackgroundTaskState.java \
  src/main/java/org/example/trademodel/ai/AiDecisionChainPromptBuilder.java \
  src/main/java/org/example/trademodel/ai/AiDecisionChainResult.java \
  ...
  src/test/java/org/example/trademodel/analysisrun/AnalysisRunBackgroundWorkerTest.java \
  src/test/java/org/example/trademodel/analysisrun/AnalysisRunOrchestratorImplTest.java \
  src/test/java/org/example/trademodel/ai/AiDecisionChainContractTest.java \
  src/test/java/org/example/trademodel/ai/AiProviderClientAdaptersTest.java \
...
printf '%s\n' "$authorization_handoff" | grep -Fq "RESOLVED_HANDOFF_STAGE: V41_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_AUTHORIZATION_REVIEW" \
  || fail "GPT background/Three-AI timeout closure authorization review stage mismatch"
...
printf '%s\n' "$authorization_ready_handoff" | grep -Fq "RESOLVED_HANDOFF_STAGE: V41_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_AUTHORIZATION_FINAL_MERGE_PATH" \
  || fail "ready GPT background/Three-AI timeout closure authorization did not resolve final merge path"

assert_handoff_blocked authorization_pending_request_v4_1 BLOCKED_PENDING_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_AUTHORIZATION_MERGED_MAIN
assert_handoff_blocked authorization_merged_unsynced BLOCKED_PENDING_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_AUTHORIZATION_MERGED_MAIN
...
for v4_1_expected in \
  ...
  "V4_1_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_IMPLEMENTATION_STATUS: NOT_STARTED" \
...
assert_handoff_blocked v4_1_unauthorized BLOCKED_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_SCOPE_NOT_AUTHORIZED
assert_handoff_blocked v4_1_permission_missing BLOCKED_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_PERMISSIONS_INCOMPLETE
assert_handoff_blocked authorization_merged_validated BLOCKED_UNKNOWN_RESOLVED_STATE \
  --request-package V41_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSUR
assert_handoff_blocked authorization_merged_validated BLOCKED_UNKNOWN_RESOLVED_STATE \
...
printf '%s\n' "$final_gate_handoff" | grep -Fq "RESOLVED_HANDOFF_STAGE: V41_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_AUTHORIZATION_FINAL_MERGE_PATH" \
  || fail "GPT background/Three-AI timeout closure authorization final gate stage mismatch"
...
assert_chain_allowed SEPARATE_CONFLICT_CURRENT_AUTHORIZATION separate_conflicting_pr_current \
  CURRENT_PACKAGE_CONTINUATION CURRENT_PACKAGE_CONTINUATION GH_QUERY
assert_chain_blocked V4_1_PERMISSION_MISSING v4_1_permission_missing BLOCKED_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_PERMISSIONS_INCOMPLETE \
  --request-package "$authorized_next_package_phase"
assert_chain_blocked V4_1_BEFORE_AUTHORIZATION authorization_pending_request_v4_1 BLOCKED_PENDING_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_AUTHORIZATION_MERGED_MAIN \
  --request-package "$authorized_next_package_phase"
assert_chain_blocked V4_1_UNAUTHORIZED v4_1_unauthorized BLOCKED_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_SCOPE_NOT_AUTHORIZED \
  --request-package "$authorized_next_package_phase"
...
assert_outer_blocked V4_1_PERMISSION_MISSING v4_1_permission_missing BLOCKED_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_PERMISSIONS_INCOMPLETE \
  --request-package "$authorized_next_package_phase"
assert_outer_blocked V4_1_PENDING_MERGE authorization_pending_request_v4_1 BLOCKED_PENDING_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_AUTHORIZATION_MERGED_MAIN \
  --request-package "$authorized_next_package_phase"
assert_outer_blocked V4_1_UNAUTHORIZED v4_1_unauthorized BLOCKED_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_SCOPE_NOT_AUTHORIZED \
  --request-package "$authorized_next_package_phase"
