...
analysis_run_idempotency_tx_fix_authorization_status="$(yaml_value docs/CODEX_NEXT_TASK.yml analysis_run_idempotency_tx_fix_authorization_status)"
...
gpt_background_three_ai_timeout_closure_authorization_status="$(yaml_value docs/CODEX_NEXT_TASK.yml v4_1_gpt_background_three_ai_timeout_closure_authorization_status)"
...
official_domain_home_analysis_telegram_closure_authorization_status="$(yaml_value docs/CODEX_NEXT_TASK.yml v4_1_official_domain_home_analysis_telegram_closure_authorization_status)"
official_domain_home_analysis_telegram_closure_implementation_status="$(yaml_value docs/CODEX_NEXT_TASK.yml v4_1_official_domain_home_analysis_telegram_closure_implementation_status)"
...
authorized_next_package_staging_deployment="$(yaml_value docs/CODEX_NEXT_TASK.yml authorized_next_package_staging_deployment_allowed)"
authorized_next_package_production_deployment="$(yaml_value docs/CODEX_NEXT_TASK.yml authorized_next_package_production_deployment_allowed)"
...
authorized_next_package_allowed_paths="$(yaml_list docs/CODEX_NEXT_TASK.yml authorized_next_package_allowed_paths)"
...
[[ "$current_task_mode" == "DOCS_GATE_BASELINE_RECONCILIATION" ]] || fail "current task mode must be the exact docs/gate authorization mode"
...
[[ "$v4_1_baseline_reconciliation_gate_status" == "EFFECTIVE_MERGED_MAIN" ]] || fail "baseline reconciliation must remain effective on merged main"
[[ "$real_data_home_blocker_closure_authorization_status" == "EFFECTIVE_MERGED_MAIN" ]] || fail "B01-B04 authorization must remain effective on merged main"
...
[[ "$real_provider_three_ai_runtime_closure_authorization_status" == "EFFECTIVE_MERGED_MAIN" ]] || fail "real Provider/Three-AI runtime closure authorization must remain effective on merged main"
[[ "$real_provider_three_ai_runtime_closure_implementation_status" == "COMPLETE" ]] || fail "real Provider/Three-AI runtime closure must remain complete"
[[ "$gpt_background_three_ai_timeout_closure_authorization_status" == "EFFECTIVE_MERGED_MAIN" ]] || fail "GPT background/Three-AI timeout closure authorization must remain effective on merged main"
...
[[ "$official_domain_home_analysis_telegram_closure_authorization_status" == "AUTHORIZED_PENDING_MERGED_MAIN" ]] || fail "official-domain Home-analysis and Telegram authorization must remain pending merged-main effectivity"
[[ "$official_domain_home_analysis_telegram_closure_implementation_status" == "NOT_STARTED" ]] || fail "official-domain Home-analysis and Telegram implementation must remain not started in the gate package"
...
[[ "$current_package_phase" == "TRINE_LOGIC_V4_1_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_AUTHORIZATION" && "$current_package_status" == "COMPLETED" ]] || fail "official-domain Home-analysis and Telegram authorization declaration mismatch"
[[ "$current_package_mode" == "DOCS_GATE_BASELINE_RECONCILIATION" ]] || fail "official-domain Home-analysis and Telegram authorization mode mismatch"
[[ "$current_package_branch" == "codex/v4-1-official-domain-home-analysis-telegram-authorization" ]] || fail "official-domain Home-analysis and Telegram authorization branch mismatch"
[[ "$current_package_starting_full_sha" == "aa683013f401cceb289b7bbc245a8ea75e00f3c1" ]] || fail "official-domain Home-analysis and Telegram authorization starting SHA mismatch"
[[ "$current_package_starting_full_sha" =~ ^[0-9a-fA-F]{40}$ ]] || fail "official-domain Home-analysis and Telegram authorization SHA must be full length"
[[ "$current_package_edits" == "true" && "$current_package_implementation" == "false" && "$current_package_pr" == "true" && "$current_package_push" == "true" && "$current_package_merge" == "true" && "$current_package_deployment" == "false" ]] || fail "official-domain Home-analysis and Telegram authorization permissions mismatch"
...
[[ "$authorized_next_package_phase" == "V41_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE" ]] || fail "authorized next package phase mismatch"
[[ "$authorized_next_package_branch" == "codex/v4-1-official-domain-home-analysis-telegram-closure" ]] || fail "authorized next package branch mismatch"
[[ "$authorized_next_package_starting_full_sha" == "aa683013f401cceb289b7bbc245a8ea75e00f3c1" ]] || fail "authorized next package starting SHA mismatch"
...
[[ "$authorized_next_package_pr" == "true" ]] || fail "authorized official-domain Home-analysis and Telegram PR creation must be true for the one-pass task"
[[ "$authorized_next_package_push" == "true" && "$authorized_next_package_merge" == "true" && "$authorized_next_package_deployment" == "false" && "$authorized_next_package_staging_deployment" == "true" && "$authorized_next_package_production_deployment" == "false" ]] || fail "authorized official-domain Home-analysis and Telegram push/merge/staging-only deployment permissions mismatch"
...
[[ "$p1b_scope" == "V41_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_ONLY" ]] || fail "scope must remain V41_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_ONLY"
...
[[ "$(printf '%s\n' "$current_package_allowed_paths" | sort)" == "$(printf '%s\n' "$expected_owner_paths" | sort)" ]] \
  || fail "official-domain Home-analysis and Telegram authorization allowlist must contain exactly the eight gate-owner paths"
if printf '%s\n' "$current_package_allowed_paths" | grep -Eq '[*?]|(^|/)(src|docs|scripts)/?$'; then
  fail "official-domain Home-analysis and Telegram authorization allowlist must not contain wildcards or directory-level grants"
fi
expected_implementation_paths="$(printf '%s\n' \
  ...
  src/main/java/org/example/trademodel/analysistrace/AnalysisTraceSnapshot.java \
  ...
  src/test/java/org/example/trademodel/analysistrace/AnalysisTraceServiceImplTest.java \
...
[[ "$(printf '%s\n' "$authorized_next_package_allowed_paths" | sort)" == "$(printf '%s\n' "$expected_implementation_paths" | sort)" ]] \
  || fail "official-domain Home-analysis and Telegram implementation allowlist must contain exactly the twenty authorized paths"
if printf '%s\n' "$authorized_next_package_allowed_paths" | grep -Eq '[*?]|(^|/)(src|docs|scripts)/?$'; then
  fail "official-domain Home-analysis and Telegram implementation allowlist must not contain wildcards or directory-level grants"
fi
...
for transition_condition in \
  ...
  EXACT_TWENTY_PATH_IMPLEMENTATION_ALLOWLIST \
...
printf '%s\n' "$authorization_handoff" | grep -Fq "RESOLVED_HANDOFF_STAGE: V41_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_AUTHORIZATION_REVIEW" \
  || fail "official-domain Home-analysis and Telegram authorization review stage mismatch"
...
printf '%s\n' "$authorization_ready_handoff" | grep -Fq "RESOLVED_HANDOFF_STAGE: V41_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_AUTHORIZATION_FINAL_MERGE_PATH" \
  || fail "ready official-domain Home-analysis and Telegram authorization did not resolve final merge path"

assert_handoff_blocked authorization_pending_request_v4_1 BLOCKED_PENDING_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_AUTHORIZATION_MERGED_MAIN
assert_handoff_blocked authorization_merged_unsynced BLOCKED_PENDING_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_AUTHORIZATION_MERGED_MAIN
...
for v4_1_expected in \
  ...
  "REAL_DATA_HOME_BLOCKER_CLOSURE_AUTHORIZATION_STATUS: EFFECTIVE_MERGED_MAIN" \
  ...
  "V4_1_REAL_PROVIDER_THREE_AI_RUNTIME_CLOSURE_IMPLEMENTATION_STATUS: COMPLETE" \
  "V4_1_GPT_BACKGROUND_THREE_AI_TIMEOUT_CLOSURE_AUTHORIZATION_STATUS: EFFECTIVE_MERGED_MAIN" \
  ...
  "V4_1_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_AUTHORIZATION_STATUS: EFFECTIVE_MERGED_MAIN" \
  "V4_1_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_IMPLEMENTATION_STATUS: NOT_STARTED" \
...
assert_handoff_blocked v4_1_unauthorized BLOCKED_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_SCOPE_NOT_AUTHORIZED
assert_handoff_blocked v4_1_permission_missing BLOCKED_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_PERMISSIONS_INCOMPLETE
...
printf '%s\n' "$final_gate_handoff" | grep -Fq "RESOLVED_HANDOFF_STAGE: V41_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_AUTHORIZATION_FINAL_MERGE_PATH" \
  || fail "official-domain Home-analysis and Telegram authorization final gate stage mismatch"
...
assert_chain_allowed SEPARATE_CONFLICT_CURRENT_AUTHORIZATION separate_conflicting_pr_current \
  CURRENT_PACKAGE_CONTINUATION CURRENT_PACKAGE_CONTINUATION GH_QUERY
assert_chain_blocked V4_1_PERMISSION_MISSING v4_1_permission_missing BLOCKED_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_PERMISSIONS_INCOMPLETE \
  --request-package "$authorized_next_package_phase"
assert_chain_blocked V4_1_BEFORE_AUTHORIZATION authorization_pending_request_v4_1 BLOCKED_PENDING_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_AUTHORIZATION_MERGED_MAIN \
  --request-package "$authorized_next_package_phase"
assert_chain_blocked V4_1_UNAUTHORIZED v4_1_unauthorized BLOCKED_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_SCOPE_NOT_AUTHORIZED \
  --request-package "$authorized_next_package_phase"
...
assert_outer_blocked V4_1_PERMISSION_MISSING v4_1_permission_missing BLOCKED_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_PERMISSIONS_INCOMPLETE \
  --request-package "$authorized_next_package_phase"
assert_outer_blocked V4_1_PENDING_MERGE authorization_pending_request_v4_1 BLOCKED_PENDING_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_AUTHORIZATION_MERGED_MAIN \
  --request-package "$authorized_next_package_phase"
assert_outer_blocked V4_1_UNAUTHORIZED v4_1_unauthorized BLOCKED_OFFICIAL_DOMAIN_HOME_ANALYSIS_TELEGRAM_CLOSURE_SCOPE_NOT_AUTHORIZED \
  --request-package "$authorized_next_package_phase"
