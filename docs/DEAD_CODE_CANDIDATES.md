...
| `scripts/v1-codex-run-next.sh` legacy independent precheck and `v1-auto.sh next` branch | duplicate workflow authority | The outer launcher only; superseded by `v1-operator.sh -> codex-next-task.sh -> v1-state.sh` | `scripts/check-workflow-contract.sh` will validate the replacement chain | None | No; the P0 contract requires the authoritative operator/resolver chain | LOW | DELETE | The duplicate branch rejects the legal current P0 package after the resolver allows it and independently reselects a static next task. Preserve the launcher file and replace only this obsolete internal branch. |
