...
| SourceTrace Numeric Source Read Model Plan | Active P327 docs-only plan | P327 defines the future SourceTraceNumericSourceContext read model, but it does not create SourceTrace Java DTOs, validators, assemblers, service wiring, dashboard runtime, executable point generation, external channel, order, execution, or auto-trading. |
...
| SourceTrace Numeric Source Read Model Java | Not completed | P327 is only the read model plan and must not be counted as SourceTrace read model Java implementation or runtime source reads. |
