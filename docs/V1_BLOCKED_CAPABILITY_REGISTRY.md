...
| Source-owned Numeric Point Candidate Assembler Plan | Active P323 docs-only plan | P323 defines future source-owned context selection, source ref binding, explicit proposal input creation, mandatory assembler invocation, and mandatory validator gating, but it does not create Java, tests, service wiring, dashboard runtime integration, executable point generation, external channel, order, execution, or auto-trading. |
...
| Numeric Point assembler / service | Explicit-input assembler skeleton completed after P322; source-owned candidate assembler is only planned in P323; service not completed | P322 is assembler-only and P323 is docs-only. Neither must be counted as service wiring, production runtime, or executable point generation. |
