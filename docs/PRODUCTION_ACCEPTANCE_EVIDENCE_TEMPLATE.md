...
Default smoke does not call live providers. Provider live smoke requires real server env and separate human approval for any external calls.

Commands:

```bash
export SMOKE_ALLOW_EXTERNAL_CALLS=true
bash scripts/prod-smoke.sh
```

Required evidence:

- Provider readiness shape is present in `/api/dashboard/home.diagnostics.providerReadiness`.
- Config-only providers are not marked `CONNECTED` unless a verified source exists.
- Binance checks remain public/read-only market or readonly position status; no private trading endpoint is called.
- AI provider readiness is recorded only if keys are configured and explicitly enabled.
