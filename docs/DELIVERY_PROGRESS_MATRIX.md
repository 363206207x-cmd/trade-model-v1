...
|---|---|---|---|---|---|---|---|
| P0-0 | Contract Lock + Baseline + Dead Code Candidate Report | DONE | PARTIAL | PROJECT_DELIVERY_CONTRACT.md exists; PROJECT_CURRENT_STATE.md exists; DELIVERY_PROGRESS_MATRIX.md exists; CODEX_TASK_TEMPLATE.md exists; CONTRACT_CHANGE_LOG.md exists; DEAD_CODE_CANDIDATES.md exists; PROJECT_GLOBAL_AUDIT.md exists; AGENTS.md references the contract; workflow automation migrated; no business code changed; no code deleted. | Effective only after merged main; P0-1 remains blocked while completion state is PENDING_MERGED_MAIN. | Maven PASS; shell syntax PASS; workflow contract PASS; task validation PASS; v1-state/v1-auto contract-first checks PASS; git diff --check PASS; no staged files; HEAD unchanged. | P0-0 Contract Delivery Package and Merge |
| P0-1 | UserPosition | NOT_STARTED | NONE | No `UserPosition`, `tm_user_position`, `CreateUserPositionReq`, or `CloseUserPositionReq` code evidence found. | schema, DO, Mapper, DTO, Service, Controller, Req/VO/Enums, tests. | None. | P0-2 only after DONE |
