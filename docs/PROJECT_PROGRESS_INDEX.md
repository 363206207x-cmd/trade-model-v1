...
- Current HEAD: `701a154 docs(runtime): select recheck preview source read (#998)`.
- Active stop-loss track: `Source Read for Recheck preview / recheck status`.
...
- Next required action: `Source Read for Recheck preview / recheck status`; after merge, `Minimal Review-Only Recheck Preview / Recheck Status Runtime Wiring Design`.
...
- Source Read for Recheck preview / recheck status is completed by this package when merged. It confirms existing PushRecheck controller/service/mapper/schema/test assets and distinguishes safe persisted-log/latest-log/replay-summary/ops-overview/status-contract/config-audit reads from forbidden Recheck execution, Replay execution, scheduler dispatch, MarketQuote/API refresh, PushSnapshot write, recheck-log write, dispatch-config write/init, Push send, external channel, Candidate generation/ranking/scoring, Decision generation, Point, final direction/entry-stop-TP-RR, order/execution/auto-trading, Position Monitor execution, schema/config/pom, new skeleton owners, P359, and P360. It returns GO to design only and requires duplication review against completed Review/Replay result status, Internal Push preview / notification preview status, and Alert fatigue / notification policy status.
