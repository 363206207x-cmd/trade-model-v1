...
- production runtime source read（生产运行时数据源读取）：仍未实现，仍暂停，原因是 P228 只是 fail-closed no-op adapter（失败关闭 no-op 适配器），P231 只是 docs-only audit closure / Java authorization gate（只改文档审计收口 / Java 授权门），不读 DB / 行情 / scheduler。
...
- production adapter implementation（生产适配器实现）：仍未实现，仍暂停，原因是 P228 只是 fail-closed no-op adapter（失败关闭 no-op 适配器），P231 只是 docs-only audit closure / Java authorization gate（只改文档审计收口 / Java 授权门），不是 DB / Market / Scheduler production adapter implementation（数据库 / 行情 / 定时器生产适配器实现）。
...
- runtime source service（运行时数据源服务）：仍未实现，仍暂停，原因是 P231 不接 service / mapper / controller / API（服务 / 映射器 / 控制器 / 接口），不改 mapper 或 service。
...
P231 只是 docs-only audit closure / Java authorization gate（只改文档审计收口 / Java 授权门），不是 DB read implementation，不是 production read implementation（生产读取实现），不是 DB / Market / Scheduler adapter implementation（数据库 / 行情 / 定时器适配器实现）。

P228 已新增一个 `DefaultWatchlistPoolRuntimeSourceReadAdapter` Java 和一个目标测试。

P231 不新增 `NoOpWatchlistPoolRuntimeSourceReadAdapter` Java。

P231 不修改既有 Java。

P231 不修改既有 test（测试）。

P231 不修改 P216 DTO Java。

P231 不修改 P218 guard / validator Java。

P231 不修改 P220 assembler Java。

P231 不改 dashboard.html。

P231 不改 schema（数据库结构）。

P231 不改 config（配置）。

P231 不接 API（接口）。

P231 不改 mapper / service / controller / dashboard / runtime service wiring（映射器 / 服务 / 控制器 / 首页 / 运行时服务接线）。
...
P231 不创建 real low-frequency scan（真实低频扫描）。
...
P231 不接 order API / execution API / auto-trading（下单接口 / 执行接口 / 自动交易）。order API（下单接口）/ execution API（执行接口）/ auto-trading（自动交易）不在 V1 范围内，保持关闭。

P231 不因为 docs-only audit closure / Java authorization gate（只改文档审计收口 / Java 授权门）而大幅上调真实生产接线进度；真实生产接线仍保持 26%-34%，真实点位仍保持 10%-18%。

P231 后仍未完成 / 未实现 / 未升级清单：
...
- `RuleConfigService` / `RuleConfigMapper` / `tm_rule_config` 已存在，但这不是 DB-backed watchlist read implementation（数据库观察库读取实现）。
- `push.watchlist.symbols` runtime config（运行时配置）仍未确认。
