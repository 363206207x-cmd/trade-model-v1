...
- P208 当前推进 Watchlist Scan Runtime DTO Skeleton（观察库扫描运行时 DTO 骨架）。
- P208 只是 pure DTO / enum / tests（纯数据传输对象 / 枚举 / 测试），不是实现。
...
Watchlist Low-Frequency Scan / Opportunity Promote（观察库低频扫描 / 机会提升）语义边界已完成，Low-Frequency Scan Scheduler（低频扫描定时器）已有默认关闭最小骨架。P206 已完成 Low-Frequency Scan Runtime Contract Audit Pack（低频扫描运行时契约审计包），只把 Watchlist runtime data source（观察库运行时数据源）、WatchlistScanResult（观察库扫描结果）和 ScanScore（扫描分数）的未来契约放进文档审计。P207 已完成 Watchlist Runtime Data Source Authorization Gate Pack（观察库运行时数据源授权门包）。P208 当前推进 Watchlist Scan Runtime DTO Skeleton（观察库扫描运行时 DTO 骨架），只是 pure DTO / enum / tests（纯数据传输对象 / 枚举 / 测试），不是 runtime scan（运行时扫描）。当前不应该继续把骨架、契约审计、授权门或 DTO skeleton（数据对象骨架）误推进成真实扫描器、实时数据读取、机会推送执行、真实点位或 Readiness（可执行就绪）；auto-trading（自动交易）不在 V1 范围内，保持关闭。
...
3. 完成 P208 Watchlist Scan Runtime DTO Skeleton（观察库扫描运行时 DTO 骨架）。
4. 后续再拆 WatchlistScanResult DTO / Contract Skeleton Authorization Gate（观察库扫描结果 DTO / 契约骨架授权门）或 ScanScore Guard（扫描分数保护器）实现前门。
...
- real low-frequency scan（真实低频扫描）：仍未完成，仍暂停，原因是 P204 只完成默认关闭骨架，P208 也只是 DTO skeleton（数据对象骨架），不创建真实扫描器。
- Runtime data reads（运行时数据读取）：仍未实现，仍暂停，原因是 P208 DTO skeleton 不读取 runtime / live / external data（运行时 / 实时 / 外部数据）。
- Watchlist runtime data source（观察库运行时数据源）：仍未实现，仍暂停，原因是 P206 只审计数据源契约，P207 只做 authorization gate（授权门），P208 只做 DTO skeleton（数据对象骨架），不实现运行时数据源。
- MarketQuoteClient integration（行情客户端接入）：仍未实现，仍暂停，原因是 P199/P200 明确不接 `MarketQuoteClient`，P208 也明确不接。
- service wiring（服务接线）：仍未实现，仍暂停，原因是 P208 不接 service / mapper / controller（服务 / 映射器 / 控制器）。
- scheduler behavior changes（定时器行为改变）：仍未实现，仍暂停，原因是 P204 只允许 disabled-by-default skeleton（默认关闭骨架），P208 不改 scheduler behavior（定时器行为）。
- Watchlist scan DTO skeleton（观察库扫描 DTO 骨架）：P208 当前推进，但这只是不接线的数据对象骨架，不是运行时扫描结果生产链路。
- real ScanScore implementation（真实扫描分数实现）：仍未实现，仍暂停，原因是 P206 只定义规则审计，P207 只定义 guard（保护器）边界，P208 不计算分数。
- Candidate Attention implementation（候选关注实现）：仍未实现，仍暂停，原因是 P206 只定义契约语义，P208 不创建运行时工作流。
- Promote To Home execution（提升到首页观察执行）：仍未实现，仍暂停，原因是 P206 只允许人工复核语义，P208 不创建执行。
...
- P208 Watchlist Scan Runtime DTO Skeleton（观察库扫描运行时 DTO 骨架）不等于真实低频扫描完成。
...
1. P206 Low-Frequency Scan Runtime Contract Audit Pack（低频扫描运行时契约审计包，已完成）。
2. P207 Watchlist Runtime Data Source Authorization Gate Pack（观察库运行时数据源授权门包，已完成）。
3. 完成 P208 Watchlist Scan Runtime DTO Skeleton 或 Watchlist Scan Result DTO Skeleton（观察库扫描运行时 / 扫描结果数据对象骨架）。
4. WatchlistScanResult DTO / Contract Skeleton 后续授权门（观察库扫描结果 DTO / 契约骨架后续授权门）。
5. ScanScore Guard / Rule Definition 后续授权门（扫描分数保护器 / 规则定义后续授权门）。
6. Low-Cost AI Event Explanation Gate（低成本 AI 事件解释授权门）。
7. Three-AI Promote-To-Home Review Gate（三 AI 提升到首页复核授权门）。
