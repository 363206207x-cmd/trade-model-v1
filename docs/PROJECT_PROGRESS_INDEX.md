...
- P260 当前推进：Push Channel Disabled No-Op Java Skeleton（推送通道禁用 no-op Java 骨架）。
- P260 是 disabled-by-default / no-op skeleton，不是 production wiring，不是 external push execution，不是 Readiness 或 point generation。
- P260 可新增 `OpportunityPushDeliveryDecisionDTO`、`OpportunityPushDeliveryDecisionStatusEnum`、`OpportunityPushDeliveryPolicy`、`NoOpOpportunityPushDeliveryPolicy` 和 targeted test。
...
- Future channels may include Telegram / email / webhook / app notification / local notification, but P260 does not implement them。
- Future delivery must include throttling / idempotency / audit requirements, but P260 only keeps no-op skeleton boundaries and does not implement delivery。
- P260 must keep no network call, no external provider dependency, no message sending, no scheduler activation, no API / dashboard wiring, and no runtime / live / external data read。
...
- Opportunity Push delivery policy Java disabled no-op skeleton 当前推进。
