...
- P257 已完成：Opportunity Push Authorization and Risk Guard Gate Pack（机会推送授权与风险保护门包）。
- P258 当前推进：Opportunity Push Review-Only Skeleton（机会推送只允许复核骨架）。
- P258 是 review-only Opportunity Push skeleton，不是外部推送执行、Readiness 或 point generation。
...
- OpportunityPushDTO 可存在。
- OpportunityPushRule 可存在。
- Opportunity Push Java review-only skeleton 可存在。
