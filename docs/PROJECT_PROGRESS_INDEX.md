...
- P280 当前推进：Real Scan Input Contract DTO Closure and Guard Validator Authorization Scope Pack（真实扫描输入契约 DTO 收口与 Guard Validator 授权范围包）。
...
- Future `RealScanInputContractGuardValidator` may validate only DTO safety / Watchlist Pool proof / fail-closed states / review-only flags。
- Future GuardValidator must not read market data, call MarketQuoteClient, create scan output, compute score, create Candidate, trigger Push, upgrade Readiness, or generate point。
- P281 may implement GuardValidator skeleton + targeted test only if separately authorized。
...
- P275 external channel closure and CI exception record（external channel 收口与 CI 例外记录）已完成；P276 已完成 real scan / real score / Candidate / Push preview planning 回归范围包；P277 已完成 real scan contract audit / market-read boundary plan；P278 已完成 real scan input contract DTO authorization gate；P279 已完成 real scan input contract DTO skeleton；P280 当前推进 guard validator authorization scope pack。
