...
- P270 当前推进：Message Envelope Disabled No-Op Java Skeleton（消息信封禁用 no-op Java 骨架）。
...
- P270 当前只能进入 message envelope disabled no-op Java skeleton，且不得实现 message rendering / sending / provider / external channel。
...
- message envelope disabled no-op skeleton（消息信封禁用 no-op 骨架）当前推进；真实 message envelope behavior、message rendering、message sending、provider 和 external channel 仍未实现。
