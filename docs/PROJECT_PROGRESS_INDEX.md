...
- P288 已完成：MarketReadRequestDTO Java Skeleton（行情读取请求 DTO Java 骨架），且 P288 merged as `d837519`。
- P288 新增 pure-data `MarketReadRequestDTO` DTO skeleton、targeted DTO test、P288 closure 文档，并更新状态索引。
...
- P289 已完成：MarketReadRequestDTO Closure and Guard Validator Authorization Scope Pack（行情读取请求 DTO 收口与保护校验器授权范围包），且 P289 merged as `c7fa73f`。
- P289 是 A docs-only Scope Pack。P289 closed P288 and authorized P290 `MarketReadRequestGuardValidator` Java skeleton only if P290 remains pure validator only。
- P290 已完成：MarketReadRequestGuardValidator Java Skeleton（行情读取请求保护校验器 Java 骨架），且 P290 merged as `42e3c6c`。
- P290 是 B Validator Java/test。P290 added `MarketReadRequestGuardValidator` skeleton、`MarketReadRequestGuardValidationResult`、`MarketReadRequestGuardValidationStatusEnum`、targeted validator test、P290 marker / closure docs, and state index updates。
- P290 GuardValidator only validates `MarketReadRequestDTO` and returns blocked / review-only / fail-closed validation results。It does not connect service/runtime/market/score/push/readiness/point/trading behavior。
- P290 targeted tests cover valid review-only DTO, null request, missing sourceContractId, missing watchlistPoolProof, missing requestedTimeframes, missing scanTimestamp, missing/invalid stalePolicy, missing/invalid missingDataPolicy, reviewOnly false path blocked, notTradeInstruction false path blocked, preserved blockingReasons/riskBlockers, no MarketQuoteClient/BinanceMarketQuoteClient dependency, no runtime/live/external data read, and no scan output / score / Candidate / Push / Readiness / point / trading behavior。
- P291 当前推进：MarketReadRequestGuardValidator Closure and Test-Only Wiring Authorization Scope Pack（行情读取请求保护校验器收口与仅测试接线授权范围包，docs-only）。
- P291 closes P290 and authorizes future P292 only as `MarketReadRequestDTO` -> `MarketReadRequestGuardValidator` test-only wiring。P291 does not write Java, does not modify tests, does not modify DTOs, and does not authorize production wiring。
...
- Runtime market-read Java remains blocked beyond the pure-data P288 DTO skeleton, P290 validator-only skeleton, and P291 docs-only test-only wiring authorization scope; service/controller/mapper/repository/scheduler/API/dashboard/schema/config/MarketQuoteClient/BinanceMarketQuoteClient/runtime reads remain unauthorized。
...
- P275 external channel closure and CI exception record（external channel 收口与 CI 例外记录）已完成；P276 已完成 real scan / real score / Candidate / Push preview planning 回归范围包；P277 已完成 real scan contract audit / market-read boundary plan；P278 已完成 real scan input contract DTO authorization gate；P279 已完成 real scan input contract DTO skeleton；P280 已完成 guard validator authorization scope pack；P281 已完成 RealScanInputContractGuardValidator skeleton；P282 已完成 guard validator closure and test-only wiring authorization scope pack；P283 已完成 RealScanInputContractGuardValidator test-only wiring skeleton；P284 已完成 market-read boundary docs-only scope pack，且 P284 merged as `cbcc34d`；P285 已完成 market-read boundary audit and real scan input assembly authorization gate，且 P285 merged as `c64c8b8`；P286 已完成 market-read request contract authorization gate，且 P286 merged as `c09a15c`；P287 已完成 market-read request DTO authorization gate，且 P287 merged as `4973503`；P288 已完成 MarketReadRequestDTO Java Skeleton，且 P288 merged as `d837519`；P289 已完成 MarketReadRequestDTO Closure and Guard Validator Authorization Scope Pack，且 P289 merged as `c7fa73f`；P290 已完成 MarketReadRequestGuardValidator Java Skeleton，且 P290 merged as `42e3c6c`；P291 当前推进 MarketReadRequestGuardValidator Closure and Test-Only Wiring Authorization Scope Pack。
