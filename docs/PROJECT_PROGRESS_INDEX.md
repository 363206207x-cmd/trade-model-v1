...
- Current HEAD: `400e604 docs(risk): design account exposure runtime wiring`.
- Active stop-loss track: `Minimal Review-Only Account Risk / Account Exposure Status Runtime Wiring Implementation`.
...
- Next required action: `Minimal Review-Only Account Risk / Account Exposure Status Runtime Wiring Implementation`.
...
- Implementation readiness gate for Account risk / account exposure status is completed by this package when merged. It returns GO to B-risk minimal implementation over existing account-risk snapshot read assets, allows at most one minimal read-only `Map` endpoint if needed, minimal dashboard account-risk / exposure status panel/copy/DOM, targeted tests, implementation docs, and source-of-truth updates, and blocks account-risk write, PushSnapshot write, Push send, Recheck execution, trading authorization, position sizing, action guidance, Candidate, Decision generation, Point, final direction, entry/stop/TP/RR, order/execution, auto-trading, new skeleton owners, schema/config/pom, P359, and P360.
...
- The next required action is `Minimal Review-Only Account Risk / Account Exposure Status Runtime Wiring Implementation`.
