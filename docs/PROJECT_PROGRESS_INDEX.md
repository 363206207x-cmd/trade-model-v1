...
- P289 当前推进：MarketReadRequestDTO Closure and Guard Validator Authorization Scope Pack（行情读取请求 DTO 收口与保护校验器授权范围包）。
- P289 是 A docs-only Scope Pack。P289 closes P288 and authorizes future P290 `MarketReadRequestGuardValidator` Java skeleton only if P290 remains pure validator only。
- Future P290 GuardValidator may only validate `MarketReadRequestDTO` and return blocked / review-only / fail-closed validation results。It must not connect service/runtime/market/score/push/readiness/point/trading behavior。
- Future P290 targeted tests must cover missing sourceContractId, missing watchlistPoolProof, missing requestedTimeframes, missing scanTimestamp, missing/invalid stalePolicy, missing/invalid missingDataPolicy, reviewOnly false impossible or blocked, notTradeInstruction false impossible or blocked, preserved blockingReasons/riskBlockers, no MarketQuoteClient/BinanceMarketQuoteClient dependency, no runtime/live/external data read, and no scan output / score / Candidate / Push / Readiness / point / trading behavior。
...
- Runtime market-read Java remains blocked beyond the pure-data P288 DTO skeleton and any future validator-only P290 skeleton; service/controller/mapper/repository/scheduler/API/dashboard/schema/config/MarketQuoteClient/BinanceMarketQuoteClient/runtime reads remain unauthorized。
...
- P275 external channel closure and CI exception record（external channel 收口与 CI 例外记录）已完成；P276 已完成 real scan / real score / Candidate / Push preview planning 回归范围包；P277 已完成 real scan contract audit / market-read boundary plan；P278 已完成 real scan input contract DTO authorization gate；P279 已完成 real scan input contract DTO skeleton；P280 已完成 guard validator authorization scope pack；P281 已完成 RealScanInputContractGuardValidator skeleton；P282 已完成 guard validator closure and test-only wiring authorization scope pack；P283 已完成 RealScanInputContractGuardValidator test-only wiring skeleton；P284 已完成 market-read boundary docs-only scope pack，且 P284 merged as `cbcc34d`；P285 已完成 market-read boundary audit and real scan input assembly authorization gate，且 P285 merged as `c64c8b8`；P286 已完成 market-read request contract authorization gate，且 P286 merged as `c09a15c`；P287 已完成 market-read request DTO authorization gate，且 P287 merged as `4973503`；P288 已完成 MarketReadRequestDTO Java Skeleton，且 P288 merged as `d837519`；P289 当前推进 MarketReadRequestDTO Closure and Guard Validator Authorization Scope Pack。
