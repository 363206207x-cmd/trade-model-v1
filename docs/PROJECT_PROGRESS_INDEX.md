...
- P253 已完成：ScanScore DTO and Rule Skeleton（扫描分数 DTO 与规则骨架）。
- P254 当前推进：ScanScore Calculation Review-Only Skeleton（扫描分数计算只允许复核骨架）。
- P254 是 review-only calculation skeleton（只允许复核的计算骨架），不是真实评分。
