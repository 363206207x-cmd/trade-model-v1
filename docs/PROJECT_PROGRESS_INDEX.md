...
- P278 当前推进：Real Scan Input Contract DTO Authorization Gate（真实扫描输入契约 DTO 授权门）。
- P278 只是 docs-only authorization gate，不是 DTO implementation。
- P279 may add DTO / enum / targeted test only if separately authorized；P279 必须保持 DTO-only 和 targeted-test-only，除非后续 Issue 另行授权。
...
- Future recommended next package after P278 should be P279 real scan input contract DTO skeleton, but only DTO-only and targeted-test-only。
...
- P278 不授权 DTO implementation、real scan implementation、market-read runtime wiring、production ScanScore computation、Candidate production workflow 或 Push execution。
...
- P275 external channel closure and CI exception record（external channel 收口与 CI 例外记录）已完成；P276 已完成 real scan / real score / Candidate / Push preview planning 回归范围包；P277 已完成 real scan contract audit / market-read boundary plan；P278 当前推进 real scan input contract DTO authorization gate。
