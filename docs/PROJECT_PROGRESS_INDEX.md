...
P171 是 Project Progress Index Refresh After SourceTrace Display（SourceTrace 展示后项目总进度索引刷新）。

本索引来自 P164 全局扫描后的持续维护。本轮只刷新 `docs/PROJECT_PROGRESS_INDEX.md`，并吸收 P165-P170 SourceTrace（证据来源追踪）Read Model（只读输出模型）/ Dashboard（首页工作台）Read-only Display（只读展示）闭环结果。P171 不新增 Java，不新增测试，不改 `dashboard.html`，不接 API（接口），不生成真实 entry / stop / TP / RR（入场 / 止损 / 止盈 / 盈亏比），不接 auto-trading（自动交易）。

## 一、当前总进度结论

当前项目仍然不是“快完成自动交易”的状态。更准确的状态是：安全地基、只读复核、失败关闭、Dashboard（首页工作台）展示骨架和 SourceTrace（证据来源追踪）只读展示已经进一步完整；但真实生产候选、`VALID`（有效候选状态）、真实 entry / stop / TP / RR（入场 / 止损 / 止盈 / 盈亏比）、ExecutionPlan（执行计划）Readiness（可执行就绪）和自动交易仍然没有闭环。

P165-P170 已完成 SourceTrace read-only display（证据来源追踪只读展示）闭环，所以项目总进度可以小幅上调。但这个上调只代表“可看见、可复核、可解释”的只读层完成，不代表交易计划生成完成。
...
| dashboard（首页工作台）展示进度 | 66%-74% |
| 自动交易进度 | 0%-3% |

这些百分比按“能否安全进入真实生产链路”估算，不按文档数量估算。SourceTrace（证据来源追踪）已经完成只读输出和 Dashboard（首页工作台）只读展示，但真实交易计划链路仍缺 source-owned runtime candidate generation（运行时证据来源候选生成）、numeric source ownership（数值来源归属）、生产 `VALID`（有效候选状态）和 Readiness（可执行就绪）闭环。
...
SourceTrace（证据来源追踪）只读输出和 Dashboard（首页工作台）只读展示已完成。当前不应该继续在 SourceTrace 页面展示上加功能，也不能直接进入真实点位。

下一阶段建议进入以下两类审计之一：

- BoundaryCandidate source wiring audit（边界候选来源接线审计）：确认边界候选的真实来源、数值来源、缺证据阻断和生产 `VALID`（有效候选状态）边界。
- ExecutionPlan review-only plan output audit（执行计划只允许复核计划输出审计）：确认如何在不升级 Readiness（可执行就绪）的前提下展示可复核计划摘要。

如果继续按最短个人可用路径推进，建议先做 BoundaryCandidate（边界候选交易计划）只读候选展示或 ExecutionPlan（执行计划）只读计划展示。它们都必须保持 `REVIEW_ONLY`（只允许复核），不能生成真实点位，不能打开可执行状态。

## 四、暂停线路

- BoundaryCandidate（边界候选交易计划）真实候选生成：仍暂停，原因是 SourceTrace（证据来源追踪）虽然已完成只读展示，但真实来源接线和生产候选生成没有闭环。
...
1. BoundaryCandidate Read-Only Candidate Display Scope Audit（边界候选只读候选展示范围审计）。
2. BoundaryCandidate Read-Only Candidate Minimal Wiring（边界候选只读候选最小接线）。
3. ExecutionPlan Review-Only Plan Display Audit（执行计划只允许复核展示审计）。
...
|---|---|---:|---|---|
| Project Overall（项目总进度） | 安全地基、只读展示和 SourceTrace（证据来源追踪）闭环小幅推进；真实候选、真实点位、Readiness（可执行就绪）、自动交易未完成 | 66%-71% | `PROJECT_PROGRESS_INDEX.md`、`PHASE_BACKEND_P170_SOURCETRACE_READ_ONLY_DISPLAY_CLOSURE.md` | P171 后继续按本索引选择路线 A 或路线 B |
| SourceTrace（证据来源追踪） | 只读输出 + Dashboard（首页工作台）只读展示已完成；真实候选 / 真实点位 / Readiness（可执行就绪）未完成 | 58%-66% | `SourceTraceRuntimePopulationHelper.java`、`SourceTraceRuntimePopulationServiceImpl.java`、`DefaultDashboardSourceTraceDetailAdapter.java`、`dashboard.html`、`PHASE_BACKEND_P170*` | 转入 BoundaryCandidate 来源审计或只读候选展示审计 |
| BoundaryCandidate（边界候选交易计划） | DTO、valid factory（有效候选工厂）、service skeleton（服务骨架）和测试存在；生产 `VALID`（有效候选状态）仍未授权 | 35%-45% | `BoundaryCandidateDTO.java`、`BoundaryCandidateServiceImpl.java`、`BoundaryCandidateServiceImplTest.java` | 做 read-only candidate display scope audit（只读候选展示范围审计）或 source wiring audit（来源接线审计） |
...
| Dashboard（首页工作台） | 页面、summary/detail API、display objects（展示对象）已存在；SourceTrace（证据来源追踪）只读展示已完成；可执行状态未打开 | 66%-74% | `DashboardController.java`、`dashboard.html`、`DashboardControllerTest.java`、`PHASE_BACKEND_P168*`、`PHASE_BACKEND_P170*` | 只允许继续做只读计划展示范围门，不打开可执行状态 |
...
| entry / stop / TP / RR（入场 / 止损 / 止盈 / 盈亏比） | DTO 和 fixture（测试夹具）存在；真实数值来源未闭环，不因 SourceTrace 展示完成而上调 | 10%-18% | `BoundaryEntryDTO.java`、`BoundaryStopDTO.java`、`BoundaryTakeProfitLevelDTO.java`、`StopTpRrSourceOwnedCandidateFixtureHelper.java` | 先做 numeric source ownership（数值来源归属）审计 |
| Auto-trading（自动交易） | 未实现；只有明确禁止和无交易指令保护 | 0%-3% | `StaticNoTradeInstructionGuardTest.java`、多份 P140-P170 禁止清单 | 继续禁止，直到真实点位、Readiness（可执行就绪）、风控和授权全部闭环 |

## 十、P171 结论

P171 只刷新 `docs/PROJECT_PROGRESS_INDEX.md`。

P171 不写代码。

P171 不新增测试。

P171 不改 `dashboard.html`。
...
P171 的核心结论是：P165-P170 已经完成 SourceTrace（证据来源追踪）只读输出和首页只读展示闭环，项目总进度和 SourceTrace（证据来源追踪）/ Dashboard（首页工作台）展示进度可以小幅上调；但真实生产候选、`VALID`（有效候选状态）、真实 entry / stop / TP / RR（入场 / 止损 / 止盈 / 盈亏比）、Readiness（可执行就绪）和自动交易仍未完成。
