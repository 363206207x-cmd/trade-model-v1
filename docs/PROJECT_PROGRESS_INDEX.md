...
- `BACKEND-P331 SourceTraceNumericSourceReadModelAssembler Java Skeleton` is active on branch `p331`; Issue / PR are not created by Codex in this fallback flow.
- P331 is a Java/test assembler skeleton package from `SOURCETRACE_NUMERIC_SOURCE_VALIDATOR_VERIFICATION` toward `SOURCETRACE_NUMERIC_SOURCE_READ_MODEL_ASSEMBLER_JAVA_SKELETON`.
- The active mainline is Readiness / Point Mainline.
- The active block is SourceTraceNumericSourceReadModelAssembler Java Skeleton.
- The next required action is `commit_and_push_branch_p331`.
