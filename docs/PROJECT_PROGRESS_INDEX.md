...
- P217 当前推进 Watchlist Runtime Source DTO Closure and Guard Boundary（观察库运行时数据源 DTO 收口与保护边界）。
...
12. 完成 P217 Watchlist Runtime Source DTO Closure and Guard Boundary（观察库运行时数据源 DTO 收口与保护边界）。
13. 后续如果进入 P218，也只能先做 pure runtime source guard / validator / tests（纯运行时数据源保护器 / 校验器 / 测试），不能直接接真实扫描、DB、行情或 scheduler（定时器）。
...
11. P216 Watchlist Runtime Source DTO Skeleton（观察库运行时数据源 DTO 骨架，已完成）。
12. 完成 P217 Watchlist Runtime Source DTO Closure and Guard Boundary（观察库运行时数据源 DTO 收口与保护边界）。
13. Runtime source implementation（运行时数据源实现）后续必须另开授权门，不能因为 DTO skeleton（数据对象骨架）或 guard boundary（保护边界）直接接 DB、行情、scheduler 或 scan loop（扫描循环）。
14. ScanScore Guard / Rule Definition 后续授权门（扫描分数保护器 / 规则定义后续授权门）。
15. Low-Cost AI Event Explanation Gate（低成本 AI 事件解释授权门）。
16. Three-AI Promote-To-Home Review Gate（三 AI 提升到首页复核授权门）。
...
## 十、P206 / P207 / P208 / P209 / P210 / P211 / P212 / P213 / P214 / P215 / P216 / P217 结论
...
P217 后仍未完成 / 未实现 / 未升级清单：

- runtime source contract（运行时数据源契约）已文档化。
- `WatchlistRuntimeSourceDTO` Java 已存在，但 runtime source implementation（运行时数据源实现）仍未实现。
- runtime source guard / validator（运行时数据源保护器 / 校验器）仍未实现。
- DB-backed watchlist read（数据库观察库读取）仍未实现。
