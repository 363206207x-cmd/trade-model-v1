...
- `BACKEND-P298 Review-Only Score Assembly Slice` is in PR #731 / branch `p298`, pending merge.
- P298 is capability movement from `REVIEW_ONLY_SCORE_INPUT_PRECHECK_SKELETON` toward `REVIEW_ONLY_SCORE_ASSEMBLY_SKELETON`.
- The active mainline is Evidence / Score Mainline.
- The active block is Review-Only Score Assembly Slice.
- The next required action is `review_pr_731`.
...
| Skeleton / Test Progress | 78%-86% | DTO, validator, no-op, audit, queue, channel, score, candidate, market-read request skeletons/tests, MarketReadRequest test-only wiring, review-only scan output skeleton, P295 evidence / score entry skeleton, P296 evidence normalization skeleton, P297 score input / precheck skeleton, and active P298 score assembly skeleton exist. | Score assembly is not merged yet, and real score calculation is not complete. |
...
2. Score-to-Candidate handoff review-only slice.
