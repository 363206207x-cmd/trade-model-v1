...
- P255 已完成：Candidate Attention and Promote To Home Gate Pack（候选关注与提升到首页授权门包）。
- P256 当前推进：Candidate Attention Review-Only Skeleton（候选关注只允许复核骨架）。
- P256 是 review-only Candidate Attention skeleton，不是 Push / Promote / Readiness。
...
- CandidateAttentionDTO 可存在。
- CandidateAttentionRule 可存在。
