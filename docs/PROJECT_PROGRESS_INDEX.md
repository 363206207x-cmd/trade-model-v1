...
- P223 当前推进 Production Runtime Source Adapter Interface Skeleton Plan（生产运行时数据源适配器接口骨架方案）。
- P223 只是 docs-only interface skeleton plan（只改文档接口骨架方案），不是实现。
- P223 不写 Java。
- P223 不新增测试。
- P223 不修改 DTO / guard / validator / assembler（数据对象 / 保护器 / 校验器 / 组装器）。
- P223 不读取 DB / runtime / live / external data（数据库 / 运行时 / 实时 / 外部数据）。
- P223 不接 MarketQuoteClient integration（行情客户端接入）。
- P223 不接 scheduler（定时器）。
- P223 不接 mapper / service / controller / API / dashboard（映射器 / 服务 / 控制器 / 接口 / 首页）。
- P223 不创建 scan loop（扫描循环）。
- P223 不实现 production adapter interface skeleton（生产适配器接口骨架）。
- P223 不实现 production read adapter（生产读取适配器）。
- P223 不实现 real ScanScore computation（真实扫描分数计算）。
- P223 不实现 Candidate Attention workflow（候选关注流程）。
- P223 不实现 Promote To Home workflow（提升到首页观察流程）。
- P223 不创建 Opportunity Push execution（机会推送执行）。
- P223 不生成真实 entry / stop / TP / RR（入场 / 止损 / 止盈 / 盈亏比）。
- P223 不升级 readiness（可执行就绪）。
- P223 不接 order API / execution API / auto-trading（下单接口 / 执行接口 / 自动交易）；auto-trading（自动交易）不在 V1 范围内，保持关闭。
...
- production runtime source read（生产运行时数据源读取）：仍未实现，仍暂停，原因是 P223 只是 interface skeleton plan（接口骨架方案），不读 DB / 行情 / scheduler。
- production read adapter（生产读取适配器）：仍未实现，仍暂停，原因是 P223 只定义 interface skeleton plan（接口骨架方案），不写 Java。
- production adapter interface skeleton（生产适配器接口骨架）：仍未实现，仍暂停，原因是 P223 只是 docs-only authorization plan（只改文档授权方案），不写 Java。
- production runtime source wiring（生产运行时数据源接线）：仍未实现，仍暂停，原因是 P223 只允许 docs-only interface skeleton plan（只改文档接口骨架方案），不接生产服务链路。
- DB-backed watchlist read（数据库观察库读取）：仍未实现，仍暂停，原因是 P223 不读 DB（数据库）。
- MarketQuoteClient integration（行情客户端接入）：仍未实现，仍暂停，原因是 P199/P200 明确不接 `MarketQuoteClient`，P215 也明确不接。
- runtime source service（运行时数据源服务）：仍未实现，仍暂停，原因是 P223 不接 service / mapper / controller / API（服务 / 映射器 / 控制器 / 接口）。
...
## 十、P206 / P207 / P208 / P209 / P210 / P211 / P212 / P213 / P214 / P215 / P216 / P217 / P218 / P219 / P220 / P221 / P222 / P223 结论
...
P223 不改 dashboard.html。

P223 不改 schema（数据库结构）。

P223 不改 config（配置）。

P223 不接 API（接口）。

P223 不接 mapper / service / controller / dashboard / runtime service wiring（映射器 / 服务 / 控制器 / 首页 / 运行时服务接线）。

P223 不读取 DB / runtime / live / external data（数据库 / 运行时 / 实时 / 外部数据）。

P223 不接 MarketQuoteClient / BinanceMarketQuoteClient（行情客户端）。

P223 不接 scheduler（定时器），不改变 scheduler behavior（定时器行为）。

P223 不创建 scan loop（扫描循环）。

P223 不创建 real low-frequency scan（真实低频扫描）。
...
P223 不接 order API / execution API / auto-trading（下单接口 / 执行接口 / 自动交易）。order API（下单接口）/ execution API（执行接口）/ auto-trading（自动交易）不在 V1 范围内，保持关闭。

P223 不因为 interface skeleton plan（接口骨架方案）而大幅上调真实生产接线进度；真实生产接线仍保持 26%-34%，真实点位仍保持 10%-18%。

P223 后仍未完成 / 未实现 / 未升级清单：
...
- production adapter interface skeleton（生产适配器接口骨架）仍未实现。
- production read adapter（生产读取适配器）仍未实现。
