...
- P279 当前推进：Real Scan Input Contract DTO Skeleton（真实扫描输入契约 DTO 骨架）。
- P279 只允许 DTO-only / enum-only / targeted-test-only，不允许 service、Spring 注解、controller、endpoint、API、scheduler、market-read runtime wiring、real scan loop、score、Candidate、Push、Readiness、point generation 或 trading path。
- Future DTO may include symbol, source, requested scan reason, Watchlist Pool membership proof, watchlist config version, requested timeframe list, scan timestamp, market-read requirement flags, data availability expectations, stale/missing input behavior, risk blockers, and review-only safety flags。
- Future DTO must preserve review-only / fail-closed / audit-only where applicable。
- Future DTO must default `manualReviewRequired=true` and `notTradeInstruction=true`。
- Future DTO must not contain trade action, order, execution, entry, stop, take profit, RR, provider, external channel, message sending, or readiness fields。
- Watchlist Pool proof is required before any future scan input can be considered eligible。
...
- P275 external channel closure and CI exception record（external channel 收口与 CI 例外记录）已完成；P276 已完成 real scan / real score / Candidate / Push preview planning 回归范围包；P277 已完成 real scan contract audit / market-read boundary plan；P278 已完成 real scan input contract DTO authorization gate；P279 当前推进 real scan input contract DTO skeleton。
