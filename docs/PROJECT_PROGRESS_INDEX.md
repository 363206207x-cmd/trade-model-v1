...
- P290 当前推进：MarketReadRequestGuardValidator Java Skeleton（行情读取请求保护校验器 Java 骨架）。
- P290 是 B Validator Java/test。P290 adds `MarketReadRequestGuardValidator` skeleton、`MarketReadRequestGuardValidationResult`、`MarketReadRequestGuardValidationStatusEnum`、targeted validator test、P290 marker / closure docs, and state index updates。
...
- Runtime market-read Java remains blocked beyond the pure-data P288 DTO skeleton and P290 validator-only skeleton; service/controller/mapper/repository/scheduler/API/dashboard/schema/config/MarketQuoteClient/BinanceMarketQuoteClient/runtime reads remain unauthorized。
