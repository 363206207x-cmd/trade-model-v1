...
- Current HEAD: `4c829d8 docs(risk): close account exposure visual verification (#977)`.
- Active stop-loss track: `Source Read for Hot Reset / Event Impact Source review-only status`.
...
- Next required action: `Source Read for Hot Reset / Event Impact Source review-only status`.
...
- Next Minimal Runtime Slice Selection After Account Risk / Account Exposure Status Closure is completed by this package when merged. It selects `Hot Reset / Event Impact Source review-only status` as the next source-read-only target after seventeen completed review-only runtime partial slices, narrowing broad Macro-news / event calendar scope to local Hot Reset / Event Impact / SourceTrace event-source ownership assets and keeping external API refresh, scheduler/collector trigger, event generation, Hot Reset execution/write behavior, Push/Recheck, Candidate, Decision generation, Point, final direction, entry/stop/TP/RR, order/execution, trading, new skeleton owners, P359, and P360 blocked.
