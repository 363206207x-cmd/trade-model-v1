...
- Current HEAD: `4cf1af5 docs(runtime): select account risk exposure next slice`.
- Active stop-loss track: `Minimal Review-Only Account Risk / Account Exposure Status Runtime Wiring Design`.
...
- Source Read for Account risk / account exposure status is completed by this package when merged. It confirms the `tm_account_risk_snapshot` read model, `TmAccountRiskSnapshotDO`, `AccountRiskSnapshotMapper`, Push/Recheck account-risk read/classification boundary, ReviewAggregate account-risk summaries, AnalysisAssembler account-risk JSON write-side context, review-page display evidence, and targeted tests; it returns GO to design only.
...
- The next required action is `Minimal Review-Only Account Risk / Account Exposure Status Runtime Wiring Design`.
