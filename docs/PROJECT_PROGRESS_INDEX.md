...
P247 只做 docs-only closure / batch scan authorization gate（只改文档收口 / 批量扫描授权门），不是 batch implementation。

P247 不写 Java。

P247 不新增测试。

P247 不修改既有 Java / test。

P247 不修改 DTO 文件。

P247 不修改 guard / validator 文件。

P247 不修改 assembler 文件。

P247 不改 dashboard.html。

P247 不改 schema（数据库结构）。

P247 不改 config（配置）。

P247 不接 API（接口）。

P247 不改 mapper / controller / dashboard / production runtime service wiring（映射器 / 控制器 / 首页 / 生产运行时服务接线）。

P247 不读取 runtime / live / external data（运行时 / 实时 / 外部数据）。

P247 不接 MarketQuoteClient / BinanceMarketQuoteClient（行情客户端）。

P247 不启用 scheduler（定时器），不创建 scheduler activation（定时器激活）。

P247 不接 batch scan（批量扫描）。

P247 不创建真实 scan loop（扫描循环）。

P247 不创建 real low-frequency scan（真实低频扫描）。

P247 不实现 Production Source Assembler（生产数据源组装器）。

P247 不实现 real ScanScore computation（真实扫描分数计算）。

P247 不实现 Candidate Attention workflow（候选关注流程）。

P247 不实现 Promote To Home workflow（提升到首页观察流程）。

P247 不创建 Opportunity Push execution（机会推送执行）。

P247 不生成真实 entry / stop / TP / RR（入场 / 止损 / 止盈 / 盈亏比）。

P247 不升级 readiness（可执行就绪）。

P247 不接 order API / execution API / auto-trading（下单接口 / 执行接口 / 自动交易）。order API（下单接口）/ execution API（执行接口）/ auto-trading（自动交易）不在 V1 范围内，保持关闭。

P247 不因为 batch 授权门文档而大幅上调真实生产接线进度；真实生产接线仍保持 26%-34%，真实点位仍保持 10%-18%。

P247 后仍未完成 / 未实现 / 未升级清单：
