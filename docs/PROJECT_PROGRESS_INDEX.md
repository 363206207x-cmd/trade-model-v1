...
- `WORKFLOW-P336B-R2 Workflow Command Automation Retry` is active on branch `workflow-command-automation-v2`.
...
- WORKFLOW-P336B-R2 is a workflow script/docs fix. It adds fixed local command helpers for status detection and Draft PR creation, preserves the merge-sync already-merged fallback, and does not move the business chain or raise Production Runtime Progress.
- The active workflow fix is `Workflow Command Automation Retry`.
- The next required action is to review and merge the workflow command automation retry before any next business package.
