...
- Current HEAD: `f21ed5f docs(risk): verify risk action guard runtime wiring (#934)`.
- Active stop-loss track: `Next minimal runtime slice selection after RiskActionGuard closure`.
...
- Selected next minimal runtime slice is not chosen yet; it must be selected by the next A-risk selection package after RiskActionGuard closure.
- The next required action is `Next minimal runtime slice selection after RiskActionGuard closure`.
