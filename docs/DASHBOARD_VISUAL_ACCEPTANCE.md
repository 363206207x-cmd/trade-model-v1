...
- Acceptance result: **PASS** for the original ten browser fixture scenarios, thirteen interactions, and three desktop CSS viewports. Reviewer regressions `ai-all-abstain` and verified/unverified position-source rendering additionally pass deterministic fixture/DOM-target validation.
...
| G. Position with source-verified monitor record | 1440 x 900 | Load `position-monitored` | Position monitor owns the main section and exact source plan is collapsed | Entry/current/PnL/user stop/user TP, logic, direction support, reversal, risk, manual advice, last monitor, and `暂无下次监控排期` are present. Missing system stop/TP are `暂无`; fixture carries explicit source analysis/plan/trace IDs and the original plan starts collapsed. | Existing layout PNG `04-position-monitored-1440x900.png`; source identity is asserted separately in service/DOM tests | PASS |
| H. Position without verifiable original-plan source | 1366 x 768 | Load `position-waiting` | Uniform first-monitor waiting state and no empty historical-plan table | Logic, direction support, reversal, risk, advice, and monitor times say `等待首次监控` or `暂无`; execution area says `暂无可关联的原执行计划`; no original-plan disclosure/table or fabricated plan boundary is rendered. | Existing layout PNG `05-position-waiting-1366x768.png`; updated source contract has no new PNG claim | PASS |
...
| L. Position source A with newer same-symbol decision B | Deterministic service/DOM target | Resolve monitor-linked A while latest selected decision is B | Only A may appear in the collapsed historical review; unverified provenance shows only `暂无可关联的原执行计划` | Exact plan/analysis mapper paths are tested; the active-position branch cannot consume the new-opportunity decision; unverified rendering exits before constructing original-plan rows | No new PNG claimed; A/B isolation and DOM guard tests | PASS |
...
12. Bound the position's historical plan to exact same-position monitor provenance, separated identity from current validity, and suppressed the disclosure table when provenance is unverified.
