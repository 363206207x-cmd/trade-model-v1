...
- Current merged main: `f32c276 docs(runtime): gate internal push preview readiness (#994)`
...
- Current active block is `Minimal Review-Only Internal Push Preview / Notification Preview Status Runtime Wiring Implementation`.
- Current next required action is B-risk Draft PR review for `Minimal Review-Only Internal Push Preview / Notification Preview Status Runtime Wiring Implementation`; after merge, `Minimal Review-Only Internal Push Preview / Notification Preview Status Runtime Wiring Verification`.
...
- Minimal Review-Only Internal Push Preview / Notification Preview Status Runtime Wiring Implementation is completed by this package when merged. It adds one minimal read-only `/api/dashboard/internal-push-preview-notification-status` endpoint, dashboard `internalPushPreviewNotificationStatusPanel`, targeted `DashboardControllerTest` coverage, implementation docs, and source-of-truth updates over existing `ReviewOnlyInternalPushPreviewDTO`, `ReviewOnlyInternalPushPreviewAssembler`, dashboard `internalPushPreviewDisplay`, and no-op external channel policy owner assets. It preserves preview-only semantics and adds no Push send, external channel, sendable message, provider payload, PushSnapshot write, Recheck/Replay execution, Candidate generation, Decision generation, Point, final direction, entry/stop/TP/RR, order/execution, auto-trading, Position Monitor execution, external API refresh, scheduler/collector/API client refresh, schema/config/pom, new DTO/Validator/Assembler/Orchestrator, new owner family, P359, or P360. This package requires Draft PR B-risk review before merge and does not count as the 19th completed slice until follow-up verification and visual closure are merged.
