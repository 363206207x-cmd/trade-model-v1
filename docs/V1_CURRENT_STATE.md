...
- Current merged main: `8f404cd docs(wiring): read executionplan boundarycandidate display source path`
...
- Current active block is `Minimal Review-Only ExecutionPlan / BoundaryCandidate Runtime Wiring Design`.
- Current next required action is `Minimal Review-Only ExecutionPlan / BoundaryCandidate Runtime Wiring Implementation Readiness Gate`.
...
- The active design package fixes the future ExecutionPlan / BoundaryCandidate review-only owner path, status mapping, dashboard/API surface, fail-closed rules, completed-slice boundaries, and readiness checklist; it does not implement runtime behavior.
...
- The active package is `Source Read for ExecutionPlan / BoundaryCandidate review-only display continuation`; it reads existing owner assets and prepares a design-only next step.
