...
- Current merged main: `400e604 docs(risk): design account exposure runtime wiring`
...
- Current active block is `Minimal Review-Only Account Risk / Account Exposure Status Runtime Wiring Implementation`.
- Current next required action is `Minimal Review-Only Account Risk / Account Exposure Status Runtime Wiring Implementation`.
...
- Implementation readiness gate for Account risk / account exposure status is completed by this package when merged: it returns GO to B-risk minimal implementation over existing account-risk snapshot read assets, allows at most one minimal read-only `Map` endpoint if needed, a minimal dashboard account-risk / exposure status panel/copy/DOM if needed, targeted tests, implementation docs, and source-of-truth updates, and keeps account-risk write, PushSnapshot write, Push send, Recheck execution, trading authorization, position sizing, reduce/close/stop/reverse guidance, Candidate, Decision generation, Point, final direction, entry/stop/TP/RR, order/execution, auto-trading, new skeleton owners, schema/config/pom, replay/recheck, P359, and P360 blocked.
