...
- Current merged main: `a7782c9 docs(wiring): read boundary candidate execution plan owners (#843)`
...
- Current next required action is `Minimal implementation readiness gate for owner-path safety adapter merge`.
...
- The active merge design defines how Codex safety adapters can be absorbed into the existing `BoundaryCandidate` / `ExecutionPlan` / `DecisionResult` / dashboard adapter owner path without reviving P359/P360 or creating new wrapper owners.
...
- The active block is now Minimal Merge Design for BoundaryCandidate / ExecutionPlan owner + safety adapters.
