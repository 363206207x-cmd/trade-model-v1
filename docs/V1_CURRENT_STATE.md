...
- Current merged main: `1e15f39 docs(runtime): design internal push preview status wiring (#993)`
...
- Current next required action is `Minimal Review-Only Internal Push Preview / Notification Preview Status Runtime Wiring Implementation`.
...
- Implementation readiness gate for Internal Push preview / notification preview status is completed by this package when merged. It returns GO to B-risk minimal implementation only if scoped to existing internal push preview owner assets and no-op external channel evidence; permits at most one minimal read-only `Map` endpoint and a minimal dashboard status/copy/DOM surface; requires targeted endpoint/panel safety, fail-closed, Push send boundary, external channel boundary, PushSnapshot write boundary, and forbidden executable/action absent tests; and blocks Alert fatigue duplicate work, Push send, external channel, sendable message, PushSnapshot write, Recheck/Replay execution, Candidate generation, Decision generation, Point, final direction, entry/stop/TP/RR, order/execution, auto-trading, Position Monitor execution, external API refresh, scheduler/collector/API client refresh, schema/config/pom, new DTO/Validator/Assembler/Orchestrator, new owner families, P359, and P360.
