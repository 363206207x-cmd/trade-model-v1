...
- Current merged main: `833b2f6 docs(recheck): gate preview status implementation readiness (#1001)`
...
- Current active block is `Minimal Review-Only Recheck Preview / Recheck Status Runtime Wiring Implementation`.
- Current next required action is B-risk GPT / human review of the Draft PR; after merge, `Minimal Review-Only Recheck Preview / Recheck Status Runtime Wiring Verification`.
...
- Minimal Review-Only Recheck Preview / Recheck Status Runtime Wiring Implementation is completed by this package when merged. It adds one minimal read-only `/api/dashboard/recheck-preview-status` endpoint, dashboard `recheckPreviewStatusPanel`, targeted `DashboardControllerTest` coverage, implementation docs, and source-of-truth updates over existing persisted Recheck log / status contract / dispatch-read evidence owner assets. It does not call `recheck(...)`, `replayByDispatch(...)`, scheduler dispatch, collector/API-client refresh, `MarketQuoteClient.fetch24hTicker`, PushSnapshot write, dispatch config write/init/update, Push send, external channel, Candidate generation/ranking/scoring, Decision generation, Point, final direction, entry/stop/TP/RR, order/execution, auto-trading, Position Monitor execution, schema/config/pom, new skeleton owners, P359, or P360. This B-risk implementation stays in Draft PR review before merge and does not complete the 20th slice until verification and visual closure are merged.
