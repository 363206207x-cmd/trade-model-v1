...
- Current merged main: `78d4f83 BACKEND-P306 Readiness / Point Boundary Planning Gate (#747)`
...
- Current open business-chain package is PR #749 / branch `p307`: `BACKEND-P307 Review-only Readiness Gate Skeleton`.
...
- P307 is review-only readiness gate skeleton.
...
- Next recommended package is P308 Review-only Point Boundary / Proposal Gate.
...
## What P287-P306 Actually Completed

P287-P306 completed market-read request contract, DTO, validator, test-only wiring, review-only output, review-only scan output, review-only Evidence / Score entry, review-only evidence normalization, review-only score input / precheck, review-only score assembly, review-only candidate handoff, review-only candidate attention, review-only candidate preview / ranking guard, review-only internal push preview / recheck handoff skeleton, push preview closure before external channel, dashboard / internal push preview display gate, Candidate / Push review-only MVP closure, and Readiness / Point boundary planning only:
...
P307 is open on branch `p307` and adds the Review-only Readiness Gate Skeleton.
...
The current block is Review-only Readiness Gate Skeleton.
...
Review-only Readiness Gate Skeleton is active in P307 only and is not completed on main until the PR merges.

Evidence generation and score calculation are not completed.

Executable Readiness, point generation, external channel, order execution, execution API, and auto-trading are not completed.
