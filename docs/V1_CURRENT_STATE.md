...
- Current merged main: `e72d2a1 docs(runtime): select next slice after marketquote closure (#865)`
...
- Current next required action is `Minimal Review-Only Evidence / Score Runtime Wiring Design`.
...
- The active source-read package checks whether existing Evidence/Score owner paths, services, controllers/API, dashboard detail output, tests, schema/mappers, and review-only/fail-closed semantics are sufficient to enter design without implementation.
...
- The active block is now Source Read for Evidence / Score Review-Only Runtime Status.
