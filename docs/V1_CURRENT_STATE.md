...
- Current merged main: `701a019 docs(point): define source-owned candidate integration runtime assembler plan (#828)`
...
- Current open business-chain package is branch `p359`: `P359 Source-Owned Candidate Integration Runtime Assembler / Orchestrator Java Skeleton`.
...
- P359 is the active Source-Owned Candidate Integration Runtime Assembler / Orchestrator Java Skeleton package.
...
- P359 adds only the plain Java `SourceOwnedCandidateIntegrationRuntimeCandidateAssembler` skeleton, targeted assembler tests, and status documentation updates.
- P359 does not add Runtime Verification, service, controller, mapper, repository, scheduler, resources, schema, config, pom, dashboard runtime, source-owned candidate runtime service, internal preview, external channel, Push, order, execution, auto-trading, real entry, real stop, real TP, RR generation, final direction, point proposal, push payload, candidate runtime output, or executable action output.
- Fixed next recommended business package after P359 is `P360 Source-Owned Candidate Integration Runtime Assembler / Orchestrator Verification`, still not service runtime or runtime-safe usable implementation.
...
- P359: Source-Owned Candidate Integration Runtime Assembler / Orchestrator Java Skeleton defining explicit input movement into `SourceOwnedCandidateIntegrationRuntimeCandidateDTO`, immediate runtime validator invocation, and context plus validation result return.
