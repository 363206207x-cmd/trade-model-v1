...
- Current merged main: `4c829d8 docs(risk): close account exposure visual verification (#977)`
...
- Current active block is `Source Read for Hot Reset / Event Impact Source review-only status`.
- Current next required action is `Source Read for Hot Reset / Event Impact Source review-only status`.
...
- Next Minimal Runtime Slice Selection After Account Risk / Account Exposure Status Closure is completed by this package when merged: it selects `Hot Reset / Event Impact Source review-only status` as the next source-read-only target after seventeen completed review-only runtime partial slices, while deferring broad Macro-news / event calendar, Three AI conflict, Push preview, Recheck preview, Candidate preview, Position Monitor, and account-risk continuation paths because each is closer to external refresh, generation, execution, action guidance, or trading boundaries.
