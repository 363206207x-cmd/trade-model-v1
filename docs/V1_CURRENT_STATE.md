...
- Current merged main: `3299671 feat(recheck): show preview status review-only (#1002)`
...
- Current active block is `Minimal Review-Only Recheck Preview / Recheck Status Runtime Wiring Verification`.
- Current next required action is `Recheck Preview / Recheck Status Visual Verification / Closure` after this verification package is merged.
- Next Minimal Runtime Slice Selection After Hot Reset / Event Impact Source Closure is completed on main as `a29574f`. It selects `AI conflict / AI role convergence read-only status` as the next source-read-only target, narrowed from Three AI / AI conflict to existing AI conflict / DecisionResult / dashboard / review-page read-model assets only. It does not authorize Three AI provider orchestration, external AI calls, final arbiter behavior, Decision generation, final direction, entry/stop/TP/RR, Candidate generation, Point, Push, external channel, Recheck/Replay, order/execution, trading, Position Monitor execution, new skeleton owners, schema/config/pom, P359, or P360.
- Source Read for AI conflict / AI role convergence read-only status is completed on main as `4317597`. It confirms the existing DecisionResult `ai_role_results`, `ai_conflict_level`, `ai_conflict_score`, `ai_plan_mode`, mapper latest joined reads, `DecisionResultVO`, `/api/dashboard/decision-result-status`, dashboard AI shell, review-page AI display context, and existing dashboard tests. It identifies duplication with the already completed DecisionResult review-only dashboard/API status slice and allows only a design step for a narrow read-only DecisionResult projection.
- Minimal Review-Only AI conflict / AI role convergence status runtime wiring design is completed on main as `fca42d8`. It decides the module is duplication-sensitive and may continue only to a readiness gate for a narrow persisted DecisionResult read-only projection; default reuse is `/api/dashboard/decision-result-status` and the existing dashboard AI shell, while any dedicated endpoint/panel must be readiness-approved and must be NO-GO if it merely duplicates DecisionResult status.
- Implementation readiness gate for AI conflict / AI role convergence read-only status is completed on main as `0700a44`. It returns `NO-GO: duplicate with DecisionResult status` because the existing DecisionResult status API/dashboard owner path already covers the persisted AI conflict / role evidence. It blocks the B-risk implementation, dedicated AI conflict endpoint/panel, new skeleton owners, external AI call, Three AI provider orchestration, AI budget/cache/fallback orchestration, final arbiter behavior, `AiConflictResolverService` / `DecisionEngineService` runtime call, Decision generation, final direction, entry/stop/TP/RR, Candidate, Point, Push, Recheck/Replay, order/execution, trading, Position Monitor execution, schema/config/pom, P359, and P360.
- Next minimal runtime slice selection after AI conflict / AI role convergence NO-GO readiness is completed on main as `ea84df3`. It selects `Internal Push preview / notification preview status` as the next source-read-only target after #990 rejected AI conflict continuation as duplicate with DecisionResult status. The selected source read must inventory existing `ReviewOnlyInternalPushPreviewDTO`, `ReviewOnlyInternalPushPreviewAssembler`, dashboard `internalPushPreviewDisplay`, historical P303/P304 display-gate docs, no-op external channel policy assets, and PushSnapshot/PushRecheck risk paths without implementing Push send, external channel, Recheck/Replay execution, Candidate generation, Decision generation, Point, final direction, entry/stop/TP/RR, order/execution, auto-trading, new skeleton owners, P359, or P360.
- Source Read for Internal Push preview / notification preview status is completed on main as `a06dbb1`. It confirms existing `ReviewOnlyInternalPushPreviewDTO`, `ReviewOnlyInternalPushPreviewAssembler`, internal push preview tests, `CandidatePushReviewOnlyMvpClosureTest`, dashboard `internalPushPreviewDisplay`, P302-P305 closure docs, and no-op external channel policy assets. It identifies missing dedicated runtime status endpoint/panel, PushSnapshot / PushRecheck write-execution risks, and partial overlap with Alert fatigue / notification policy status; it returns GO to design only without Push send, external channel, PushSnapshot write, Recheck/Replay execution, Candidate generation, Decision generation, Point, final direction, entry/stop/TP/RR, order/execution, auto-trading, Position Monitor execution, external API refresh, scheduler/collector/API client refresh, new skeleton owners, schema/config/pom, P359, or P360.
- Minimal Review-Only Internal Push Preview / Notification Preview Status Runtime Wiring Design is completed on main as `1e15f39`. It reuses `ReviewOnlyInternalPushPreviewDTO`, `ReviewOnlyInternalPushPreviewAssembler`, dashboard `internalPushPreviewDisplay`, no-op external channel policy assets, and P302-P305 closure evidence; treats Alert fatigue / notification policy overlap as partial but not duplicate when scoped to internal push preview ownership; allows only a future readiness-approved minimal read-only `Map` endpoint/panel; and returns GO to implementation readiness gate only.
- Implementation readiness gate for Internal Push preview / notification preview status is completed on main as `f32c276`. It returns GO to B-risk minimal implementation only if scoped to existing internal push preview owner assets and no-op external channel evidence; permits at most one minimal read-only `Map` endpoint and a minimal dashboard status/copy/DOM surface; requires targeted endpoint/panel safety, fail-closed, Push send boundary, external channel boundary, PushSnapshot write boundary, and forbidden executable/action absent tests; and blocks Alert fatigue duplicate work, Push send, external channel, sendable message, PushSnapshot write, Recheck/Replay execution, Candidate generation, Decision generation, Point, final direction, entry/stop/TP/RR, order/execution, auto-trading, Position Monitor execution, external API refresh, scheduler/collector/API client refresh, schema/config/pom, new DTO/Validator/Assembler/Orchestrator, new owner families, P359, and P360.
- Minimal Review-Only Internal Push Preview / Notification Preview Status Runtime Wiring Implementation is completed on main as `07d37de`. It adds one minimal read-only `/api/dashboard/internal-push-preview-notification-status` endpoint, dashboard `internalPushPreviewNotificationStatusPanel`, targeted `DashboardControllerTest` coverage, implementation docs, and source-of-truth updates over existing `ReviewOnlyInternalPushPreviewDTO`, `ReviewOnlyInternalPushPreviewAssembler`, dashboard `internalPushPreviewDisplay`, and no-op external channel policy owner assets. It preserves preview-only semantics and adds no Push send, external channel, sendable message, provider payload, PushSnapshot write, Recheck/Replay execution, Candidate generation, Decision generation, Point, final direction, entry/stop/TP/RR, order/execution, auto-trading, Position Monitor execution, external API refresh, scheduler/collector/API client refresh, schema/config/pom, new DTO/Validator/Assembler/Orchestrator, new owner family, P359, or P360.
- Minimal Review-Only Internal Push Preview / Notification Preview Status Runtime Wiring Verification is completed on main as `37a14a4`. It verifies `/api/dashboard/internal-push-preview-notification-status`, dashboard `internalPushPreviewNotificationStatusPanel`, `ReviewOnlyInternalPushPreviewAssembler -> ReviewOnlyInternalPushPreviewDTO` owner path, safety fields, fail-closed/review-only states, Push send / external channel / PushSnapshot / Recheck / Replay / Candidate / Decision / Point / Trading boundaries, targeted `DashboardControllerTest`, full tests, forbidden semantics classification, and source-of-truth alignment without Java/test/dashboard/schema/config/pom changes.
- Internal Push Preview / Notification Preview Status Visual Verification / Closure is completed on main as `2c2972c`. Environment-limited evidence confirms `internalPushPreviewNotificationStatusPanel` DOM/copy/safety copy, preview-only / no-send / no-external-channel / no-sendable-message / no-provider-payload boundaries, PushSnapshot / Recheck / Replay / Candidate / Decision / Point / trading blocked copy, Alert fatigue / notification policy comparison-only scope, and no live UI success claim. Internal Push preview / notification preview status is the 19th completed Review-Only Runtime partial slice.
- Next minimal runtime slice selection after Internal Push Preview / Notification Preview Status closure is completed on main as `701a154`. It selects `Recheck preview / recheck status` as the next source-read-only target after nineteen completed Review-Only Runtime partial slices and explicitly keeps Recheck execution, Replay execution, scheduler/collector/API refresh, Push send, external channel, sendable message, provider payload, PushSnapshot write, Candidate generation/ranking/scoring, Decision generation, Point generation, final direction, entry/stop/TP/RR, order/execution, auto-trading, Position Monitor execution, new skeleton owners, schema/config/pom, P359, and P360 blocked.
- Source Read for Recheck preview / recheck status is completed on main as `3dfd350`. It inventories existing PushRecheck controller/service/mapper/schema/test assets, separates safe persisted-log/latest-log/replay-summary/ops-overview/status-contract/config-audit reads from forbidden Recheck execution, Replay execution, scheduler dispatch, MarketQuote/API refresh, PushSnapshot write, recheck-log write, dispatch-config write/init, Push send, external channel, Candidate/Decision/Point generation, final direction/entry-stop-TP-RR, order/execution/auto-trading, Position Monitor execution, new skeleton owners, schema/config/pom, P359, and P360, and returns GO to design only with duplication review against completed Review/Replay, Internal Push preview, and Alert fatigue slices.
...
- Implementation readiness gate for Recheck preview / recheck status is completed on main as `833b2f6`. It returns GO to B-risk minimal implementation only if scoped to persisted Recheck log / status contract / dispatch-read evidence; permits at most one minimal read-only `Map` endpoint and one minimal dashboard Recheck preview/status panel/copy/DOM; requires targeted safety, fail-closed, duplication, Recheck/Replay/scheduler/MarketQuote/PushSnapshot/dispatch-config/Push/Candidate/Decision/Point/Trading boundary tests; and blocks `recheck(...)`, `replayByDispatch(...)`, scheduler dispatch, collector/API-client refresh, `MarketQuoteClient.fetch24hTicker`, PushSnapshot write, Recheck log insert, dispatch config init/update/write, Push send, external channel, Candidate generation/ranking/scoring, Decision generation, Point, final direction, entry/stop/TP/RR, order/execution, auto-trading, Position Monitor execution, schema/config/pom, new skeleton owners, duplicate completed Review/Replay/Internal Push/Alert fatigue behavior, P359, and P360.
- Minimal Review-Only Recheck Preview / Recheck Status Runtime Wiring Implementation is completed on main as `3299671`. PR #1002 completed B-risk GPT / human review and merged. It adds one minimal read-only `/api/dashboard/recheck-preview-status` endpoint, dashboard `recheckPreviewStatusPanel`, targeted `DashboardControllerTest` coverage, implementation docs, and source-of-truth updates over existing persisted Recheck log / status contract / dispatch-read evidence owner assets. It does not call `recheck(...)`, `replayByDispatch(...)`, scheduler dispatch, collector/API-client refresh, `MarketQuoteClient.fetch24hTicker`, PushSnapshot write, dispatch config write/init/update, Push send, external channel, Candidate generation/ranking/scoring, Decision generation, Point, final direction, entry/stop/TP/RR, order/execution, auto-trading, Position Monitor execution, schema/config/pom, new skeleton owners, P359, or P360.
- Minimal Review-Only Recheck Preview / Recheck Status Runtime Wiring Verification is completed by this package when merged. It verifies `/api/dashboard/recheck-preview-status`, dashboard `recheckPreviewStatusPanel`, safety fields, fail-closed/review-only states, persisted Recheck log / status contract / dispatch-read owner path, Recheck/Replay/scheduler/MarketQuote/PushSnapshot/dispatch-config/Push/Candidate/Decision/Point/Trading boundary, targeted `DashboardControllerTest`, full tests, forbidden semantics classification, and source-of-truth alignment from `833b2f6` / `3299671` without Java/test/dashboard/schema/config/pom changes.
...
- Next Minimal Runtime Slice Selection After Runtime Readiness / System Guardrail Status Closure is completed on main as `4cf1af5` and selects `Account risk / account exposure status` as the next source-read-only target. This selection is docs/source-of-truth only and does not implement account-risk status, Push/Recheck behavior, executable readiness, trading authorization, position sizing, Candidate generation, Decision generation, Point, entry/stop/TP/RR, order/execution, auto-trading, or Position Monitor execution.
- Source Read for Account risk / account exposure status is completed on main as `1d87e1d`: it confirms `tm_account_risk_snapshot`, `TmAccountRiskSnapshotDO`, `AccountRiskSnapshotMapper`, account-risk read references in Push/Recheck and ReviewAggregate, account-risk JSON write-side context in AnalysisAssembler, review-page display evidence, and targeted tests; it returns GO to design only without account-risk write, PushSnapshot write, Push send, recheck execution, trading authorization, position sizing, Candidate, Decision generation, Point, order/execution, or trading behavior.
- Minimal Review-Only Account Risk / Account Exposure Status Runtime Wiring Design is completed on main as `400e604`: it selects the existing account-risk snapshot read owner path, prefers `AccountRiskSnapshotMapper.selectLatestByAnalysisId(...)` as the future read source, treats `riskAllowed` as read-only evidence rather than trading authorization, treats exposure/leverage/risk score as status rather than position sizing, and returns GO to implementation readiness gate without implementation or capability movement.
- Implementation readiness gate for Account risk / account exposure status is completed on main as `bcbeb15`: it returns GO to B-risk minimal implementation over existing account-risk snapshot read assets, allows at most one minimal read-only `Map` endpoint if needed, a minimal dashboard account-risk / exposure status panel/copy/DOM if needed, targeted tests, implementation docs, and source-of-truth updates, and keeps account-risk write, PushSnapshot write, Push send, Recheck execution, trading authorization, position sizing, reduce/close/stop/reverse guidance, Candidate, Decision generation, Point, final direction, entry/stop/TP/RR, order/execution, auto-trading, new skeleton owners, schema/config/pom, replay/recheck, P359, and P360 blocked.
- Minimal Review-Only Account Risk / Account Exposure Status Runtime Wiring Implementation is completed on main as `d3a79da`: it adds one minimal read-only `/api/dashboard/account-risk-exposure-status` endpoint, dashboard `accountRiskExposureStatusPanel`, targeted `DashboardControllerTest` coverage, implementation docs, and source-of-truth updates over `AccountRiskSnapshotMapper.selectLatestByAnalysisId(...)` without account-risk write, PushSnapshot write, Push send, Recheck execution, trading authorization, position sizing, reduce/close/stop/reverse guidance, Candidate, Decision generation, Point, final direction, entry/stop/TP/RR, order/execution, auto-trading, new skeleton owners, schema/config/pom, replay/recheck, P359, or P360.
- Minimal Review-Only Account Risk / Account Exposure Status Runtime Wiring Verification is completed on main as `16dabe4`: it verifies `/api/dashboard/account-risk-exposure-status`, `accountRiskExposureStatusPanel`, safety fields, fail-closed/review-only states, latest DecisionResult -> analysisId -> `AccountRiskSnapshotMapper.selectLatestByAnalysisId(...)` owner path, account action boundary, `DashboardControllerTest`, full tests, forbidden semantics classification, and source-of-truth alignment without Java/test/dashboard/schema/config/pom changes.
- Account Risk / Account Exposure Status Visual Verification / Closure is completed on main as `4c829d8`: environment-limited evidence confirms `accountRiskExposureStatusPanel` DOM/copy/safety copy, `riskAllowed` read-only evidence / not trading authorization copy, exposure read-only status / not position sizing copy, review-only/manual-review/fail-closed/not account-risk write/not PushSnapshot write/not Push send/not Recheck/not trading authorization/not position sizing/not reduce-close-stop-reverse guidance/not Candidate/not Decision generation/not Point/not final direction/not entry-stop-TP-RR/not trading/not executable boundaries, no live UI success is claimed, and Account risk / account exposure status is the 17th completed Review-Only Runtime partial slice.
- Next Minimal Runtime Slice Selection After Account Risk / Account Exposure Status Closure is completed on main as `e8e9a9f`: it selects `Hot Reset / Event Impact Source review-only status` as the next source-read-only target after seventeen completed review-only runtime partial slices, while deferring broad Macro-news / event calendar, Three AI conflict, Push preview, Recheck preview, Candidate preview, Position Monitor, and account-risk continuation paths because each is closer to external refresh, generation, execution, action guidance, or trading boundaries.
- Source Read for Hot Reset / Event Impact Source review-only status is completed on main as `00cee60`: it confirms Hot Reset rule/service write-side adjacency, the persisted `tm_hot_reset_event` / `HotResetEventMapper` read model, `EventImpactInputVO`, existing EvidenceService event-impact read path, SourceTrace event-source fail-closed ownership assets, run-baseline/review-page/dashboard context, and dedicated status endpoint/panel/safety-copy gaps; it returns GO to design only without Hot Reset execution/write, event generation, external API refresh, news fetch, scheduler/collector trigger, Push/Recheck/Replay, Candidate generation, Decision generation, Point, final direction, entry/stop/TP/RR, order/execution, trading, new skeleton owners, P359, or P360.
- Minimal Review-Only Hot Reset / Event Impact Source Status Runtime Wiring Design is completed on main as `761af58`: it selects persisted Hot Reset event reads, Event Impact input/evidence read semantics, SourceTrace event-source ownership fail-closed status, run-baseline Hot Reset summary, review-page `sec-hot-reset`, and dashboard `kpi-hot-reset` context as reusable owner/display assets; it permits only a future readiness-approved minimal read-only `Map` endpoint/panel and defines the safety fields, status mapping, fail-closed rules, and Hot Reset / event generation / refresh / Push / Recheck / Candidate / Point / trading boundaries without implementation or capability movement.
- Implementation readiness gate for Hot Reset / Event Impact Source review-only status is completed on main as `4d4a675`: it returns GO to B-risk minimal implementation over existing Hot Reset event persisted reads, Event Impact input/evidence read semantics, SourceTrace event-source ownership fail-closed status, run-baseline summary, and review/dashboard display contexts; it allows only a minimal read-only `Map` endpoint if needed, minimal dashboard status panel/copy/DOM, targeted tests, implementation docs, and source-of-truth updates, while forbidding Hot Reset execution/write, event generation, external API refresh, news fetch, scheduler/collector trigger, Push send, external channel, Recheck/Replay execution, Candidate/Decision/Point generation, final direction, entry/stop/TP/RR, order/execution/auto-trading, Position Monitor execution, schema/config/pom, new skeleton owners, P359, and P360.
...
- RuleConfig Runtime Audit / Rule Explainability Visual Verification / Closure is completed by this package when merged with environment-limited evidence: it verifies only the scoped dashboard RuleConfig audit status panel, safety copy, RuleVersionLog context-only boundary, `/api/rule/reload` boundary copy, layout boundary, and no forbidden semantics.
- The active package after this visual closure is `Next minimal runtime slice selection after RuleConfig closure`; it may add only selection docs and source-of-truth updates.
...
- Minimal Review-Only RiskActionGuard Read-Only Status Runtime Wiring Design is completed on main as `5d25e53`. It fixes owner path, status mapping, manual-review-only wording, fail-closed rules, dashboard/API boundary, and readiness checklist without implementation or capability movement.
- Implementation readiness gate for RiskActionGuard read-only status is completed on main as `c7fb97e`. It returns GO to B-risk minimal implementation over existing Dashboard detail / RiskActionGuard display assets, without DTO / Validator / Assembler / Orchestrator, schema/config/pom, Position Monitor execution, Push, Candidate generation, Decision generation, Point, final direction, entry/stop/TP/RR, order/execution, auto-trading, replay/recheck, P359, or P360.
- Minimal Review-Only RiskActionGuard Read-Only Status Runtime Wiring Implementation is completed on main as `06ca17f`. It adds a minimal read-only `/api/dashboard/risk-action-guard-status` endpoint, dashboard `riskActionGuardStatusPanel`, targeted `DashboardControllerTest` coverage, implementation docs, and source-of-truth updates.
...
- Alert Fatigue / Notification Policy Status Visual Verification / Closure is completed on main as `a9ec1c9`. Environment-limited evidence confirms `alertFatiguePolicyStatusPanel` DOM/copy/safety copy, review-only/fail-closed/not Push/not external channel/not recheck/not scheduler/not collector/not API-client refresh/not alert write/not trading/not Candidate/not Decision generation/not Point/not executable boundaries, no live UI success is claimed, and no Push / recheck / refresh / Candidate / Decision generation / Point / trading semantics are present. Alert fatigue / notification policy status is the 12th completed Review-Only Runtime partial slice.
- Next Minimal Runtime Slice Selection After Alert Fatigue Closure is completed on main as `8a2d8e7` and selects `SourceTrace / RuntimeKline / DataQuality / MultiTimeframe aggregate review-only status` as the next source-read-only target. This does not implement SourceTrace, RuntimeKline, DataQuality, MultiTimeframe, Point, Candidate, Push, Decision generation, order/execution, or trading behavior.
- Source Read for SourceTrace / RuntimeKline / DataQuality / MultiTimeframe aggregate review-only status is completed on main as `f4a274a`. It confirms the existing `/api/dashboard/detail` SourceTrace / RuntimeKline owner path, persisted OHLCV readiness read path, dashboard diagnostics, and tests, and returns GO to design only without implementation or capability movement.
- Minimal Review-Only SourceTrace / RuntimeKline / DataQuality / MultiTimeframe Aggregate Runtime Wiring Design is completed on main as `2ae6a4c`. It selects the existing dashboard detail owner path, defines the optional thin read-only Map endpoint boundary, status mapping, fail-closed rules, frozen source-binding boundary, and returns GO to implementation readiness gate without implementation or capability movement.
- Implementation Readiness Gate for SourceTrace / RuntimeKline / DataQuality / MultiTimeframe aggregate review-only status is completed on main as `dec084b`. It returns GO to B-risk minimal implementation over the existing dashboard detail / adapter owner path and forbids new DTO / Validator / Assembler / Orchestrator, new source-binding family, refresh, generation, point, Push, external channel, order/execution, trading, P359, and P360.
- Minimal Review-Only SourceTrace / RuntimeKline / DataQuality / MultiTimeframe Aggregate Runtime Wiring Implementation is completed on main as `71a8e2e`. It adds one minimal read-only `/api/dashboard/source-runtime-data-quality-status` endpoint, dashboard `sourceRuntimeDataQualityStatusPanel`, targeted `DashboardControllerTest` coverage, implementation docs, and source-of-truth updates over existing dashboard detail SourceTrace / RuntimeKline assets.
- Minimal Review-Only SourceTrace / RuntimeKline / DataQuality / MultiTimeframe Aggregate Runtime Wiring Verification is completed on main as `58b1ab5`. It verifies the endpoint/dashboard review-only behavior, safety fields, fail-closed/review-only states, refresh/generation boundary, tests, forbidden semantics classification, and source-of-truth alignment to `71a8e2e`.
- SourceTrace / RuntimeKline / DataQuality / MultiTimeframe Aggregate Visual Verification / Closure is completed on main as `f579b24`. Environment-limited evidence confirms `sourceRuntimeDataQualityStatusPanel` DOM/copy/safety copy, review-only/fail-closed/not scheduler/not collector/not API-client refresh/not external refresh/not source-binding generation/not Candidate/not Decision generation/not Point/not final direction/not entry-stop-TP-RR/not trading/not executable boundaries, no live UI success is claimed, and no refresh / generation / Candidate / Decision generation / Point / Push / trading semantics are present. SourceTrace / RuntimeKline / DataQuality / MultiTimeframe aggregate review-only status is the 13th completed Review-Only Runtime partial slice.
...
- Paper Observation / Paper Trading Status Visual Verification / Closure is completed on main as `8a4e594`. Environment-limited evidence confirms `paperObservationStatusPanel` DOM/copy/safety copy, review-only/manual-review/fail-closed/not real position/not trade instruction/not paper order/not simulated execution/not paper PnL/not Position Monitor execution/not Candidate/not Decision generation/not Point/not final direction/not entry-stop-TP-RR/not trading/not executable boundaries, no live UI success is claimed, and no paper execution / Candidate / Decision generation / Point / Push / trading semantics are present. Paper Observation / Paper Trading status is the 14th completed Review-Only Runtime partial slice.
- Next Minimal Runtime Slice Selection After Paper Observation / Paper Trading Status Closure is completed on main as `2f535cf` and selects `Review Archive Analytics / Missed Opportunity Aggregate Status` as the next source-read-only target. It does not implement archive analytics, missed opportunity aggregation, review result generation, replay/recheck, Push, Candidate generation, Decision generation, Point generation, Position Monitor execution, paper execution, order/execution, or trading behavior.
- Source Read for Review Archive Analytics / Missed Opportunity Aggregate Status is completed on main as `f5e7092`. It confirms the existing MissedOpportunity read/query/count owner path, `/api/missed-opportunity/review-archive-status`, `ReviewAggregateService` missed summary/detail metadata, review page `sec-missed`, dashboard `missedArchiveStatusPanel`, and targeted tests; it returns GO to design only without missed-opportunity generation/write behavior, review result generation, replay/recheck execution, Push, Candidate generation, Decision generation, Point, entry/stop/TP/RR, order/execution, auto-trading, DTO/Validator/Assembler/Orchestrator, schema/config/pom, new owner family, P359, or P360.
- Minimal Review-Only Review Archive Analytics / Missed Opportunity Aggregate Status Runtime Wiring Design is completed on main as `91332b7`. It keeps the existing status endpoint/panel as the preferred owner path, defines aggregate status mapping and safety fields, blocks generation/write/replay/recheck/Push/Candidate/Point/trading boundaries, and returns GO to implementation readiness gate only.
...
- Minimal Review-Only Review Archive Analytics / Missed Opportunity Aggregate Status Runtime Wiring Implementation is completed on main as `8504cb1`. It reuses `/api/missed-opportunity/review-archive-status` and `missedArchiveStatusPanel`, adds aggregate review-only status/safety fields, targeted `MissedOpportunityControllerTest` / `DashboardControllerTest` coverage, implementation docs, and source-of-truth updates without missed-opportunity generation/write, review result generation, replay/recheck, Push, Candidate, Decision, Point, final direction, entry/stop/TP/RR, order/execution, trading, new DTO/Validator/Assembler/Orchestrator, new owner family, P359, or P360.
