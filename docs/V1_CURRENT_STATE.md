...
```text
PR #665：BACKEND-P273 Provider Channel Closure and External Channel Authorization Scope Pack
Branch：p273
Issue：#664
风险档位：A docs-only Scope Pack, no Java/test/DTO/dashboard/schema/config/API/scheduler/external channel behavior/provider credentials/live provider calls/message rendering/message sending/readiness/point generation/trading path
状态：Draft PR（草稿合并请求）
```

P273 只允许 docs-only 收口 P272 provider channel disabled no-op Java skeleton，并定义 future external channel disabled no-op Java skeleton 的授权门。P273 不写 Java，不新增测试，不改 DTO，不实现 external channel behavior，不实现 provider selection，不处理 provider credentials，不做 live provider call，不接 Telegram / email / webhook / app notification / local notification，不实现 message rendering，不发送 message，不启用 scheduler，不接 API / dashboard，不读 runtime / live / external data，不升级 Readiness，不生成 point generation 或真实 entry / stop / TP / RR，不接 order / execution / auto-trading。

## 5. 当前 open Issue（未关闭问题单）

```text
#664：BACKEND-P273 Provider Channel Closure and External Channel Authorization Scope Pack
```

P204、P205、P206、P207、P208、P209、P210、P211、P212、P213、P214、P215、P216、P217、P218、P219、P220、P221、P222、P223、P224、P225、P226、P227、P228、P229、P230、P231、P232、P233、P234、P235、P236、P237、P238、P239、P240、P241、P242、P243、P244、P245、P246、P247、P248、P249、P250、P251、P252、P253、P254、P255、P256、P257、P258、P259、P260、P261、P262、P263、P264、P265、P266、P267、P268、P269、P270、P271、P272 和 WORKFLOW-P1 已合并，不再作为当前 open PR（未合并请求）处理。
...
```text
完成 P273 Provider Channel Closure and External Channel Authorization Scope Pack。
```

P273 属于 A docs-only Scope Pack，用于收口 P272 disabled no-op provider channel skeleton，并定义 future external channel authorization scope gate（未来 external channel 授权范围门）和 future external channel Java authorization gate（未来 external channel Java 授权门）。P273 不修改 Java，不新增测试，不改 DTO，不改 dashboard，不改 schema / config，不接 API，不新增 controller / endpoint / mapper / repository / service / scheduler，不实现 external channel behavior，不实现 provider selection，不处理 provider credentials，不做 live provider call，不实现 message rendering，不发送任何 message，不接 Telegram / email / webhook / app notification / local notification，不接 MarketQuoteClient 或 BinanceMarketQuoteClient，不读取 runtime / live / external data，不创建 scan loop，不实现 external Opportunity Push execution，不实现 Promote To Home runtime logic，不升级 Readiness，不生成 point generation 或 entry / stop / TP / RR，不接 order / execution / auto-trading。
