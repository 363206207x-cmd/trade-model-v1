...
- Current merged main: `07d37de feat(push): show internal push preview review-only status`
...
- Current active block is `Minimal Review-Only Internal Push Preview / Notification Preview Status Runtime Wiring Verification`.
- Current next required action is `Minimal Review-Only Internal Push Preview / Notification Preview Status Runtime Wiring Verification`; after merge, `Internal Push Preview / Notification Preview Status Visual Verification / Closure`.
...
- Minimal Review-Only Internal Push Preview / Notification Preview Status Runtime Wiring Verification is completed by this package when merged. It verifies `/api/dashboard/internal-push-preview-notification-status`, dashboard `internalPushPreviewNotificationStatusPanel`, `ReviewOnlyInternalPushPreviewAssembler -> ReviewOnlyInternalPushPreviewDTO` owner path, safety fields, fail-closed/review-only states, Push send / external channel / PushSnapshot / Recheck / Replay / Candidate / Decision / Point / Trading boundaries, targeted `DashboardControllerTest`, full tests, forbidden semantics classification, and source-of-truth alignment without Java/test/dashboard/schema/config/pom changes.
