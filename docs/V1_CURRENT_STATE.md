...
- Current merged main: `7e73b41 docs(wiring): map source-owned runtime and point proposal ownership (#842)`
...
- Current next required action is `Minimal merge design for BoundaryCandidate / ExecutionPlan owner + safety adapters`.
...
- The active source read verifies `BoundaryCandidateService`, `BoundaryCandidateDTO`, `PlanService`, `ExecutionPlanVO/DO/Mapper`, `DecisionResult` read model, and dashboard display adapters as canonical owner candidates before merge design.
...
- The active block is now Targeted Source Read for BoundaryCandidate / ExecutionPlan owner.
