...
```text
PR #697：BACKEND-P289 MarketReadRequestDTO Closure and Guard Validator Authorization Scope Pack
Branch：p289
Issue：#696
风险档位：A docs-only Scope Pack, no Java/tests/DTO/dashboard/schema/config/API/service/controller/scheduler/MarketQuoteClient/BinanceMarketQuoteClient/runtime data read/scan output/real scan loop/production ScanScore/Candidate/Push/Readiness/point generation/trading path
状态：Draft PR（草稿合并请求）
```

P288 merged as `d837519`。P288 完成 `MarketReadRequestDTO` pure-data DTO skeleton 和 targeted DTO test。P288 不是真实行情读取，不代表 `MarketQuoteClient` / `BinanceMarketQuoteClient` 接线，不创建 scan output，不创建 real scan loop，不实现 ScanScore，不实现 Candidate / Push / Readiness / 点位，不生成 entry / stop / TP / RR，不接 order / execution / auto-trading。

P289 是 docs-only Scope Pack。P289 收口 P288，并授权未来 P290 只实现 `MarketReadRequestGuardValidator` Java skeleton。P289 不写 Java，不改 tests，不改 DTO。

`MarketReadRequestDTO` fields are frozen to symbol, requestId, sourceContractId, watchlistPoolProof, watchlistConfigVersion, requestedScanReason, requestedTimeframes, scanTimestamp, dataAvailabilityExpectation, stalePolicy, missingDataPolicy, riskBlockers, reviewOnly, notTradeInstruction, guardValidationStatus, and blockingReasons。
...
Future P290, if implemented, must be pure validator only：only validate `MarketReadRequestDTO`，only return blocked / review-only / fail-closed validation result，and no service / runtime / market / score / push / readiness / point / trading behavior。
...
```text
#696：BACKEND-P289 MarketReadRequestDTO Closure and Guard Validator Authorization Scope Pack
```

P204、P205、P206、P207、P208、P209、P210、P211、P212、P213、P214、P215、P216、P217、P218、P219、P220、P221、P222、P223、P224、P225、P226、P227、P228、P229、P230、P231、P232、P233、P234、P235、P236、P237、P238、P239、P240、P241、P242、P243、P244、P245、P246、P247、P248、P249、P250、P251、P252、P253、P254、P255、P256、P257、P258、P259、P260、P261、P262、P263、P264、P265、P266、P267、P268、P269、P270、P271、P272、P273、P274、P275、P276、P277、P278、P279、P280、P281、P282、P283、P284、P285、P286、P287、P288 和 WORKFLOW-P1 已合并，不再作为当前 open PR（未合并请求）处理。
...
```text
完成 P289 MarketReadRequestDTO Closure and Guard Validator Authorization Scope Pack。
```

P289 属于 A docs-only Scope Pack。P289 只允许新增 P289 文档并更新当前状态索引。P289 不允许修改 Java/tests/DTO，不允许 service/controller/mapper/repository/scheduler/dashboard/schema/config/API，不允许 MarketQuoteClient / BinanceMarketQuoteClient，不允许 runtime/live/external data read，不允许 scan output，不允许 real scan loop，不允许 production ScanScore，不允许 Candidate production workflow，不允许 Opportunity Push execution，不允许 Readiness，不允许 point generation，不允许 entry-stop-TP-RR，不允许 order/execution/auto-trading。
