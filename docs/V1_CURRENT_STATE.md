...
- Current merged main: `7593c10 docs(wiring): verify review-only positionsync runtime slice (#840)`
...
- Current next required action is `Source-Owned Runtime vs Existing Point Proposal Merge Map`.
...
- The active visual verification checks `/dashboard` browser visibility, PositionSync field rendering, safety copy visibility, simulated fallback warning visibility, layout overlap, misleading copy risk, and absence of trading action semantics.
...
- The active block is now Dashboard PositionSync Visual Verification.
