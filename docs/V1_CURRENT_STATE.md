...
- Current merged main: `4cf1af5 docs(runtime): select account risk exposure next slice`
...
- Current active block is `Minimal Review-Only Account Risk / Account Exposure Status Runtime Wiring Design`.
- Current next required action is `Minimal Review-Only Account Risk / Account Exposure Status Runtime Wiring Design`.
...
- Source Read for Account risk / account exposure status is completed by this package when merged: it confirms `tm_account_risk_snapshot`, `TmAccountRiskSnapshotDO`, `AccountRiskSnapshotMapper`, account-risk read references in Push/Recheck and ReviewAggregate, account-risk JSON write-side context in AnalysisAssembler, review-page display evidence, and targeted tests; it returns GO to design only without account-risk write, PushSnapshot write, Push send, recheck execution, trading authorization, position sizing, Candidate, Decision generation, Point, order/execution, or trading behavior.
