...
| Manual position with monitor record | Initial Home load and position panel view | Position monitoring takes over; persisted monitor result/time and calculated PnL are shown; no new-position plan is promoted | Position mode uses latest persisted monitor record and read-only current quote | `homePositionUsesLatestPersistedMonitorLogAndCalculatesLongPnl`, `positionMonitorUsesRealMonitorFields`, `executionCardSwitchesToRealPositionMonitorWithoutInventingSystemStops` | PASS |
| Manual position without monitor record | Initial Home load and position panel view | Monitoring fields say `等待首次监控`; no monitor result or next time is fabricated | No log maps to `WAITING_MONITOR`; a prior log with no next schedule instead says `暂无下次监控排期` | `homeAggregatesStableReadOnlySemanticsWithoutCrossFallbacks`, `existingMonitorWithoutNextScheduleDoesNotSayFirstMonitor` | PASS |
