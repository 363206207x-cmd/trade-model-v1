...
When JSON MIME output fails normalization or strict V1 validation, the controlled smoke may emit `GEMINI_RESPONSE_SHAPE_DIAGNOSTIC`. It retains only sanitized top-level field names, nested object paths, JSON types, expected/missing/unexpected field names, and type mismatches. The raw candidate, field values, summary, reasoning, prompt, request body, headers, request ID, and API key are never retained in the diagnostic or printed. The evidence distinguishes a deterministic alias gap from an unsupported wrapper or unsafe semantic mismatch, but it never changes mappings or repairs output automatically.
