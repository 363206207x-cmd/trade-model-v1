...
| GAP-P0-005 | Product-contract multi-timeframe decision (`CLOSED`) | Decision Engine reads authoritative persisted `5m/15m/1h/4h`; formal push invalidation uses 5m, not 1m | `IMPLEMENTED_AND_TRACED` | Closed by `PROVIDER_SNAPSHOT_ADOPTION_CLOSURE.md`; preserve four-timeframe fail-closed tests. |
| GAP-P0-006 | Unified provider-call and snapshot adoption (`CLOSED`) | Dashboard/Decision/PositionMonitor/PushRecheck/market environment use coordinated snapshots; Decision uses persisted OHLCV; architecture guard bans direct clients | `IMPLEMENTED_AND_TRACED` | Code adoption closed; live-provider and production release evidence remain separate gates. |
