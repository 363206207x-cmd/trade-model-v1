...
- Baseline: PR #1187 at `303165e0e935bab5a474767f425b2420be8445a6`
...
This matrix was produced before production-code remediation. `GAP` means remediation is required in this package. `PASS/REGRESSION` means the owner already satisfies the frozen contract and must be protected by the final regression gate.
...
| HTTP 451 | routed provider + capability owner | runtime observation | `REGION_RESTRICTED` | 451 repeat-call test | PARTIAL | Persist region restriction and block subsequent calls until verified revalidation. |
...
| Current-price refresh | `MarketPriceSnapshotService` | exact Binance mapping + coordinator only | not configured/unavailable | quote call-count tests | GAP | Add unified pre-call capability decision for exact spot identity. |
| Derivatives refresh | `BinanceDerivativesSnapshotService` | exact Binance perpetual mapping + coordinator only | not configured/unavailable | derivatives call-count tests | GAP | Add unified pre-call capability decision for exact perpetual identity. |
| Push recheck price | `PushRecheckServiceImpl` -> price snapshot | downstream gate | quote unavailable | push recheck test | PARTIAL | Covered by the price snapshot gate. |
| Position monitoring price | `PositionMonitorServiceImpl` -> price snapshot | downstream gate | current data unavailable | position monitor test | PARTIAL | Covered by the price snapshot gate. |
...
| Runtime dataset call | `AbstractCoinGlassDatasetSnapshotService` | enable/key/external only | not configured | zero external-call tests | GAP | Add RPM state gate before coordinator/client. |
...
5. Re-run the complete B01-B04 suite, standard executable-JAR smoke, PostgreSQL V1-to-V13 migration/restart/checksum validation, source/workflow gates, and secret/auto-trading scans.
...
| B. Provider capability | PASS | Unified pre-call owner, exact identity, dataset-specific enablement, independent fallback, stale directory-only revalidation, zero-call matrix, architecture guard, truthful partial scan |
