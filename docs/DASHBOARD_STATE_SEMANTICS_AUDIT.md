...
- An active manual position takes over the selected asset's main suggestion area. Position Monitor writes source IDs only after resolving an explicitly typed `EXECUTION_PLAN:<id>` reference, or `ANALYSIS:<id>` when that analysis has exactly one plan. Dashboard does not trust those IDs merely because they are non-sentinel: the monitor analysis/plan IDs must match the current typed position source and the exact plan/run/symbol chain. Legacy untyped `sourceRefId` values remain diagnostic-only. Neither layer selects a latest sibling or latest same-symbol decision. A verified original plan is retained only as a collapsed review/reference record.
...
- `PositionPlanSourceResolver` is the shared read-only identity resolver for active monitoring, Dashboard original-plan review, and closed-position review. Typed-position fallback may resolve an exact plan or a typed analysis with exactly one plan. A monitor row is stricter: it must carry both exact analysis and plan IDs, those IDs must agree with the current typed position source, and the plan/run/analysis/symbol chain must pass. Closed-position feedback uses `USER_POSITION_<id>` when identity is unverified and never attaches to a foreign analysis.
- `POSITION_SOURCE_UNVERIFIED` remains an internal non-null persistence sentinel only. Generic monitor DTO/API reads cannot perform the database join and therefore never claim verification: real-looking IDs are `PENDING_VERIFICATION / 来源待验证`, while the sentinel is cleared as `UNVERIFIED / 来源不可验证`. Dashboard and closed-review business models expose `VERIFIED / 来源已验证` only after `PositionPlanSourceResolver` passes; failed business joins clear both IDs.
...
| Position original-plan identity | `PositionMonitorSourceContract`, `PositionPlanSourceResolver`, same-position monitor logs, `ExecutionPlanMapper`, `AnalysisRunMapper` | Active monitoring, Dashboard, and closed review share one resolver. Monitor-derived identity additionally requires exact monitor plan/analysis IDs to match the current typed position source. Closed review chooses the newest row that passes that contract, preserving frozen plan A even if sibling B appears later. No latest/sibling/symbol fallback exists. |
| Monitor source presentation | `PositionMonitorLogSourceViewPolicy`, `PositionMonitorLogDTO` | A generic log read is `PENDING_VERIFICATION` even when IDs look normal. The database sentinel is cleared as `UNVERIFIED`. Only a resolver-backed business read may mark `VERIFIED`; failed Dashboard/Review joins clear IDs so historical guessed siblings cannot reach user business copy. |
...
| 多持仓待选择 | Selected symbol has two or more active manual positions and no exact position identity | `POSITION_SELECTION_REQUIRED` | `positionSelectionStatus`, `matchingPositionCount`, null `selectedPositionId` | 请选择具体持仓 | Symbol-only selection could silently choose the first position and cross plan A/B. | No position or plan is selected; every original-plan field remains empty until the user selects a concrete row. |
| 精确持仓选择 | Caller provides a positive `positionId` for an active manual position matching the selected symbol | `EXACT_POSITION_SELECTED` | `selectedPositionId`, `positionSelectionStatus` | Selected row plus its own monitor/plan | Position identity could be inferred from list order. | Exact ID is checked before trusted monitor-source resolution. Unknown, closed, non-manual, and cross-symbol IDs fail closed without fallback. |
...
| UTC-naive metrics were split across JVM/database local clocks | Decision and missed-opportunity “today” used different calendar dates; Monitor Alert writes/throttles used JVM/database local clocks; Hot Reset fallbacks used JVM local time; Baseline readers used application UTC. | Rounds 8/10 introduced one UTC day and explicit Baseline bounds. Round 11 explicitly persists new Monitor Alert UTC-naive timestamps, uses the same bounds for throttling, and moves Hot Reset service fallbacks to an injected UTC clock. Historical rows remain unverified. |
...
1. `PositionPlanSourceResolver` recognizes only `EXECUTION_PLAN:<id>` and `ANALYSIS:<id>`. Before returning trusted source IDs it verifies the exact plan, its analysis ID, the corresponding `AnalysisRun`, ID agreement, and the position/run symbol. Its monitor entry point also requires both monitor IDs and verifies them against the current typed position reference.
2. An untyped/invalid/mismatched source still permits price/risk/manual monitoring, but it cannot establish plan identity. Generic log reads preserve non-sentinel IDs only as controlled `PENDING_VERIFICATION` diagnostics; sentinel values are cleared. Dashboard and Review Center clear all IDs on failed resolver validation.
3. Dashboard identity is verified only from the latest monitor row for the same `positionId`, and only when that row agrees with the current typed `UserPosition.sourceRefId`. `ANALYSIS:X` monitor rows still require an exact `executionPlanId`; Dashboard never reruns unique/latest analysis selection.
4. Closed-position review examines the same position's monitor history newest-first, chooses the newest row satisfying the typed-source contract, and restores its exact plan ID. Only when no trusted monitor row exists does it fall back to the typed position reference. Summary and feedback share this resolver path. Failure yields `PLAN_CONTEXT_MISSING`, `NOT_COMPUTABLE`, empty plan fields, `PLAN_SOURCE_UNVERIFIED`, and feedback identity `USER_POSITION_<positionId>`.
...
### 7.2 Reviewer Round 9 multi-position closure

Dashboard Home accepts optional `positionId` in addition to `selectedSymbol`. The symbol establishes asset context; it is not a position identity. With no `positionId`, zero matching active manual positions keeps new-opportunity mode, one matching position may auto-select, and two or more matching positions fail closed with `POSITION_SELECTION_REQUIRED` and `请选择具体持仓`.

An explicit ID is accepted only when it identifies a `MANUAL` position in `OPEN` or `PARTIALLY_CLOSED` status and its normalized symbol matches the selected asset. Unknown, non-positive, closed, non-manual, and cross-symbol IDs return a blocked selection status and never fall back to list order, latest open time, largest ID, latest monitor from another position, latest same-symbol decision, or sibling plan. Only after exact position selection does the existing trusted monitor-source resolver load that position's exact historical plan.
...
Run Baseline calculates `asOfUtc` once and derives `windowStartUtc` once. The same inclusive pair is passed to Monitor Alert, Analysis Run, low-data-quality Analysis Run, Push Recheck, Hot Reset count, and Hot Reset trigger-type distribution. Their Baseline SQL contains explicit `>= #{windowStartInclusive}` and `<= #{asOfInclusive}` predicates, so records after the as-of boundary are excluded. Round 10 left the Monitor Alert write-side throttle methods unchanged; Round 11 below closes that separate writer/read-window contract.

This closes the Baseline read dependence on database-local clocks. It does not claim that every historical `analysis_time`, Monitor Alert timestamp, or Hot Reset `event_time` has a uniformly proven time basis; that broader inventory remains a separate audit. Production readiness remains `BLOCKED`.

### 7.4 Reviewer Round 11 Monitor Alert and Hot Reset write closure

`MonitorAlertWriteServiceImpl` now derives `created_at`, `updated_at`, `cooldown_until`, the throttle start, and the semantic-suppression start from one injected UTC clock read per alert candidate. `MonitorAlertMapper.insert` binds typed `LocalDateTime` values explicitly, and the two write-side window queries share the same inclusive UTC bounds as Run Baseline. New writes no longer depend on database `DEFAULT CURRENT_TIMESTAMP`.

`HotResetServiceImpl` uses the same UTC clock policy for null command-time fallback, event/create/completion timestamps, asset-state updates, invalidation timestamps, and rebuild outcome completion. A supplied `HotResetCommand.occurredAt` has an explicit UTC-naive contract; trigger thresholds and state transitions are unchanged.

This closure is deliberately prospective. Monitor Alert rows written before Round 11 may reflect the old database session timezone, and historical Hot Reset caller times cannot be proved UTC from the stored value alone. No historical conversion or production migration was executed. The PostgreSQL three-session-timezone test is present but was skipped locally because Docker/Testcontainers had no daemon/socket; it is not claimed as PASS. Production readiness remains `BLOCKED`.
...
- Position plan A remains isolated from a newer same-symbol decision B. Dashboard monitor identity always requires exact plan ID plus typed-position agreement; analysis-only monitor rows fail closed without querying a sibling.
- Same-symbol multi-position tests cover unique auto-selection, explicit A/B selection, no-ID selection-required state, unknown/cross-symbol/closed/non-manual IDs, and the invariant that A/B entries, stops, take profits, analysis IDs, and plan IDs never cross. Controller and DOM tests prove row clicks send `positionId`, asset clicks clear it, and a blocked refresh clears the previous plan.
- The open-monitor -> manual-close -> closed-review acceptance keeps exact plan A through both lifecycle reads while a mocked latest sibling B remains unused. A typed `ANALYSIS:X` position with monitor-frozen A still reviews A after B appears; summary and feedback use the same A resolution. Legacy non-sentinel/untyped rows and typed-source mismatches are cleared rather than promoted.
...
- New Monitor Alert writes use one fixed UTC-naive timestamp under JVM defaults `UTC`, `Asia/Shanghai`, and `America/New_York`; throttle/semantic windows derive from that same value, Mapper insert bypasses database defaults, the real H2 Writer-to-Baseline path counts the exact as-of row, and a one-second future row is excluded.
- Hot Reset fallback, legacy delegation, event/create/completion, asset-state update, and rebuild timestamps use the injected UTC clock. The controlled PostgreSQL session-timezone test is environment-gated and remains skipped on this Docker-daemon-unavailable workstation.
...
9. `tm_analysis_run.analysis_time`, upgrade-era `tm_monitor_alert` timestamps, and historical caller-supplied `tm_hot_reset_event.event_time` values have no uniformly provable time basis. New Monitor Alert and Hot Reset fallback writes are UTC-naive and Baseline reads use explicit UTC boundaries, but historical cleanup/cutoff policy and controlled PostgreSQL session-timezone execution remain production-readiness gates.
