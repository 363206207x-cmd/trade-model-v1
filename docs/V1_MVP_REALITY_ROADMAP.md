...
| review-only score input / precheck | Active P297 PR #729: normalized evidence becomes a score input precheck skeleton, not real ScoreItem and not score calculation. | Review-only score assembly. | B | Keep real ScoreItem generation, score calculation, Candidate, Push, Readiness, and point generation separate. |
| review-only ScanScore | Review-only calculator skeleton exists. | Review-only score over normalized evidence / precheck output with targeted tests. | B | Can group with score explanation output after review-only score assembly. |
