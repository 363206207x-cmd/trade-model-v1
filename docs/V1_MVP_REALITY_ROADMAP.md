...
| Market read request review-only output | Completed in P293: validation result becomes readable review-only output with status, reasons, blockers, safety flags, and next-step message. | P294 review-only scan output skeleton. | B | Do not combine with provider wiring, scan score, Candidate, Push, Readiness, or point generation. |
| Review-only MarketRead scan output | Active P294: MarketRead request review output becomes a review-only scan output skeleton. | Evidence / Score entry point after P294 merges. | B | Keep production scan output, Evidence generation, Score, Candidate, Push, Readiness, and point generation separate. |
