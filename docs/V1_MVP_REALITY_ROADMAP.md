...
| Review-only MarketRead scan output | Completed in P294: MarketRead request review output becomes a review-only scan output skeleton. | Review-only Scan Output to Evidence / Score Entry. | B | Keep production scan output, Evidence generation, Score, Candidate, Push, Readiness, and point generation separate. |
...
| scan output | Review-only MarketRead scan output skeleton exists after P294; no production output from live read. | Review-only Scan Output to Evidence / Score Entry. | B | Can group with ScanScore input contract if no runtime provider. |
