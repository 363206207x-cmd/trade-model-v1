...
| Exact implementation paths | The ninety-seven literal paths in `docs/CODEX_NEXT_TASK.yml`; no wildcard, directory or prefix grant; the AI evidence-canonicalization amendment and responsive browser-only `dashboard-mobile.js` path keep the package, implementation branch and starting SHA unchanged; no native, Electron or WebView APP client is authorized |
