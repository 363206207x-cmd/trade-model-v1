...
| 4 | Schema migration from current main state | LIVE5 SKIPPED | PDR-PF4 defines the drill process and LIVE5 adds a guarded no-op/default helper, but controlled source DB env, recovery DB env, and local PostgreSQL client tools are missing. Backup is `SKIPPED_MISSING_CONTROLLED_DB`, restore is `SKIPPED_MISSING_RECOVERY_DB`, and current-state migration rehearsal is `SKIPPED`. |
...
3. Migration from current main/live state is not executed; PDR-PF4 defines the rehearsal plan and LIVE5 adds a guarded helper, but current-state rehearsal is `SKIPPED` because controlled source/recovery DB env and PostgreSQL client tools are missing.
4. Backup and restore drill evidence is still missing; LIVE5 records backup `SKIPPED_MISSING_CONTROLLED_DB` and restore `SKIPPED_MISSING_RECOVERY_DB` until run in a safe staging/server-backed environment.
...
16. `PDR-LIVE5 Controlled Current-State Migration + Restore Drill Evidence`: current package; adds a guarded no-op/default helper and records backup `SKIPPED_MISSING_CONTROLLED_DB`, restore `SKIPPED_MISSING_RECOVERY_DB`, and current-state migration rehearsal `SKIPPED`.
