...
25. `PDR-LIVE14 Secrets Manager Credential Rotation Evidence`: current package; records secrets manager and credential rotation as `DOCUMENTED_WITH_PLAN`, keeps real secret-store injection and rotation drill evidence missing, and keeps production readiness BLOCKED.
