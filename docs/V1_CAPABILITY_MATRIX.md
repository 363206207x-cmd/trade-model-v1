...
| MarketReadRequest test-only wiring | 4 TEST_ONLY_WIRING | Test-scope helper feeds `MarketReadRequestDTO` into `MarketReadRequestGuardValidator` and returns review-only validation result. | No production service wiring and no runtime market-read execution. | Review-only MarketRead output / scan output slice. | Not production wiring. |
| MarketReadRequest assembler | 4 TEST_ONLY_WIRING | Test-only assembler slice returns validation status, blocking reasons, risk blockers, manual-review and not-trade-instruction flags. | No production assembler and no conversion from RealScanInputContractDTO in runtime code. | Review-only MarketRead output assembler pack. | Not runtime provider call or production assembler. |
