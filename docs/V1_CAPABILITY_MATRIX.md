...
| Review-only MarketRead scan output | REVIEW_ONLY_SCAN_OUTPUT_SKELETON | Active P294 PR #713 adds a review-only scan output DTO and assembler from `MarketReadReviewOnlyOutputDTO`. | No production scan output, no runtime market read, no Score, no Evidence generation, no Candidate, no Push, no Readiness, and no point generation. | Evidence / Score entry point over review-only scan output after P294 merges. | Not production scan output, not score, not Evidence, not Candidate, not Push, not Readiness. |
