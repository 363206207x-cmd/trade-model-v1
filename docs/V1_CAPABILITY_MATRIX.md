...
| MarketReadRequest review-only output assembler | REVIEW_ONLY_OUTPUT_SKELETON | Active P293 PR #711 adds a review-only DTO and assembler from `MarketReadRequestDTO` + guard result into readable manual-review output. | No production assembler, no provider call, no runtime market-read execution, and no scan output. | Review-only MarketRead output / scan output slice after P293 merges. | Not runtime provider call, not production assembler, not scan output. |
