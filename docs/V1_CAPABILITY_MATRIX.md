...
| Score input / precheck | REVIEW_ONLY_SCORE_INPUT_PRECHECK_SKELETON | Active P297 PR #729 adds a review-only score input precheck DTO and assembler from `ReviewOnlyNormalizedEvidenceDTO`. | No ScoreItem generation, no score calculation, no trend/liquidity/macro/confidence/leverage/sentiment/final score, no Candidate, no Push, no Readiness, and no point generation. | Review-only score assembly after P297 merges. | Not real ScoreItem, not score calculation, not Candidate, not Push, not Readiness. |
...
| ScanScore | 3 TARGETED_TEST | DTO / rule / review-only calculator skeleton and tests exist. | No production ScanScore over live scan output, and P295-P297 only add entry, normalization, and precheck envelopes, not score calculation. | Review-only score assembly before broader score runtime. | Not production scoring. |
