...
| Evidence / Score entry point | 0 NOT_STARTED | No merged entry point consumes `MarketReadReviewOnlyScanOutputDTO` for Evidence / Score yet. | No Evidence item generation, no Score item generation, no production ScanScore, no Candidate, no Push, no Readiness, and no point generation. | Review-only Scan Output to Evidence / Score Entry. | Not completed Evidence, not completed Score, not Candidate, not Push, not Readiness. |
...
| ScanScore | 3 TARGETED_TEST | DTO / rule / review-only calculator skeleton and tests exist. | No production ScanScore over live scan output, and no new Evidence / Score entry point from P294 scan output is merged yet. | Review-only Scan Output to Evidence / Score Entry before broader score runtime. | Not production scoring. |
