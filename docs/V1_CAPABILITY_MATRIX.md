...
| Evidence normalization | REVIEW_ONLY_EVIDENCE_NORMALIZATION_SKELETON | Active P296 PR #727 adds a review-only normalized evidence DTO and assembler from `ReviewOnlyEvidenceScoreEntryDTO`. | No real EvidenceItem generation, no DB write, no ScoreItem generation, no score calculation, no Candidate, no Push, no Readiness, and no point generation. | Score input / precheck review-only slice after P296 merges. | Not real EvidenceItem, not persisted evidence, not score calculation, not Candidate, not Push, not Readiness. |
...
| ScanScore | 3 TARGETED_TEST | DTO / rule / review-only calculator skeleton and tests exist. | No production ScanScore over live scan output, and P295/P296 only add entry and normalization envelopes, not score calculation. | Score input / precheck review-only slice before broader score runtime. | Not production scoring. |
